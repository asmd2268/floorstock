...
async function prepare(page, profile) {
  ...
  await page.evaluate(({ who, visibleSource }) => {
    ...
    Object.assign(window.S.cache, {
      ...
      crash_cart_reports: [{ id: 'rep-a', cartId: 'cart-a', deptId: 'dept-a', status: 'open', openedAt: today, reason: 'Cardiac arrest', consumed: [{ itemId: 'item-a', qty: 1 }] }],
      ...
      accountability_usage_v2: [{ id: 'use-a', assignmentId: 'asg-a', deptId: 'dept-a', medName: 'Morphine 10mg', units: 2, status: 'pending_pharmacy', submittedAt: today, consumptionDate: day }],
      dept_notes: [{ id: 'note-a', deptId: 'dept-a', status: 'open', body: 'Check stock', createdAt: today }],
  ...
  }, { who: profile, visibleSource: VISIBLE });
}
...
test('no role is shown a control whose only answer is "no permission"', async ({ page }) => {
      ...
      for (const control of controls) {
        ...
        for (const dialogControl of inModal) {
          ...
          if (dialogRefusal && REFUSAL.test(dialogRefusal)) {
            findings.push(`${profile.role} · ${pageId} · dialog "${dialogControl.label}" → ${dialogRefusal.split('\n')[0].slice(0, 80)}`);
          }
        }
  ...
  console.log(`Walked ${walked.join(' | ')}\nPressed ${pressed} control(s) and ${dialogsSeen} dialog(s). Pages with none: ${noControls.length}`);
  ...
  expect(dialogsSeen, 'no dialog opened, so no dialog control was audited').toBeGreaterThan(10);
