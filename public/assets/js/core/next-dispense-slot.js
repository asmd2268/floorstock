...
function getNextDispSlot(deptId){
  ...
  for(var d=0;d<7;d++){var day=(dow+d)%7;slots.forEach(function(s){if((s.days||[]).indexOf(day)<0)return;var sm=globalThis.timeToMins(s.time),diff=d*1440+(sm-nowMins),offset=d;if(diff<=0&&d===0){diff+=1440;offset=1}if(diff>0&&diff<bestDiff){var scheduled=new Date(now);scheduled.setDate(now.getDate()+offset);scheduled.setHours(Math.floor(sm/60),sm%60,0,0);bestDiff=diff;best={slot:s,day:names[day],time:globalThis.fmt12(s.time),minsAway:diff,scheduledAt:scheduled.toISOString()}}})}
  return best;
}
