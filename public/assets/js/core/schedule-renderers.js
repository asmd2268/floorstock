...
function renderDispenseSlotCard(s,i){
  ...
  return '<div class="win-card"><div><div style="font-weight:600">'+s.label+'</div><div class="win-days">'+dept+' &nbsp;|&nbsp; '+globalThis.dayBits(s.days||[])+notes+'</div></div><div class="win-time">⏰ '+timeDisplay+'</div><div class="fl g8 ic"><button class="btn bg bxs" data-i="'+i+'" onclick="editDispSlot(+this.dataset.i)">✏</button><button class="btn bd2c bxs" data-i="'+i+'" onclick="delSlot(+this.dataset.i)">✕</button></div></div>';
}
