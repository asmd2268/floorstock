...
function renderRequestWindowCard(w,i){
  ...
  return '<div class="win-card'+(w.active?'':' inactive')+'"><div><div style="font-weight:600">'+w.label+'</div><div class="win-days">'+dept+' &nbsp;|&nbsp; '+globalThis.dayBits(w.days||[])+'</div></div><div class="win-time">'+globalThis.fmt12(w.from)+' &ndash; '+globalThis.fmt12(w.to)+'</div><div class="fl g8 ic"><button class="btn bg bxs" data-i="'+i+'" data-clickact="editReqWindow">✏</button><button class="btn '+(w.active?'bg':'bp')+' bxs" data-i="'+i+'" data-clickact="toggleWindow">'+(w.active?'Pause':'Enable')+'</button><button class="btn bd2c bxs" data-i="'+i+'" data-clickact="delWindow">✕</button></div></div>';
}
function renderDispenseSlotCard(s,i){
  var dept=globalThis.scheduleDepartmentName(s.dept), notes=s.notes?' &nbsp;|&nbsp; <i>'+s.notes+'</i>':'';
  var from=s.from||s.time,timeDisplay=from?(s.to?globalThis.fmt12(from)+' &ndash; '+globalThis.fmt12(s.to):globalThis.fmt12(from)):'—';
  return '<div class="win-card"><div><div style="font-weight:600">'+s.label+'</div><div class="win-days">'+dept+' &nbsp;|&nbsp; '+globalThis.dayBits(s.days||[])+notes+'</div></div><div class="win-time">⏰ '+timeDisplay+'</div><div class="fl g8 ic"><button class="btn bg bxs" data-i="'+i+'" data-clickact="editDispSlot">✏</button><button class="btn bd2c bxs" data-i="'+i+'" data-clickact="delSlot">✕</button></div></div>';
}
