/* The expiry tracking rules: early warning, near expiry, critical.

   Three numbers that every controlled screen colours by, edited in one small
   modal by whoever may manage controlled medicines. The order between them is
   enforced on save — critical ≤ near ≤ early — because a set that crosses over
   would colour rows by a rule nobody could read off the screen.

   Merged into modules/51 during an earlier consolidation and moved out here with
   the rest of it. */
...
/* Registered from this IIFE — the scope that declares the modal / من نفس النطاق. */
installActions(document.body,{aaFinalCloseExpiryRules:function(){CM('aa-final-expiry-rules-modal')},aaFinalSaveExpiryRules:function(){return window.aaFinalSaveExpiryRules()}},{event:'click',attribute:'clickact'});
})();
