...
function renderReqs(){
  ...
  if((window.fsEffectiveRole?window.fsEffectiveRole():String((window.CU&&CU.role)||''))==='outpatient_pharmacy_supervisor'){var opd=String((window.CU&&window.CU.deptId)||'')||(window.fsOutpatientDeptId?window.fsOutpatientDeptId():'');if(opd)rs=rs.filter(function(r){return String(r.deptId)===String(opd)})}
...
function mrSetPeriod(p,btn){MR_PERIOD=p;document.querySelectorAll('.mr-fbtn').forEach(function(b){b.classList.remove('on')});if(btn)btn.classList.add('on');renderMyReqs();};
...
installActions(document.body,{
  mrSetPeriod:function(el){mrSetPeriod(el.dataset.period,el)}
