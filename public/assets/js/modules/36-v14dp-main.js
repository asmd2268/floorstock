...
function crashAnnualSection(fy,ty){var a=typeof window.crashReports==='function'?(window.crashReports()||[]):[],rows=a.filter(function(r){var y=new Date(r.openedAt||r.created||0).getFullYear();return y>=fy&&y<=ty}),open=rows.filter(function(r){return r.status==='open'}).length,closed=rows.filter(function(r){return r.status==='closed'}),repl=rows.reduce(function(n,r){return n+(Array.isArray(r.replacements)?r.replacements.length:0)},0),mins=closed.map(function(r){var x=new Date(r.openedAt||r.created||0),y=new Date(r.closedAt||r.lastEditedAt||0);return y>x?(y-x)/60000:0}).filter(function(x){return x>0}),avg=mins.length?mins.reduce(function(a,b){return a+b},0)/mins.length:0;return '<section><h2>Crash Cart safety performance / أداء عربات الطوارئ</h2><div class="summary"><b>'+rows.length+'</b> reports · <b>'+open+'</b> open · <b>'+repl+'</b> replacement medication events · <b>'+(avg?avg.toFixed(1):'—')+'</b> min average response</div><table><tr><th>Metric</th><th>Value</th></tr><tr><td>Opened reports</td><td>'+rows.length+'</td></tr><tr><td>Closed reports</td><td>'+closed.length+'</td></tr><tr><td>Open reports</td><td>'+open+'</td></tr><tr><td>Replacement medications</td><td>'+repl+'</td></tr><tr><td>Average report-to-closure time</td><td>'+(avg?avg.toFixed(1)+' minutes':'No completed intervals')+'</td></tr></table></section>'}
