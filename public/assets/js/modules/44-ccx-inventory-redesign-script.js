...
window.renderCrashCarts=function(){function after(){[window.refreshCrashBulkUi,window.refreshCrashSupervisorBulkButton,window.refreshCrashFilteredBulkUi,window.refreshCrashOpeningLogUi,window.ensureCrashRecoveryButton,window.enhanceCrashButtons].forEach(function(fn){if(typeof fn==='function')fn()})}if(!ensureUI()){after();return};var all=(typeof crashCarts==='function'?crashCarts():[])||[],reports=(typeof crashReports==='function'?crashReports():[])||[];var scopedOutpatient=CU&&CU.role==='outpatient_pharmacy_supervisor',scopeDept=scopedOutpatient?String(CU.deptId||''):'';var open=reports.filter(function(r){return r.status==='open'&&(isDepartment()||scopedOutpatient?String(r.deptId)===scopeDept:true)});var deptFilter=isDepartment()||scopedOutpatient?scopeDept:String((E('ccx-dept')||{}).value||''),state=String((E('ccx-state')||{}).value||''),expiry=String((E('ccx-expiry')||{}).value||''),search=String((E('ccx-search')||{}).value||'').toLowerCase().trim();var carts=all.filter(function(c){if((isDepartment()||scopedOutpatient)&&String(c.deptId)!==scopeDept)return false;if(deptFilter&&String(c.deptId)!==deptFilter)return false;var rep=open.find(function(r){return String(r.cartId)===String(c.id)});if(state==='open'&&!rep)return false;if(state==='closed'&&rep)return false;var cl=cartLevel(c);if(expiry&&cl!==expiry&&!((c.items||[]).some(function(it){return itemLevel(it)===expiry})))return false;if(search){var hay=[c.name,c.number,c.location,deptName(c.deptId)].concat((c.items||[]).map(function(it){var m=meta(it);return m.generic+' '+m.concentration})).join(' ').toLowerCase();if(hay.indexOf(search)<0)return false}return true});
