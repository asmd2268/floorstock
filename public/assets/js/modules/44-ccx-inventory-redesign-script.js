...
function ccxInstallActions(root){if(!root)return; /* one listener per event: core/delegated-actions.js */installActions(root,{ccxOpenReport:function(el,event){ccxOpenReport(el.dataset.a1)},ccxOpenFromList:function(el,event){if(typeof showPg==='function')showPg('pg-crashcart');ccxOpenReport(el.dataset.a1)},ccTabCarts:function(){window.ccTabCarts()},ccTabOps:function(){window.ccTabOps()},crashPrint:function(el,event){crashPrint(el.dataset.a1)},crashAddItem:function(el,event){crashAddItem(el.dataset.a1)},crashReportOpen:function(el,event){crashReportOpen(el.dataset.a1)},showPg:function(el,event){showPg(el.dataset.a1)},moveManagedCategory:function(el,event){moveManagedCategory(el.dataset.a1,1)},removeManagedCategory:function(el,event){removeManagedCategory(el.dataset.a1)},openManageCats:function(el,event){openManageCats()},ccxAcceptReport:function(el){if(typeof window.ccAcceptReport==='function')window.ccAcceptReport(el.dataset.a1)},ccxRejectReport:function(el){if(typeof window.ccRejectReport==='function')window.ccRejectReport(el.dataset.a1)}},{event:'click',attribute:'clickact'});installActions(root,{renameManagedCategory:function(el,event){renameManagedCategory(el.dataset.a1,el.value)}},{event:'change',attribute:'changeact'});}
function mkTabBar(tabs,activeId){ccxInstallActions(document.body);var bar=document.createElement('div');bar.style.cssText='display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap';bar.innerHTML=tabs.map(function(t){var on=t[0]===activeId;return '<button class="btn '+(on?'bp':'bg')+' bsm" '+(on?'disabled':'')+' data-clickact="'+t[2]+'">'+(on?'<b>'+t[1]+'</b>':t[1])+'</button>'}).join('');return bar}
...
function ensureUI(){var pg=E('pg-crashcart'),alerts=E('crash-open-alerts'),list=E('crash-list');if(!pg||!alerts||!list)return false;
 ...
 var minSealBtn=E('ccx-min-seal');if(minSealBtn)minSealBtn.style.display=(typeof window.isMaster==='function'&&window.isMaster())?'':'none';
 var dsel=E('ccx-dept');if(dsel){var current=dsel.value;var dselDepts=typeof window.fsAllowedDepts==='function'?window.fsAllowedDepts():(typeof gd==='function'?gd():[]);var opts='<option value="">All departments / كل الأقسام</option>'+dselDepts.map(function(d){return '<option value="'+escx(d.id)+'">'+escx(d.name)+'</option>'}).join('');if(dsel.innerHTML!==opts)dsel.innerHTML=opts;if(isDepartment()){dsel.value=CU.deptId||'';dsel.disabled=true}else{dsel.disabled=false;if(current)dsel.value=current}}
...
function reportCard(r,c){
  ...
  /* A pending report is waiting for the pharmacy to approve it: accepting
     deducts the reported quantities from the cart, rejecting returns it with a
     note. Both calls live in module 52 and have always worked — the buttons
     that reached them were dropped when modules 27/43/45/63/66 were merged, so
     the workflow had a server, a client and no way in. */
  var approval=isPending&&canOperate
    ?'<div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap">'
      +'<button class="btn bs bsm" data-clickact="ccxAcceptReport" data-a1="'+escx(r.id)+'">✔ قبول / Accept</button>'
      +'<button class="btn bd2c bsm" data-clickact="ccxRejectReport" data-a1="'+escx(r.id)+'">✕ رفض / Reject</button>'
      +'</div>'
    :'';
  var actions=(isPending||r.status==='open')&&canOperate
    ?'<div style="margin-top:8px"><button class="btn bs bsm" data-clickact="ccxOpenReport" data-a1="'+escx(r.id)+'">↻ Respond to report / الرد على البلاغ</button></div>'
    :(!canOperate)?'':'<div style="margin-top:8px"><span class="btn bd2c bsm" data-clickact="ccxOpenReport" data-a1="'+escx(r.id)+'">Open and respond / فتح والرد</span></div>';
  ...
  return '<div class="ccx-alert-card"><div class="ccx-alert-title">⚠ '+escx(deptName(r.deptId))+' — '+escx((c&&c.name)||'Crash Cart')+badge+ncBadge+'</div><div class="fhint">'+escx(r.reason||'Opening report')+'</div>'+approval+'<div class="fhint">'+escx(r.openedBy||'')+' · '+escx(fmtWhen(r.openedAt))+'</div>'+actions+'</div>'
}
...
window.renderCrashCarts=function(){ccxInstallActions(document.body);function after(){[window.refreshCrashBulkUi,window.refreshCrashSupervisorBulkButton,window.refreshCrashFilteredBulkUi,window.refreshCrashOpeningLogUi,window.ensureCrashRecoveryButton,window.enhanceCrashButtons,window.refreshCrashDeletionControls].forEach(function(fn){if(typeof fn==='function')fn()});(window.__renderCrashCartsAfterExtensions||[]).forEach(function(fn){try{fn()}catch(e){console.error('renderCrashCarts extension failed',e)}})}if(!ensureUI()){after();return};var all=(typeof crashCarts==='function'?crashCarts():[])||[],reports=(typeof crashReports==='function'?crashReports():[])||[];var departmentScoped=isDepartmentRole()||(CU&&CU.role==='outpatient_pharmacy_supervisor'),scopeDept=departmentScoped?String(CU&&CU.deptId||''):'';if(departmentScoped&&all.length)window.__ccxLastScopedCarts=all.slice();else if(departmentScoped&&!all.length&&Array.isArray(window.__ccxLastScopedCarts))all=window.__ccxLastScopedCarts.slice();if(departmentScoped&&reports.length)window.__ccxLastScopedReports=reports.slice();else if(departmentScoped&&!reports.length&&Array.isArray(window.__ccxLastScopedReports))reports=window.__ccxLastScopedReports.slice();/* Why a pending report is not on screen, counted at each stage it can be lost.
   Twice now a report was invisible to one account and visible to another, and
   every stage looks identical from outside: no rows loaded, a status that is not
   open, a department the viewer may not see, or a preview role quietly standing
   in for the real one. The counts are kept so the page can say which. */
 var ccxDrop={loaded:reports.length,notActive:0,otherDept:0,noDeptAccess:0};
 var open=reports.filter(function(r){
  if(!(r.status==='open'||r.status==='pending')){ccxDrop.notActive++;return false}
  if(departmentScoped&&String(r.deptId)!==scopeDept){ccxDrop.otherDept++;return false}
  if(typeof window.fsCanAccessDepartment==='function'){
   var _c=all.find(function(x){return String(x.id)===String(r.cartId)});
   var _deptId=(_c&&_c.deptId)||r.deptId;
   if(!window.fsCanAccessDepartment(_deptId)){ccxDrop.noDeptAccess++;return false}
  }
  return true;
 });
 window.__ccxReportDiagnostics=ccxDrop;var deptFilter=departmentScoped?scopeDept:String((E('ccx-dept')||{}).value||''),state=String((E('ccx-state')||{}).value||''),expiry=String((E('ccx-expiry')||{}).value||''),search=String((E('ccx-search')||{}).value||'').toLowerCase().trim();var stateScoped=departmentScoped&&CU&&CU.role==='department'&&typeof S!=='undefined'&&S.cache&&S.cache.__scopedDepartmentState===true;var carts=all.filter(function(c){if(departmentScoped&&!stateScoped&&String(c.deptId)!==scopeDept)return false;if(typeof window.fsCanAccessDepartment==='function'&&!window.fsCanAccessDepartment(c.deptId))return false;if(deptFilter&&String(c.deptId)!==deptFilter)return false;var rep=open.find(function(r){return String(r.cartId)===String(c.id)});if(state==='open'&&!rep)return false;if(state==='closed'&&rep)return false;var cl=cartLevel(c);if(expiry&&cl!==expiry&&!((c.items||[]).some(function(it){return itemLevel(it)===expiry})))return false;if(search){var hay=[c.name,c.number,c.location,deptName(c.deptId)].concat((c.items||[]).map(function(it){var m=meta(it);return m.generic+' '+m.concentration})).join(' ').toLowerCase();if(hay.indexOf(search)<0)return false}return true});
 var diag=E('pg-crashcart');if(diag){diag.dataset.crashStateSource=String((typeof S!=='undefined'&&S.transport)||'unknown');diag.dataset.crashStateRaw=String((typeof S!=='undefined'&&S.cache&&Array.isArray(S.cache.crash_carts)?S.cache.crash_carts.length:0));diag.dataset.crashStateFunction=String(all.length);
  // Enough to answer "where did the report go?" from a screenshot of the page.
  diag.dataset.crashReportsLoaded=String(ccxDrop.loaded);diag.dataset.crashReportsShown=String(open.length);
  diag.dataset.crashReportsDropped=ccxDrop.notActive+'/'+ccxDrop.otherDept+'/'+ccxDrop.noDeptAccess;
  diag.dataset.crashPreviewRole=String(!!window.MASTER_EFFECTIVE)}var add=E('crash-add-btn');if(add)add.style.display=canEditContents()?'inline-flex':'none';
 /* An empty alert strip means "nothing to answer" — unless something was
    dropped on the way here, in which case saying so is the difference between a
    quiet screen and a department waiting on a master who cannot see them. */
 var ccxCanOperate=typeof window.fsHasCapability==='function'&&window.fsHasCapability('crashCart.operate');
 var ccxNote='';
 if(ccxCanOperate&&!open.length){
  var lost=[];
  if(ccxDrop.notActive)lost.push(ccxDrop.notActive+' already answered / تم الرد عليها');
  if(ccxDrop.otherDept)lost.push(ccxDrop.otherDept+' from another department / من قسم آخر');
  if(ccxDrop.noDeptAccess)lost.push(ccxDrop.noDeptAccess+' in a department you cannot view / في قسم لا تملك صلاحية عرضه');
  var preview=!!window.MASTER_EFFECTIVE;
  if(!ccxDrop.loaded){
   ccxNote='<div class="fhint" style="padding:8px 12px;border:1px dashed var(--bd);border-radius:8px">No Crash Cart reports have loaded in this session'
     +' <span dir="ltr">('+escx(String((typeof S!=='undefined'&&S.transport)||'unknown'))+')</span>.'
     +' If a department is waiting on one, refresh the page.<br/>لم تُحمَّل أي بلاغات في هذه الجلسة. إن كان هناك قسم ينتظر، حدّث الصفحة.</div>';
  }else if(lost.length||preview){
   ccxNote='<div class="fhint" style="padding:8px 12px;border:1px dashed var(--bd);border-radius:8px">'
     +escx(ccxDrop.loaded)+' report(s) loaded, none shown here'
     +(lost.length?': '+escx(lost.join(' · ')):'')
     +(preview?'<br/><b>A test role is active</b> — reports are filtered as that user sees them. Clear it from “Test User / Role”.<br/><b>وضع تجربة دور مفعّل</b> — البلاغات تُعرض كما يراها ذلك المستخدم. ألغِه من زر تجربة المستخدم.':'')
     +'</div>';
  }
 }
 E('crash-open-alerts').innerHTML=(open.length?'<div class="ccx-alerts-grid">'+open.map(function(r){return reportCard(r,all.find(function(c){return String(c.id)===String(r.cartId)}))}).join('')+'</div>':'')+ccxNote;
 var html=renderSummary(carts,open)+carts.map(function(c){var rep=open.find(function(r){return String(r.cartId)===String(c.id)}),level=cartLevel(c),closed=latestClosedReport(c.id);var allItems=(c.items||[]).slice(),items=(expiry?allItems.filter(function(it){return itemLevel(it)===expiry}):allItems).sort(function(a,b){var ma=meta(a),mb=meta(b);var oa=CCX_ORDER[ma.generic],ob=CCX_ORDER[mb.generic];if(oa==null)oa=999;if(ob==null)ob=999;return oa-ob||ma.generic.localeCompare(mb.generic)});var actions='<button class="btn bg bsm" data-clickact="crashPrint" data-a1="'+escx(c.id)+'">🖨 Print</button>';if(canEditContents())actions+='<button class="btn bp bsm" data-clickact="crashAddItem" data-a1="'+escx(c.id)+'">+ Add medication</button>';if(isDepartmentRole()&&!rep)actions+='<button class="btn bd2c bsm" data-clickact="crashReportOpen" data-a1="'+escx(c.id)+'">Report opened cart</button>';if(canManage()&&rep)actions+='<button class="btn bs bsm" data-clickact="ccxOpenReport" data-a1="'+escx(rep.id)+'">Respond to report</button>';var closedMeta=(closed&&!isDepartmentRole())?'<div class="ccx-closed-meta"><b>Last closure / آخر إغلاق:</b> '+escx(closed.closedByName||closed.closedBy||'—')+' · <b>User:</b> '+escx(closed.closedByUser||'—')+' · '+escx(fmt(closed.closedAt||closed.lastEditedAt))+' · <b>Seal:</b> '+escx(closed.newSeal||c.seal||'—')+'</div>':'';return '<div id="ccx-cart-'+escx(c.id)+'" class="card ccx-cart ccx-'+level+' '+(rep?'crash-open':'')+'"><div class="ch"><div><span class="ct">'+escx(c.name||'Crash Cart')+'</span><div class="fhint">'+escx(deptName(c.deptId))+' · '+escx(c.number||'—')+' · '+escx(c.location||'—')+' · Seal '+escx(c.seal||'—')+'</div>'+(expiry?'<div class="ccx-filter-note">Filtered rows: '+escx(levelLabel(expiry))+' · '+items.length+' medication(s)</div>':'')+closedMeta+'</div><div class="ccx-toolbar-actions">'+actions+'</div></div><div class="tw"><table class="ccx-table"><thead><tr><th>#</th><th>Generic name / الاسم العلمي</th><th>Concentration / التركيز</th><th>Standard qty</th><th>Present qty</th><th>Expiry → Qty</th><th>Status</th></tr></thead><tbody>'+items.map(itemRow).join('')+'</tbody></table></div></div>'}).join('');E('crash-list').innerHTML=html||'<div class="card"><div class="cb" style="text-align:center;color:var(--tx2)">No Crash Carts match the selected filters.</div></div>';if(typeof window.ccUpdateBadges==='function')window.ccUpdateBadges();window.ccxRenderDashboardAlerts();after()
...
window.ccxRenderDashboardAlerts=function(){
  ...
  var allActive=(typeof crashReports==='function'?crashReports():[]).filter(function(r){if(!(r.status==='open'||r.status==='pending'))return false;if(typeof window.fsCanAccessDepartment==='function'){var _c=typeof crashCart==='function'?crashCart(r.cartId):null;var _deptId=(_c&&_c.deptId)||r.deptId;if(!window.fsCanAccessDepartment(_deptId))return false;}return true;});
  ...
  var rows=allActive.map(function(r){
    ...
    var ncBadge=r.noConsumption?'<span dir="ltr" style="background:var(--bl,#3498db);color:#fff;border-radius:4px;padding:1px 5px;font-size:10px;font-weight:700;margin-left:4px">🔵 No Consumption · لا يوجد استهلاك</span>':'';
    var btn=(isPending||r.status==='open')&&isPharmacy()
      ?'<button class="btn bs bsm" data-clickact="ccxOpenFromList" data-a1="'+escx(r.id)+'">↻ Respond / الرد</button>'
      :(isPending||r.status==='open')?''
      :'<span class="btn bd2c bsm" data-clickact="ccxOpenFromList" data-a1="'+escx(r.id)+'">Open</span>';
    return '<div class="ccx-dashboard-report"><div><b>'+escx(deptName(r.deptId))+' — '+escx((c&&c.name)||'Crash Cart')+badge+ncBadge+'</b><div class="fhint">'+escx(r.reason||'Opening report')+'</div></div>'+btn+'</div>';
  }).join('');
