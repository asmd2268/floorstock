...
window.crashPrint=function(id){
  ...
  var publicUrl=new URL(typeof getAppUrl==='function'?getAppUrl():location.href);publicUrl.searchParams.set('view','crash-cart-public');publicUrl.searchParams.set('id',String(c.id));
  var qr=typeof window.makeReadableQR==='function'?window.makeReadableQR(publicUrl.toString()):'https://api.qrserver.com/v1/create-qr-code/?size=600x600&format=png&color=000000&bgcolor=ffffff&qzone=4&ecc=H&data='+encodeURIComponent(publicUrl.toString());
  ...
  '</style></head><body><div class="toolbar"><button onclick="window.print()">Print / طباعة</button></div><div class="page">'+official+
  '<div class="title-grid"><div class="titles"><h1>'+e(c.name||'Crash Cart')+'</h1><h2>'+e(dn)+'</h2></div><div class="qr"><img id="crash-live-qr" src="'+e(qr)+'" alt="Live Crash Cart QR"><small>Live check — no login</small></div></div>'+
  ...
  '<script>(function(){var done=false;function go(){if(done)return;done=true;setTimeout(function(){window.focus();window.print()},180)}var image=document.getElementById("crash-live-qr");if(image&&!image.complete){image.onload=go;image.onerror=go;setTimeout(go,2200)}else{go()}})();<\/script></body></html>';
