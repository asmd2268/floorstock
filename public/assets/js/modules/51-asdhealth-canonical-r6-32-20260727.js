...
function fsR5OrdersHtml(orders){
  ...
  var safeJson=JSON.stringify(printData)
    .replace(/</g,'\\u003c')
...
function buildLandscapePdf(jpegBytes,pixelWidth,pixelHeight){
...
async function start(){
  ...
  var pdfBytes=buildLandscapePdf(
    jpegBytes,
  ...
  if(window.__PRINT_TEST_MODE__){
    ...
    var status=document.getElementById('status');
    if(status)status.textContent='Landscape PDF ready.';
    return;
  }

  var status=document.getElementById('status');
  if(status)status.textContent='Opening the final A4 landscape PDF…';
  setTimeout(function(){
...
window.addEventListener('load',function(){
  ...
  return '<!doctype html><html><head><meta charset="utf-8">'+
    ...
    '<title>Print Orders — Preparing Landscape PDF</title>'+
    '<style>html,body{margin:0;width:100%;height:100%;background:#fff;font-family:Arial,Tahoma,sans-serif}#status{display:flex;align-items:center;justify-content:center;min-height:100%;padding:24px;color:#111;text-align:center}</style>'+
    '</head><body><canvas id="page-canvas" hidden></canvas><div id="status">Preparing the final A4 landscape PDF…</div>'+
...
window.doPrint=function(){
  ...
  popup.document.open();popup.document.write(fsR5OrdersHtml(orders));popup.document.close();
  try{
...
function fsR5ControlledPrintHtml(dept,rows){
  ...
  var qr='https://api.qrserver.com/v1/create-qr-code/?size=600x600&format=png&color=000000&bgcolor=ffffff&qzone=4&ecc=H&data='+
...
@media screen and (orientation:portrait){
  ...
  var content='<div class="fit">'+official+
    '<header class="document-head">'+
      ...
      '</div>'+
      '<img class="qr" src="'+fsR5Esc(qr)+'" alt="Live controlled list QR">'+
    '</header>'+
    legend+
    '<table>'+
      '<colgroup>'+
