...
function enhanceCrash(){var reason=E('ccr-reason');if(reason){Array.from(reason.options||[]).forEach(function(o){var v=String(o.value||'').toLowerCase();if(v==='other')o.textContent='Other / سبب آخر'});if(!E('v16-crash-other')){var inp=document.createElement('input');inp.id='v16-crash-other';inp.placeholder='Other reason / السبب الآخر';inp.style.display='none';reason.parentNode.appendChild(inp);reason.addEventListener('change',function(){inp.style.display=String(reason.value).toLowerCase()==='other'?'':'none'})}}
 // The detailed per-report alert card (module 44's reportCard(), with its
 // role-gated Accept/Reject/Open-and-respond actions) is the authoritative
 // renderer for #crash-open-alerts. A legacy plain-text summary used to
 // overwrite it here for master/pharmacy/inpatient_supervisor (mgr()),
 // silently hiding the real actions from exactly the roles that need them.
}
...
/* ══════════════════════════════════════════════════════════════
   ...
   - نسخ renderPrint
══════════════════════════════════════════════════════════════ */
var _v14PrintFilter='today'; /* default: today's fulfilled — the day's printing is the common case */
...
function renderPrintTable(){
  ...
  if((window.fsEffectiveRole?window.fsEffectiveRole():String((window.CU&&CU.role)||''))==='outpatient_pharmacy_supervisor'){var opd=String((window.CU&&window.CU.deptId)||'')||(window.fsOutpatientDeptId?window.fsOutpatientDeptId():'');if(opd)allReqs=allReqs.filter(function(r){return String(r.deptId)===String(opd)})}
