...
/* This file is a stack of closures, so each one installs the actions for the
   markup it builds; installActions merges them into the one table on the root.
   v13XSaveStock is read from window at click time: module 40 replaces it at load. */
function v13xInstallActions(root){if(!root)return;
  installActions(root,{v13XClose:function(el){v13XClose(el.dataset.a1)},v13XAddBatch:function(el){v13XAddBatch(el.dataset.a1)},v13XSaveCatalogMed:function(){v13XSaveCatalogMed()},v13XSaveStock:function(){window.v13XSaveStock()}},{event:'click',attribute:'clickact'});
  installActions(root,{valQ:function(el){valQ(el)}},{event:'input',attribute:'inputact'});
}
function ensureModalsX(){v13xInstallActions(document.body);
 if(!E('v13x-med-modal')){var m=document.createElement('div');m.id='v13x-med-modal';m.className='modal-bg v13x-modal';m.innerHTML='<div class="modal"><div class="mh"><span class="mt">Edit controlled medicine / تعديل الدواء المقيد</span><button class="xbtn" data-clickact="v13XClose" data-a1="v13x-med-modal">×</button></div><input type="hidden" id="v13x-med-id"><div class="v13x-grid"><div><label>Medicine name</label><input id="v13x-med-name"></div><div><label>Type</label><select id="v13x-med-class"><option value="narcotic">Narcotic</option><option value="psychotropic">Psychotropic</option></select></div><div><label>MOH code</label><input id="v13x-med-moh"></div><div><label>NUPCO code</label><input id="v13x-med-nupco"></div><div><label>Minimum quantity</label><input type="number" id="v13x-med-min"></div><div><label>Maximum quantity</label><input type="number" id="v13x-med-max"></div></div><div class="v13x-toolbar"><label><input type="checkbox" id="v13x-med-ha" style="width:auto;margin:0"> High Alert</label><label><input type="checkbox" id="v13x-med-lasa" style="width:auto;margin:0"> LASA</label><label><input type="checkbox" id="v13x-med-fridge" style="width:auto;margin:0"> Refrigerator</label><label><input type="checkbox" id="v13x-med-hazard" style="width:auto;margin:0"> Hazard</label></div><div style="margin-top:12px"><div class="fl jb ic"><b>Reference expiry dates / تواريخ الانتهاء المرجعية</b><button type="button" class="btn bg bsm" data-clickact="v13XAddBatch" data-a1="v13x-med-batches">+ Add expiry</button></div><div id="v13x-med-batches" class="v13x-batches" style="margin-top:8px"></div></div><div class="fl g8" style="justify-content:flex-end;margin-top:16px"><button class="btn bg" data-clickact="v13XClose" data-a1="v13x-med-modal">Cancel</button><button class="btn bs" data-clickact="v13XSaveCatalogMed">Save</button></div></div>';document.body.appendChild(m)}
 if(!E('v13x-stock-modal')){var s=document.createElement('div');s.id='v13x-stock-modal';s.className='modal-bg v13x-modal';s.innerHTML='<div class="modal"><div class="mh"><span class="mt" id="v13x-stock-title">Edit custody</span><button class="xbtn" data-clickact="v13XClose" data-a1="v13x-stock-modal">×</button></div><input type="hidden" id="v13x-stock-id"><input type="hidden" id="v13x-stock-scope"><div class="v13x-qty"><div><label id="v13x-stock-qty-label">Actual quantity</label><input type="number" id="v13x-stock-qty" min="0"></div><div id="v13x-required-wrap"><label>Required quantity / الكمية المفروض توفرها</label><input type="number" id="v13x-stock-required" min="0"></div></div><div class="fl jb ic"><b>Expiry date → quantity / تاريخ الانتهاء ← الكمية</b><button type="button" class="btn bg bsm" data-clickact="v13XAddBatch" data-a1="v13x-stock-batches">+ Add expiry</button></div><div id="v13x-stock-batches" class="v13x-batches" style="margin-top:8px"></div><div class="fl g8" style="justify-content:flex-end;margin-top:16px"><button class="btn bg" data-clickact="v13XClose" data-a1="v13x-stock-modal">Cancel</button><button class="btn bs" data-clickact="v13XSaveStock">Save custody</button></div></div>';document.body.appendChild(s)}

 document.addEventListener('keydown',function(e){if(e.key==='Escape'){['v13x-med-modal','v13x-stock-modal'].forEach(closeX)}})
}
function v13XClose(id){closeX(id)}
function v13XAddBatch(id){addBatchRowX(E(id),{})}
...
async function v13XSaveCatalogMed(){
...
/* The New Request table is rebuilt by refreshCurrentPage() on every state
   ...
   accountability_*, notes) still triggered a full innerHTML rebuild, so the
   table visibly flashed several times before the page settled. Writing only
   when the markup actually differs makes those refreshes no-ops, and keeps
   quantities already typed into the inputs from being discarded. */
function setReqHtml(node,html){if(!node)return;if(node.innerHTML===html)return;node.innerHTML=html}
window.renderReqForm=function(){v13xInstallActions(document.body);function after(){if(typeof window.refreshRequestIdleTimer==='function')window.refreshRequestIdleTimer();if(typeof window.refreshRequestCountLimitWarning==='function')window.refreshRequestCountLimitWarning();if(typeof window.cleanRequestFormUi==='function')window.cleanRequestFormUi();if(typeof window.refreshRequestScheduleMessage==='function')window.refreshRequestScheduleMessage()}var root=E('rfbody'),sub=E('dnsub');if(!root){after();return}var d=resolveDeptX();if(!d){setReqHtml(root,'<div class="alert-banner">No department is linked to this account. Assign a department in User Management.</div>');after();return}if(window.CU){CU.deptId=d.id;CU.deptName=d.name||''}if(sub)sub.textContent=(d.name||'Department')+' — Enter quantities needed';var meds=typeof window.getMeds==='function'?(getMeds(d.id)||[]):[];var q=((E('rsrch')||{}).value||'').toLowerCase().trim();meds=meds.filter(function(m){return m&&m.id&&m.name&&(!q||String(m.name).toLowerCase().indexOf(q)>=0)});if(!meds.length){setReqHtml(root,'<div class="v13x-empty">No medicines are assigned to this department.</div>');if(E('rcnt'))E('rcnt').textContent='0';after();return}var groups={};meds.forEach(function(m){var c=m.category||'Uncategorized';(groups[c]||(groups[c]=[])).push(m)});var cats=Object.keys(groups);var cfg=typeof getPharmacyCategoryConfig==='function'?getPharmacyCategoryConfig(d.id):{order:[]};var order=cfg.order||[];cats.sort(function(a,b){var ai=order.indexOf(a),bi=order.indexOf(b);if(ai<0)ai=999;if(bi<0)bi=999;return ai-bi||String(a).localeCompare(String(b))});var reqHtml=cats.map(function(c){var items=groups[c]||[];items.sort(function(a,b){return String(a.name||'').localeCompare(String(b.name||''),'en',{sensitivity:'base',numeric:true})});return '<div class="cath">'+escX(c)+'</div><div class="tw"><table><thead><tr><th>#</th><th>Medication</th><th>Classification</th><th>Min</th><th>Max</th><th>Qty</th></tr></thead><tbody>'+items.map(function(m,i){var max=Math.max(0,numX(m.max));return '<tr class="'+(typeof window.rowCls==='function'?rowCls(m):'')+'"><td>'+(i+1)+'</td><td><b>'+escX(m.name)+'</b></td><td>'+(typeof window.bdg==='function'?bdg(m):'')+'</td><td>'+numX(m.min)+'</td><td>'+max+'</td><td><div class="qwrap"><input type="number" class="rqi" data-mid="'+escX(m.id)+'" data-max="'+max+'" min="0" max="'+max+'" placeholder="0" data-inputact="valQ"><span class="qlim">/'+max+'</span></div></td></tr>'}).join('')+'</tbody></table></div>'}).join('');setReqHtml(root,reqHtml);if(typeof window.cntItems==='function')cntItems();after()};
...
(function(){
  ...
  /* The announcement editor is built and removed as one node. */
  function annCloseEditor(){var editor=E('ann-editor');if(editor)editor.remove()}
  /* The actions for the announcements page. */
  function annInstallActions(root){if(!root)return;
    installActions(root,{openAnnouncementEditor:function(el){openAnnouncementEditor(el.dataset.a1)},toggleAnnouncement:function(el){toggleAnnouncement(el.dataset.a1)},deleteAnnouncement:function(el){deleteAnnouncement(el.dataset.a1)},saveAnnouncement:function(){saveAnnouncement()},annCloseEditor:function(){annCloseEditor()}},{event:'click',attribute:'clickact'});
    installActions(root,{toggleAnnouncementDepartments:function(){toggleAnnouncementDepartments()}},{event:'change',attribute:'changeact'});
  }
  function ensurePage(){annInstallActions(document.body);if(E('pg-announcements'))return;var p=document.createElement('div');p.className='pg';p.id='pg-announcements';p.innerHTML='<div class="fl ic jb mb14" style="flex-wrap:wrap;gap:10px"><div><div class="stitle">Announcements / الإعلانات والتنبيهات</div><div class="ssub" style="margin:0">إعلانات مؤقتة لكل الأقسام أو أقسام محددة، بالعربي والإنجليزي.</div></div><button class="btn bp" id="ann-new-btn" data-clickact="openAnnouncementEditor">+ New announcement / إعلان جديد</button></div><div id="announcements-list"></div>';var app=E('app');if(app)app.appendChild(p)}
  window.renderAnnouncements=function(){ensurePage();/* Hidden rather than shown-and-refused for a role that may read announcements but not write them. */var newBtn=E('ann-new-btn');if(newBtn)newBtn.style.display=canManage()?'':'none';var box=E('announcements-list');if(!box)return;var list=announcements().slice().sort(function(a,b){return parseMs(b.createdAt)-parseMs(a.createdAt)});if(!list.length){box.innerHTML='<div class="card"><div class="cb" style="text-align:center;color:var(--tx2)">لا توجد إعلانات حاليًا / No announcements yet</div></div>';return}box.innerHTML=list.map(function(a){var isOn=active(a),targets=a.allDepartments?'All departments / جميع الأقسام':(a.departmentIds||[]).map(deptName).join('، ');return '<div class="ann-card '+(isOn?'':'inactive')+'"><div class="ann-card-head"><div><div class="ann-card-title">'+(a.level==='urgent'?'⚠️ Urgent':'📢 Announcement')+' · '+(isOn?'Active / فعال':'Inactive / غير فعال')+'</div><div class="ann-targets">'+escA(targets)+'</div></div><div class="fl g8"><button class="btn bg bsm" data-clickact="openAnnouncementEditor" data-a1="'+escA(a.id)+'">Edit</button><button class="btn bg bsm" data-clickact="toggleAnnouncement" data-a1="'+escA(a.id)+'">'+(a.enabled===false?'Enable':'Disable')+'</button><button class="btn bd2c bsm" data-clickact="deleteAnnouncement" data-a1="'+escA(a.id)+'">Delete</button></div></div>'+(a.arabic?'<div class="ann-ar" style="margin-top:8px">'+escA(a.arabic)+'</div>':'')+(a.english?'<div class="ann-en">'+escA(a.english)+'</div>':'')+'<div class="ann-targets">Start: '+escA(new Date(a.startAt).toLocaleString())+' · End: '+escA(new Date(a.endAt).toLocaleString())+'</div></div>'}).join('')};
...
// --- Merged from 05-controlled-ui-compatibility-fixes.js (Phase 6 consolidation) ---
(function(){
  'use strict';
  const E=globalThis.E;
  ...
  /* The actions for the bulk-replacement modal. */
  function bulkInstallActions(root){if(!root)return;
    installActions(root,{CM:function(el){CM(el.dataset.a1)},runBulkReplacement:function(){runBulkReplacement()}},{event:'click',attribute:'clickact'});
    installActions(root,{bulkReplacementToggleLimits:function(){bulkReplacementToggleLimits()}},{event:'change',attribute:'changeact'});
  }
  function ensureModal(){bulkInstallActions(document.body);
    if(E('mbulk-replacement'))return;
    var bg=document.createElement('div');bg.className='modal-bg';bg.id='mbulk-replacement';
    bg.innerHTML='<div class="modal" style="width:820px;max-width:97vw"><div class="mh"><span class="mt">⇄ Bulk Floor Stock Medication Replacement</span><button class="xbtn" data-clickact="CM" data-a1="mbulk-replacement">✕</button></div>'+
      ...
      '<div class="fl jb"><button class="btn bg" data-clickact="CM" data-a1="mbulk-replacement">Cancel</button><button class="btn bp" data-clickact="runBulkReplacement">Apply replacement</button></div></div>';
  ...
  function bulkReplacementToggleLimits(){E('br-limits').style.display=E('br-override').checked?'grid':'none'};
  ...
  async function runBulkReplacement(){
