...
(function(){
  ...
  function ahClose(db){try{if(db)db.close()}catch(ignore){/* tearing down something already gone is not a failure */}}
...
function encodeValue(v){
  ...
  window.masterRestoreDatabase=async function(){
    ...
    }catch(e){console.error(e);status.textContent='Restore failed: '+(e.message||e);toast('Database restore failed','err');try{await S.init();if(typeof window.repairImportedDepartmentAliases==='function')await window.repairImportedDepartmentAliases()}catch(_){/* an optional source that is not loaded in this session */}btn.disabled=false}
  };
})();
