...
function ctlEnhInstallActions(root){if(!root)return; /* one listener per event: core/delegated-actions.js */installActions(root,{ctlPdfSetField:function(el,event){ctlPdfSetField(el.dataset.a1,el.dataset.a2,el.value)},ctlDispTypeChanged:function(el,event){ctlDispTypeChanged()},ctlSavePrintLogo:function(el,event){ctlSavePrintLogo(el.files[0])},ctlToggleDeptMed:function(el,event){ctlToggleDeptMed(el)},toggleShelfMedication:function(el,event){toggleShelfMedication(el.dataset.mid,el.checked)}},{event:'change',attribute:'changeact'});installActions(root,{removeParent:function(el){if(el.parentElement)el.parentElement.remove()},ctlSavePendingPdfExpiry:function(el,event){ctlSavePendingPdfExpiry(el.dataset.a1,el.dataset.a2,el.dataset.a3)},CM:function(el,event){CM(el.dataset.a1)},ctlAddBatchEditorRow:function(el,event){ctlAddBatchEditorRow()},ctlSaveBatchEditor:function(el,event){ctlSaveBatchEditor()},ctlConfirmDispense:function(el,event){ctlConfirmDispense()},ctlEditDeptMedicine:function(el,event){ctlEditDeptMedicine(el.dataset.id)},ctlRemoveDeptMedicine:function(el,event){ctlRemoveDeptMedicine(el.dataset.id)},openEditExpiry:function(el,event){openEditExpiry(el)},delBatch:function(el,event){delBatch(el.dataset.bid,el)},openAddExpiryForMed:function(el,event){openAddExpiryForMed(el.dataset.mid)}},{event:'click',attribute:'clickact'});}
