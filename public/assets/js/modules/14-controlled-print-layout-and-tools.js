(function(){
  ...
  window.ccSubmitReport=async function(){
    ...
    var canDeduct=typeof window.fsCanWriteStateKey!=='function'||window.fsCanWriteStateKey('crash_carts');
    /* Department users report usage only. Pharmacy-side roles are the sole
       writers that deduct stock; retain the submitted quantities for review. */
    if(!canDeduct)carts=JSON.parse(JSON.stringify(originalCarts||[]));
    var stamp=nowISO(),rep={id:old&&old.id||('ccr_'+Date.now().toString(36)),cartId:id,deptId:CU.deptId,reason:reason,oldSeal:val('ccr-old-seal'),consumed:consumed,status:'open',openedAt:old&&old.openedAt||stamp,openedBy:old&&old.openedBy||actualActorName(),inventoryDeductedAtReport:canDeduct,inventoryDeductedAt:canDeduct?stamp:null};
    if(existingIdx>=0)rs[existingIdx]=rep;else rs.push(rep);
    try{if(canDeduct)await setCrashCarts(carts);try{await setCrashReports(rs)}catch(reportError){if(canDeduct)await setCrashCarts(originalCarts);throw reportError}auditAction('crash_cart_open_report',{reportId:rep.id,cartId:id,consumed:consumed,inventoryDeductedAtReport:canDeduct});CM('mcc-report');renderCrashCarts();ccUpdateBadges();toast(canDeduct?(existingIdx>=0?'Report updated and quantities deducted ✓':'Report sent; reported quantities were deducted and are ready for replacement ✓'):(existingIdx>=0?'Report updated ✓':'Report sent to pharmacy ✓'),'succ')}catch(error){console.error(error);toast('Report was not saved; cart quantities were restored.','err')}
  };
