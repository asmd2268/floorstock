(function(){
  ...
  function isPharmacyCrashRole(){var rRole=String((window.CU&&CU.role)||'');return !!window.CU&&['pharmacy','inpatient_supervisor','pharmacy_staff'].includes(rRole)}
  ...
  function ccUpdateBadges(){
    var open=openReports().length, req=(typeof gr==='function'?(gr()||[]):[]).filter(function(r){return r.status==='pending'}).length;
  ...
  window.crashReportOpen=function(id){
    ...
    q('ccr-cart-id').value=id;q('ccr-old-seal').value=c.seal||'';q('ccr-reason').value='';
    q('ccr-items').innerHTML=(c.items||[]).map(function(it){var max=ccItemPresent(it);return '<div class="cc-med-choice" data-id="'+esc(it.id)+'"><div class="cc-med-head"><label><input type="checkbox" onchange="this.closest(\'.cc-med-choice\').classList.toggle(\'on\',this.checked)"><span>'+esc(it.name)+' <small style="color:var(--tx2)">'+esc(it.strength||it.concentration||'')+'</small></span></label><span class="chip">Available: '+max+' / Standard: '+ccItemStandard(it)+'</span></div><div class="cc-med-qty"><div class="cc-report-item-grid"><div class="fg"><label>Consumed quantity / الكمية المستخدمة</label><input class="ccr-qty" type="number" min="0.001" max="'+max+'" step="any" value="1"><div class="fhint">Maximum allowed: '+max+'</div></div><div class="fg"><label>Consumed expiry — optional / تاريخ الكمية المستخدمة — اختياري</label><select class="ccr-expiry">'+ccExpiryOptions(it,'',true)+'</select><div class="fhint">Leave blank if the department does not know the batch date.</div></div></div></div></div>'}).join('')||'<div class="fhint">No medicines configured.</div>';
    OM('mcc-report');
  };
  window.ccSubmitReport=async function(){
    var id=val('ccr-cart-id'),c=crashCart(id);if(!c)return;var consumed=[],errors=[];
    document.querySelectorAll('#ccr-items .cc-med-choice').forEach(function(row){var chk=row.querySelector('input[type=checkbox]');if(!chk.checked)return;var it=(c.items||[]).find(function(x){return String(x.id)===String(row.dataset.id)}),qty=n(row.querySelector('.ccr-qty').value),max=ccItemPresent(it),reportedExpiry=ccDateKey((row.querySelector('.ccr-expiry')||{}).value);if(!(qty>0)||qty>max){errors.push((it&&it.name||'Medicine')+': quantity exceeds available stock.');return}if(reportedExpiry&&qty>ccExpiryQuantity(it,reportedExpiry)){errors.push((it&&it.name||'Medicine')+': selected expiry has less than the reported quantity.');return}consumed.push({itemId:it.id,name:it.name,strength:it.strength||it.concentration||'',qty:qty,max:max,reportedExpiry:reportedExpiry||''})});
    ...
    var rs=crashReports().slice(),existingIdx=rs.findIndex(function(r){return String(r.cartId)===String(id)&&r.status==='open'}),old=existingIdx>=0?rs[existingIdx]:null,stamp=nowISO(),rep={id:old&&old.id||('ccr_'+Date.now().toString(36)),cartId:id,deptId:CU.deptId,reason:reason,oldSeal:val('ccr-old-seal'),consumed:consumed,status:'open',openedAt:old&&old.openedAt||stamp,openedBy:old&&old.openedBy||actualActorName(),lastReportedAt:stamp,lastReportedBy:actualActorName()};
    if(existingIdx>=0)rs[existingIdx]=rep;else rs.push(rep);await setCrashReports(rs);auditAction('crash_cart_open_report',{reportId:rep.id,cartId:id,consumed:consumed});CM('mcc-report');renderCrashCarts();ccUpdateBadges();toast(existingIdx>=0?'Crash Cart report updated ✓':'Crash Cart report sent successfully ✓','succ');
  };
  ...
  window.ccSavePharmacyResponse=async function(){
    ...
    var replacements=[];try{document.querySelectorAll('#ccc-items tr').forEach(function(row){var p=ccResponseRowPlan(row),it=(c.items||[]).find(function(x){return String(x.id)===String(p.itemId)});if(!it)throw new Error('A reported medicine no longer exists in the cart.');var current=ccItemPresent(it),standard=ccItemStandard(it),removed=0;if(p.action==='replace'){if(!p.sourceExpiry)throw new Error(it.name+': choose the old expiry.');removed=p.reportedQty;ccRemoveFromExpiry(it,p.sourceExpiry,removed)}if(p.qty>0){if(!p.expiry)throw new Error(it.name+': replacement expiry is required.');ccAddDatedQuantity(it,p.expiry,p.qty,id)}var result=current-removed+p.qty;if(result<0||result>standard+0.000001)throw new Error(it.name+': final quantity '+result+' exceeds the allowed range 0–'+standard+'.');it.present=result;it.stockStatus=result<=0?'out_of_stock':result<standard?'partial':'available';it.updatedAt=nowISO();it.updatedBy=actualActorName();replacements.push({itemId:p.itemId,name:it.name||'',action:p.action,sourceExpiry:p.sourceExpiry,reportedQty:p.reportedQty,removedQty:removed,qty:p.qty,expiry:p.expiry,resultingPresent:result,standardQty:standard})});
      ...
      await setCrashCarts(carts);try{await setCrashReports(rs)}catch(reportError){await setCrashCarts(originalCarts);throw reportError}auditAction('crash_cart_report_closed_exact_dated_replacement',{reportId:id,newSeal:seal,replacements:replacements});if(q('ccx-state'))q('ccx-state').value='';CM('mcc-close');renderCrashCarts();ccUpdateBadges();toast('Pharmacy response saved; old dated quantities were deducted exactly and standards verified ✓','succ');
