...
(function(){
  ...
  window.crashCloseReport=function(reportId){
    ...
    function ccNonReportedRow(it,labelNote,rowBg,sectionId){var saved=(r.replacements||[]).find(function(z){return String(z.itemId)===String(it.id)})||{},qty=saved.qty!=null?n(saved.qty):0,expiry=saved.expiry||'',choiceValue=expiry?(ccDatedBatches(it).some(function(b){return ccDateKey(b.expiry)===ccDateKey(expiry)})?ccDateKey(expiry):'__new__'):'';var savedUnavailable=qty===0&&!expiry;var secAttr=sectionId?' data-cc-section="'+sectionId+'"':'';var rowStyle=rowBg?'background:'+rowBg+';':'';return '<tr data-id="'+esc(it.id)+'" data-reported="0" data-reported-expiry=""'+secAttr+' style="'+rowStyle+'"><td><b>'+esc(it.name)+'</b><div class="fhint">'+esc(it.strength||it.concentration||'')+'</div><div class="fhint" style="margin-top:2px">'+labelNote+'</div><label class="ccc-unavailable-label"><input type="checkbox" class="ccc-unavailable" '+(savedUnavailable?'checked':'')+' onchange="ccCrashUnavailableToggled(this)"> No change / بدون تغيير</label><div class="ccc-row-result"></div></td><td>—<div class="fhint">Not reported / غير مُبلَّغ</div></td><td><div class="crash-current-batches">'+ccBatchSummary(it)+'</div><div class="fhint">Present '+ccItemPresent(it)+' / Standard '+ccItemStandard(it)+'</div></td><td><select class="ccc-exp-choice" '+(savedUnavailable?'disabled':'')+' onchange="ccCrashExpiryChoiceChanged(this)"><option value="">Choose expiry date...</option>'+ccMergedExpiryOptions(it,'',choiceValue==='__new__'?'':choiceValue)+'<option value="__new__" '+(choiceValue==='__new__'?'selected':'')+'>+ New date / تاريخ جديد</option></select><input class="ccc-exp" type="date" value="'+esc(choiceValue==='__new__'?expiry:'')+'" style="'+(choiceValue==='__new__'&&!savedUnavailable?'':'display:none')+'" '+(savedUnavailable?'disabled':'')+' onchange="ccCrashResponsePreview()"></td><td><input class="ccc-qty" type="number" min="0" step="any" value="'+(savedUnavailable?0:qty)+'" '+(savedUnavailable?'disabled':'')+' oninput="ccCrashResponsePreview()"></td></tr>';}
