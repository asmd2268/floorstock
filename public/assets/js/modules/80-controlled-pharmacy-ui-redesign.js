...
function syncHeroBtns(){
  ...
  if(view==='departments'&&role!=='warehouse'&&role!=='department')
    btns+='<button class="btn bp" data-clickact="ctlOpenDepartmentPrintOptions">🖨 Print custody / طباعة العهدة</button>';
  if(view==='overview'&&(role==='controlled_pharmacy'||typeof window.isMaster==='function'&&window.isMaster()))
    btns+='<button class="btn bg bsm" data-clickact="ctlAddCatalogMedicine">+ Add medicine</button>';
  if(view==='overview'&&(role==='controlled_pharmacy'||typeof window.isMaster==='function'&&window.isMaster())&&typeof window.ctlOpenHijriLedger==='function')
    btns+='<button class="btn bg bsm" data-clickact="ctlOpenHijriLedger">📜 Hijri Ledger / السجل الهجري</button>';
  if(view==='overview'&&(role==='controlled_pharmacy'||typeof window.isMaster==='function'&&window.isMaster())&&typeof window.ctlOpenCustodyHandover==='function')
    btns+='<button class="btn bg bsm" data-clickact="ctlOpenCustodyHandover">🧾 Handover / محضر تسليم</button>';
  if(view==='analytics')
    btns+='<button class="btn bg bsm" data-clickact="ctlAnPrint">🖨 Print analytics</button>';
  row.innerHTML=btns;
}
    ...
    '<div class="ctl-an-shell">'+
      ...
      '<div id="ctl-an-panel-records" class="ctl-an-panel active">'+
        '<div class="ctl-an-filters">'+
          ...
          '<button class="btn bp" data-clickact="ctlAnApply">Apply</button>'+
          '<button class="btn bg bsm" data-clickact="ctlAnPrint">🖨 Print</button>'+
        '</div>'+
      ...
      '<div id="ctl-an-panel-compare" class="ctl-an-panel">'+
        '<div class="ctl-period-sel">'+
          ...
          '<button class="btn bp" data-clickact="ctlCmpApply">Compare / قارن</button>'+
          '<button class="btn bg bsm" data-clickact="ctlCmpPrint">🖨 Print</button>'+
        '</div>'+
...
function ctlAnApply(){var c=document.getElementById('ctl-analytics-view');if(c)_applyFilters(c);};
function ctlCmpApply(){var c=document.getElementById('ctl-analytics-view');if(c)_applyComparison(c);};
function ctlAnPrint(){
...
(function(){
  ...
  window.buildNav=function(){
    if(!window.CU)return;
    ccInstallActions(document.body);
    var nav=q('mnav');
  ...
  function ccUpdateBadges(){
    ...
    if(deptRoles.indexOf(badgeRole)>=0){
      ...
      if(typeof window.fsCanAccessDepartment==='function')openFiltered=openFiltered.filter(function(r){var _c=typeof crashCart==='function'?crashCart(r.cartId):null;var _deptId=(_c&&_c.deptId)||r.deptId;return window.fsCanAccessDepartment(_deptId)});
    }
  ...
  var ccItemPresent=itemPresent,ccItemStandard=itemStandard,ccDateKey=dateKey,ccDatedBatches=datedBatches,ccExpiryQuantity=quantityAtExpiry;
  ...
  var ccRemoveFromExpiry=removeFromExpiry;
  function ccAddDatedQuantity(item,date,qty,reportId){return addDatedQuantity(item,date,qty,reportId,ccBatchDeps())}
  function ccDeductReported(item,qty,reportedExpiry){return deductReported(item,qty,reportedExpiry,ccBatchDeps())}
  function ccBatchDeps(){return {now:nowISO,actor:actualActorName,newId:function(){return 'ccb_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,7)}}}
  ...
  window.crashReportOpen=function(id){
    ...
    q('ccr-items').innerHTML=(c.items||[]).map(function(it){
      ...
      var max=ccItemPresent(displayItem),checked=!!prior,qty=prior?n(prior.qty):'',reportedExpiry=prior&&prior.reportedExpiry||'';
      return '<div class="cc-med-choice '+(checked?'on':'')+'" data-id="'+esc(it.id)+'"><div class="cc-med-head"><label><input type="checkbox" '+(checked?'checked ':'')+(max>0?'':'disabled ')+'data-changeact="toggleChoice"><span>'+esc(it.name)+' <small style="color:var(--tx2)">'+esc(it.strength||it.concentration||'')+'</small></span></label><span class="chip">Available: '+max+' / Standard: '+ccItemStandard(it)+'</span></div><div class="cc-med-qty"><div class="cc-report-item-grid"><div class="fg"><label>Consumed quantity / الكمية المستخدمة</label><input class="ccr-qty" type="number" min="0.001" max="'+max+'" step="any" value="'+qty+'" data-inputact="capToMax"><div class="fhint">Maximum / الحد الأقصى: '+max+'</div></div><div class="fg"><label>Consumed expiry / تاريخ الكمية المستخدمة'+(ccDatedBatches(displayItem).length?' <span style="color:var(--err,#ef4444)">*</span>':'')+'</label><select class="ccr-expiry">'+(ccDatedBatches(displayItem).length?'<option value="">Choose the batch date... / اختر تاريخ الدفعة...</option>':'')+ccExpiryOptions(displayItem,reportedExpiry,!ccDatedBatches(displayItem).length)+'</select><div class="fhint">'+(ccDatedBatches(displayItem).length?'Required — choose the batch the quantity was taken from. / إلزامي — اختر الدفعة التي أُخذت منها الكمية.':'This medicine has no dated batches in the cart. / لا توجد دفعات مؤرخة لهذا الدواء في العربة.')+'</div></div></div></div></div>';
  ...
  function ccResponseRowPlan(row){
    ...
    var reportedQty=n(row.dataset.reported),isNonReported=row.dataset.nonReported==='1';
    var qty=unavailable?0:n((row.querySelector('.ccc-qty')||{}).value);
    if(isNonReported){
      var srcEl=row.querySelector('.ccc-src-exp'),newExpEl=row.querySelector('.ccc-exp');
      var sourceExpiry=unavailable?'':ccDateKey((srcEl?srcEl.value:'')||'');
      var expiry=unavailable?'':ccDateKey((newExpEl?newExpEl.value:'')||'');
      var removeQty=sourceExpiry?qty:0;
      return {itemId:row.dataset.id,reportedQty:0,removeQty:removeQty,unavailable:unavailable,action:'replace',sourceExpiry:sourceExpiry,qty:qty,expiry:expiry};
    }
    ...
    /* Reports submitted before the expiry became mandatory carry no date, and the
       quantity still has to come off some batch. The pharmacist picks which one
       here instead of being told to contact an admin — the options are the
       cart's own batches, so nothing can be deducted from a batch that does not
       exist. */
    var srcPick=row.querySelector('.ccc-src-exp');
    var sourceExpiry=unavailable?'':(ccDateKey(row.dataset.reportedExpiry||'')||ccDateKey(srcPick?srcPick.value:''));
    return {itemId:row.dataset.id,reportedQty:reportedQty,removeQty:reportedQty,unavailable:unavailable,action:unavailable?'add':'replace',sourceExpiry:sourceExpiry,qty:qty,expiry:expiry}
  }
  ...
  function ccCrashExpiryChoiceChanged(select){var row=select.closest('tr'),input=row&&row.querySelector('.ccc-exp');if(input)input.style.display=select.value==='__new__'?'':'none';ccCrashResponsePreview()};
  ...
  function ccToggleCrashSection(section){var rows=document.querySelectorAll('[data-cc-section="'+section+'"]');var collapsed=rows.length&&rows[0].style.display==='none';rows.forEach(function(r){r.style.display=collapsed?'':'none'});};
  function ccCrashUnavailableToggled(checkbox){var row=checkbox.closest('tr');if(!row)return;var choice=row.querySelector('.ccc-exp-choice'),srcExp=row.querySelector('.ccc-src-exp'),newExp=row.querySelector('.ccc-exp'),qty=row.querySelector('.ccc-qty'),checked=checkbox.checked;if(choice)choice.disabled=checked;if(srcExp){srcExp.disabled=checked;if(checked)srcExp.value=''}if(newExp){newExp.disabled=checked;if(checked)newExp.value=''}if(qty){qty.disabled=checked;if(checked)qty.value=0}ccCrashResponsePreview()};
  function ccCrashResponsePreview(){var id=val('ccc-report-id'),r=(crashReports()||[]).find(function(x){return String(x.id)===String(id)}),c=r&&crashCart(r.cartId),alreadyDeducted=!!(r&&r.inventoryDeductedAtReport),messages=[],valid=true;document.querySelectorAll('#ccc-items tr[data-id]').forEach(function(row){var p=ccResponseRowPlan(row),it=c&&(c.items||[]).find(function(x){return String(x.id)===String(p.itemId)}),standard=ccItemStandard(it),check=crashResponseRowIssue(p,{present:ccItemPresent(it),standard:standard,alreadyDeducted:alreadyDeducted,datedBatchCount:ccDatedBatches(it).length,quantityAtExpiry:ccExpiryQuantity(it,p.sourceExpiry)}),result=check.result,error=check.error;row.classList.toggle('crash-response-error',!!error);var out=row.querySelector('.ccc-row-result');if(out)out.textContent=error||(p.unavailable
  ?'لن يُعوَّض · تبقى العربة '+result+' من '+standard+' / Not replaced · cart will hold '+result+' of '+standard
  :('ستصبح الكمية في العربة '+result+' من '+standard+' / The cart will hold '+result+' of '+standard+(alreadyDeducted?' · الكمية المبلّغ عنها مخصومة مسبقاً':'')));if(error){valid=false;messages.push((it&&it.name||'Medicine')+': '+error)}});var box=q('ccc-validation');if(box){box.className='crash-response-validation '+(valid?'ok':'err');box.innerHTML=valid?'✓ All rows are within the approved standard quantity. / جميع الكميات ضمن العدد المعتمد.':esc(messages.join(' | '))}var save=q('ccc-save-btn');if(save)save.disabled=!valid;return valid};
  window.crashCloseReport=function(reportId){
    ...
    q('ccc-report-id').value=reportId;q('ccc-new-seal').value=r.newSeal||'';q('ccc-note').value=r.pharmacyNote||'';var _oldSealEl=q('ccc-old-seal-display');if(_oldSealEl)_oldSealEl.textContent=r.oldSeal||c.seal||'—';
    var reportedIds=(r.consumed||[]).map(function(x){return String(x.itemId)});
    function ccSecHdr(icon,label,bg,sectionId){var clk=sectionId?' data-clickact="ccToggleCrashSection" data-a1="'+sectionId+'" style="cursor:pointer"':'';return '<tr'+clk+'><td colspan="5" style="background:'+bg+';color:#fff;font-weight:700;font-size:11px;letter-spacing:.04em;padding:5px 10px">'+icon+' '+label+(sectionId?' ▾':'')+'</td></tr>';}
    function ccNonReportedRow(it,labelNote,rowBg,sectionId){var saved=(r.replacements||[]).find(function(z){return String(z.itemId)===String(it.id)})||{},qty=saved.qty!=null?n(saved.qty):0,savedSrcExpiry=saved.sourceExpiry||'',savedNewExpiry=saved.expiry||'';var savedUnavailable=qty===0&&!savedNewExpiry;var secAttr=sectionId?' data-cc-section="'+sectionId+'"':'';return '<tr data-id="'+esc(it.id)+'" data-reported="0" data-reported-expiry="" data-non-reported="1"'+secAttr+' class="'+(rowBg||'ccc-row-other')+'"><td><div class="ccc-med"><b>'+esc(it.name)+'</b><span class="ccc-med-strength">'+esc(it.strength||it.concentration||'')+'</span></div><div class="ccc-med-note">'+labelNote+'</div><label class="ccc-unavailable-label"><input type="checkbox" class="ccc-unavailable" '+(savedUnavailable?'checked':'')+' data-changeact="ccCrashUnavailableToggled"><span><span class="ccc-unavailable-title">لا تغيير على هذا الصنف / Do not touch this medicine</span><span class="ccc-unavailable-note">اتركه كما هو في العربة — بدون إضافة كمية وبدون استبدال دفعة.<br/>Leave it exactly as it is in the cart — nothing added, no batch swapped.</span></span></label><div class="ccc-row-result"></div></td><td>—<div class="fhint">Not reported / غير مُبلَّغ</div></td><td><div class="crash-current-batches">'+ccBatchSummary(it)+'</div><div class="fhint">Present '+ccItemPresent(it)+' / Standard '+ccItemStandard(it)+'</div></td><td><div class="fhint" style="font-size:10px;margin-bottom:2px">Batch to replace / الدفعة المراد استبدالها</div><select class="ccc-src-exp" '+(savedUnavailable?'disabled':'')+' data-changeact="ccCrashResponsePreview"><option value="">— No swap / بدون استبدال</option>'+ccExpiryOptions(it,savedSrcExpiry,false)+'</select><div class="fhint" style="font-size:10px;margin-top:4px;margin-bottom:2px">New expiry / التاريخ الجديد</div><input class="ccc-exp" type="date" value="'+esc(savedUnavailable?'':savedNewExpiry)+'" '+(savedUnavailable?'disabled':'')+' data-changeact="ccCrashResponsePreview"></td><td><input class="ccc-qty" type="number" min="0" step="any" value="'+(savedUnavailable?0:qty)+'" '+(savedUnavailable?'disabled':'')+' data-inputact="ccCrashResponsePreview"></td></tr>';}
    var reportedRows=(r.consumed||[]).map(function(x){
      ...
      return '<tr data-id="'+esc(x.itemId)+'" data-reported="'+n(x.qty)+'" data-reported-expiry="'+esc(x.reportedExpiry||'')+'" class="ccc-row-reported"><td><div class="ccc-med"><b>'+esc(x.name)+'</b><span class="ccc-med-strength">'+esc(x.strength||'')+'</span></div><label class="ccc-unavailable-label"><input type="checkbox" class="ccc-unavailable" '+(savedUnavailable?'checked':'')+' data-changeact="ccCrashUnavailableToggled"><span><span class="ccc-unavailable-title">ما عندي هذا الدواء الآن / I have none of this medicine</span><span class="ccc-unavailable-note">ضع علامة إذا لم تستطع تعويضه: تصبح الكمية صفراً، ويُغلق البلاغ، وتبقى العربة ناقصة هذا الصنف.<br/>Tick if you cannot replace it: the quantity becomes 0, the report closes, and the cart stays short of this medicine.</span></span></label><div class="ccc-row-result"></div></td><td>'+n(x.qty)+(x.reportedExpiry?'<div class="fhint">Reported expiry: '+esc(x.reportedExpiry)+'</div>':(ccDatedBatches(it).length?'<div class="fhint" style="color:var(--or,#e67e22)">Expiry not reported — choose the batch to deduct from / لم يُبلَّغ عن التاريخ — اختر الدفعة المخصومة</div><select class="ccc-src-exp" '+(savedUnavailable?'disabled':'')+' data-changeact="ccCrashResponsePreview"><option value="">Batch to deduct from... / الدفعة المخصومة...</option>'+ccExpiryOptions(it,saved.sourceExpiry||'',false)+'</select>':'<div class="fhint">Expiry not reported · no dated batches / لا توجد دفعات مؤرخة</div>'))+'</td><td><div class="crash-current-batches">'+ccBatchSummary(it)+'</div><div class="fhint">Present '+ccItemPresent(it)+' / Standard '+ccItemStandard(it)+'</div></td><td><select class="ccc-exp-choice" '+(savedUnavailable?'disabled':'')+' data-changeact="ccCrashExpiryChoiceChanged"><option value="">Choose expiry date... / اختر تاريخ الانتهاء...</option>'+ccMergedExpiryOptions(it,x.reportedExpiry,choiceValue==='__new__'?'':choiceValue)+'<option value="__new__" '+(choiceValue==='__new__'?'selected':'')+'>+ New date / تاريخ جديد</option></select><input class="ccc-exp" type="date" value="'+esc(savedUnavailable?'':(choiceValue==='__new__'?expiry:''))+'" style="'+(choiceValue==='__new__'&&!savedUnavailable?'':'display:none')+'" '+(savedUnavailable?'disabled':'')+' data-changeact="ccCrashResponsePreview"></td><td><input class="ccc-qty" type="number" min="0" step="any" value="'+(savedUnavailable?0:qty)+'" '+(savedUnavailable?'disabled':'')+' data-inputact="ccCrashResponsePreview"></td></tr>';
    }).join('');
    var nonReported=(c.items||[]).filter(function(it){return reportedIds.indexOf(String(it.id))<0;});
    var belowStd=nonReported.filter(function(it){return ccItemPresent(it)<ccItemStandard(it);});
    var atStd=nonReported.filter(function(it){return ccItemPresent(it)>=ccItemStandard(it);});
    var belowRows=belowStd.map(function(it){return ccNonReportedRow(it,'⬇ Below standard / أقل من المعياري','ccc-row-below');}).join('');
    var otherRows=atStd.map(function(it){return ccNonReportedRow(it,'✓ At standard / عند المستوى','ccc-row-other','other');}).join('');
    var hdrReported=ccSecHdr('🔴','Reported medicines / الأدوية المبلغ عنها','rgba(180,40,40,.85)');
    var hdrBelow=belowRows?ccSecHdr('🟠','Below standard (not reported) / أقل من المعياري','rgba(200,100,20,.82)'):'';
    var hdrOther=otherRows?ccSecHdr('⚪','Other cart medicines / بقية أدوية العربة','rgba(80,95,115,.80)','other'):'';
    q('ccc-items').innerHTML=hdrReported+reportedRows+hdrBelow+belowRows+hdrOther+otherRows;
    document.querySelectorAll('#ccc-items [data-cc-section="other"]').forEach(function(r){r.style.display='none'});
    // NC banner
    var ncBanner=q('ccc-nc-banner');if(ncBanner){if(r.noConsumption){ncBanner.innerHTML='<div style="background:rgba(52,152,219,.13);border:1.5px solid var(--bl,#3498db);border-radius:10px;padding:12px 14px;margin-bottom:10px;display:flex;align-items:center;gap:10px"><span style="font-size:22px">🔵</span><div><div style="font-weight:700;font-size:13px">No Consumption · لا يوجد استهلاك</div><div class="fhint" style="margin-top:3px">The department opened this cart for a check or emergency with no medications consumed. All medicines appear as "Not part of this report". Verify the seal then close. / فتح القسم العربة للفحص أو طارئ دون استهلاك أي دواء. تحقق من رقم القفل ثم أغلق البلاغ.</div></div></div>';ncBanner.hidden=false;}else{ncBanner.hidden=true;ncBanner.innerHTML='';}}
    // Extra batches panel hidden — all items now shown in main #ccc-items table
    var xPanel=q('ccc-extra-batches-panel');if(xPanel)xPanel.hidden=true;
  ...
  window.ccApplyExtraBatches=async function(){if(!canManageCrashCart())return toast('No permission.','err');var id=val('ccc-report-id');var r=(crashReports()||[]).find(function(x){return String(x.id)===String(id)});if(!r)return toast('Report not found.','err');var c=crashCart(r.cartId);if(!c)return toast('Cart not found.','err');var carts=JSON.parse(JSON.stringify(crashCarts()||[])),cartClone=carts.find(function(x){return String(x.id)===String(c.id)});if(!cartClone)return toast('Cart not found.','err');var applied=0;document.querySelectorAll('#ccc-extra-batches-body tr[data-extra-id]').forEach(function(row){var it=(cartClone.items||[]).find(function(x){return String(x.id)===String(row.dataset.extraId)});if(!it)return;var choiceEl=row.querySelector('.ccc-xb-choice'),dateEl=row.querySelector('.ccc-xb-date'),qtyEl=row.querySelector('.ccc-xb-qty');var choiceVal=choiceEl?choiceEl.value:'',date=choiceVal==='__new__'?(dateEl?dateEl.value:''):choiceVal,qty=n(qtyEl?qtyEl.value:0);if(!date||!(qty>0))return;var prevPresent=ccItemPresent(it);ccAddDatedQuantity(it,date,qty,id);it.present=prevPresent+qty;it.stockStatus=it.present<=0?'out_of_stock':it.present<ccItemStandard(it)?'partial':'available';it.updatedAt=nowISO();it.updatedBy=actualActorName();var resCell=row.querySelector('.ccc-xb-result');if(resCell)resCell.textContent='✓ New present: '+it.present+' / '+ccItemStandard(it);applied++;});if(!applied)return toast('Nothing to apply — set expiry date and quantity (> 0) for at least one medicine.','err');try{await setCrashCarts(carts);auditAction('crash_cart_extra_batch_update',{reportId:id,cartId:c.id,applied:applied});toast(applied+' batch update(s) saved ✓','succ');setTimeout(function(){crashCloseReport(id)},600)}catch(e){toast(String(e&&e.message||e),'err')}};
...
/* Pharmacy inventory: expiry filtering is integrated into the existing list card. */
  ...
  function phExpirySetView(v){window.phExpiryView=v||'inventory';renderPhExpiryIntegrated()};
  function phExpiryToggleDept(id,checked){
  ...
  function phExpirySelectAllDepartments(){window.phExpiryDeptIds=[];renderPhExpiryIntegrated()};
  function phExpiryClearDepartments(){window.phExpiryDeptIds=['__none__'];renderPhExpiryIntegrated()};
  function phExpiryEditRules(){
  ...
  function renderPhExpiryIntegrated(){
    ...
    var deptChecks=depts.map(function(d){var checked=!noneSelected&&selectedIds.includes(String(d.id));return '<label class="expiry-dept-option"><input type="checkbox" '+(checked?'checked':'')+' data-changeact="phExpiryToggleDept" data-a1="'+esc(d.id)+'"><span>'+esc(d.name)+'</span></label>'}).join('');
    ui.bar.innerHTML='<div class="expiry-filter-row">'+
      '<select class="psel expiry-view-select" data-changeact="phExpirySetView"><option value="inventory" '+(view==='inventory'?'selected':'')+'>Medication list / قائمة الأدوية</option><option value="all" '+(view==='all'?'selected':'')+'>All expiry records ('+counts.all+')</option><option value="urgent" '+(view==='urgent'?'selected':'')+'>Urgent ('+counts.urgent+')</option><option value="near" '+(view==='near'?'selected':'')+'>Near expiry ('+counts.near+')</option><option value="expired" '+(view==='expired'?'selected':'')+'>Expired ('+counts.expired+')</option></select>'+
      '<details class="expiry-dept-picker" '+(view==='inventory'?'style="display:none"':'')+'><summary>Departments: '+selectedLabel+'</summary><div class="expiry-dept-menu"><div class="expiry-dept-actions"><button type="button" class="btn bg bxs" data-clickact="phExpirySelectAllDepartments">All</button><button type="button" class="btn bg bxs" data-clickact="phExpiryClearDepartments">Clear</button></div>'+deptChecks+'</div></details>'+
      '<button type="button" class="btn bg bsm expiry-rules-btn" '+(view==='inventory'?'style="display:none"':'')+' data-clickact="phExpiryEditRules">⚙ Rules: urgent ≤ '+rules.urgentDays+'d · near ≤ '+rules.nearDays+'d</button></div>';
