...
(function(){
  ...
  window.ccToggleCrashSection=function(section){var rows=document.querySelectorAll('[data-cc-section="'+section+'"]');var collapsed=rows.length&&rows[0].style.display==='none';rows.forEach(function(r){r.style.display=collapsed?'':'none'});};
  ...
  window.ccCrashResponsePreview=function(){var id=val('ccc-report-id'),r=(crashReports()||[]).find(function(x){return String(x.id)===String(id)}),c=r&&crashCart(r.cartId),alreadyDeducted=!!(r&&r.inventoryDeductedAtReport),messages=[],valid=true;document.querySelectorAll('#ccc-items tr[data-id]').forEach(function(row){var p=ccResponseRowPlan(row),it=c&&(c.items||[]).find(function(x){return String(x.id)===String(p.itemId)}),current=ccItemPresent(it),standard=ccItemStandard(it),remove=alreadyDeducted?0:p.removeQty,result=current-remove+p.qty,error='';if(!p.unavailable&&!alreadyDeducted&&!p.sourceExpiry&&p.reportedQty>0&&ccDatedBatches(it).length)error='Choose the batch this quantity is deducted from. / اختر الدفعة التي تُخصم منها الكمية.';else if(!p.unavailable&&!alreadyDeducted&&p.reportedQty>0&&ccExpiryQuantity(it,p.sourceExpiry)<p.reportedQty)error='The reported expiry no longer has enough quantity to deduct.';else if(!p.unavailable&&!alreadyDeducted&&p.reportedQty===0&&p.removeQty>0&&ccExpiryQuantity(it,p.sourceExpiry)<p.removeQty)error='Selected batch does not have enough quantity. / الدفعة المحددة لا تحتوي على كمية كافية.';else if(p.qty<0)error='Replacement quantity cannot be negative.';else if(!p.unavailable&&p.qty>0&&!p.expiry)error='Every replacement quantity requires an expiry date.';else if(result<0)error='Resulting quantity cannot be negative.';else if(result>standard+0.000001)error='Result '+result+' exceeds standard '+standard+'.';row.classList.toggle('crash-response-error',!!error);var out=row.querySelector('.ccc-row-result');if(out)out.textContent=error||(p.unavailable
  ...
  window.ccExtraBatchChoiceChanged=function(sel){var row=sel.closest('tr');if(!row)return;var dateInput=row.querySelector('.ccc-xb-date');if(dateInput){dateInput.style.display=sel.value==='__new__'?'':'none';if(sel.value!=='__new__')dateInput.value=''}};
