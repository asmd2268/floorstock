...
(function(){
  ...
  window.crashCloseReport=function(reportId){
    ...
    var belowRows=belowStd.map(function(it){return ccNonReportedRow(it,'⬇ Below standard / أقل من المعياري');}).join('');
    var otherRows=atStd.map(function(it){return ccNonReportedRow(it,'✓ At standard / عند المستوى');}).join('');
    var hdrReported=ccSecHdr('🔴','Reported medicines / الأدوية المبلغ عنها','rgba(52,120,200,.85)');
    var hdrBelow=belowRows?ccSecHdr('🟠','Below standard (not reported) / أقل من المعياري','rgba(200,100,20,.82)'):'';
    var hdrOther=otherRows?ccSecHdr('⚪','Other cart medicines / بقية أدوية العربة','rgba(80,95,115,.80)'):'';
