...
window.acc3AddItemRow=function(){var c=document.getElementById('acc3-items-container');if(!c)return;var div=document.createElement('div');div.className='acc3-item-row fl g8 ic';div.style.marginBottom='6px';div.innerHTML='<input class="acc3-item-name" list="acc3-med-suggestions" placeholder="Medicine name / اسم الدواء" style="flex:1"><input class="acc3-item-qty" type="number" min="0.001" step="any" placeholder="Qty / الكمية" style="width:110px"><button class="btn bd2c bxs" type="button" onclick="acc3RemoveItemRow(this)">✕</button>';c.appendChild(div);div.querySelector('.acc3-item-name').focus()};
window.acc3RemoveItemRow=function(btn){var row=btn&&btn.closest('.acc3-item-row');if(row)row.remove()};
window.acc3SaveRegimen=async function(){if(!canManage())return toast('Not authorized.','err');var edit=UI.edit?rows(REGIMENS).find(function(r){return String(r.id)===String(UI.edit)}):null,dept=edit?edit.deptId:((document.getElementById('acc3-regimen-dept')||{}).value||''),name=String((document.getElementById('acc3-name')||{}).value||'').trim(),source=String((document.getElementById('acc3-source')||{}).value||'').trim(),label=String((document.getElementById('acc3-label')||{}).value||'').trim(),items=[];if(!validDept(dept)||!name||!source||!label)return toast('Department, plan name, indication and version label are required.','err');document.querySelectorAll('#acc3-items-container .acc3-item-row').forEach(function(row){var medName=String((row.querySelector('.acc3-item-name')||{}).value||'').trim(),qty=Number((row.querySelector('.acc3-item-qty')||{}).value||0);if(medName&&qty>0)items.push({medName:medName,qty:qty})});if(!items.length)return toast('Add at least one medicine with a quantity.','err');var lineType=((document.getElementById('acc3-line')||{}).value||'first_line'),severity=String(((document.getElementById('acc3-severity')||{}).value)||'moderate').trim()||'moderate',all=rows(REGIMENS).map(function(r){return Object.assign({},r)});if(edit){var row=all.find(function(r){return String(r.id)===String(edit.id)});row.name=name;row.infectionSource=source;row.updatedAt=now();row.updatedBy=actor();var newVer=!!(document.getElementById('acc3-new-version')&&document.getElementById('acc3-new-version').checked);if(newVer){var version={id:id('acc3v'),label:label,lineType:lineType,severity:severity,items:items,createdAt:now(),createdBy:actor()};row.versions=(row.versions||[]).concat([version]);row.activeVersionId=version.id}else{var av=activeVersion(row);av.label=label;av.lineType=lineType;av.severity=severity;av.items=items;av.updatedAt=now();av.updatedBy=actor()}}else all.push({id:id('acc3r'),deptId:dept,name:name,infectionSource:source,versions:[{id:id('acc3v'),label:label,lineType:lineType,severity:severity,items:items,createdAt:now(),createdBy:actor()}],activeVersionId:null,paused:false,createdAt:now(),createdBy:actor()});if(!edit)all[all.length-1].activeVersionId=all[all.length-1].versions[0].id;await save(REGIMENS,all);UI.edit='';toast('Regimen saved ✓');window.renderMedicationAccountability()};
