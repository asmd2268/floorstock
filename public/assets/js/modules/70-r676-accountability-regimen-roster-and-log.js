...
/* The archive itself lives in core/accountability-retention.js. It used to be
   ...
   trimming, so the reports keep their figures. */
window.acc2PurgeHistoricalHistory=function(){return window.archiveAccountabilityHistory()};
...
function logRows(){
  ...
  state('accountability_receipts_v2').forEach(function(row){if(!row)return;rows.push({at:row.receivedAt||row.createdAt||row.receivedDate,kind:'completed',label:'Completed handover / استلام وتسليم مكتمل',dept:row.deptId,medicine:(row.medicineTotals||[]).map(function(item){return (item.medName||'Medicine')+' × '+(item.units||0)}).join('، ')||'—',units:row.totalUnits,actor:(row.pharmacyName||'—')+' → '+(row.nurseName||'—'),method:(row.confirmationMethod==='temporary_dual_qr'||row.confirmationMethod==='temporary_qr')?row.confirmationMethod:row.confirmationMethod==='manual'?'manual':'recorded',status:'received_locked'})});
...
/* Receipts issued while both parties scanned carry 'temporary_dual_qr'; since the
   pharmacist is stamped from their account only the department scans, and new
   receipts carry 'temporary_qr'. Both are QR handovers and filter as one. */
function methodLabel(value){
  if(value==='temporary_qr')return 'QR / رمز QR';
  if(value==='temporary_dual_qr')return 'Dual QR / رمزان QR';
  return value==='manual'?'Manual / يدوي':'Recorded / مسجل';
}
...
function acc3InstallActions(root){if(!root)return; /* one listener per event: core/delegated-actions.js */installActions(root,{acc3UpdateDoseRate:function(el,event){acc3UpdateDoseRate(el,el.dataset.a2)},acc3SetDept:function(el,event){acc3SetDept(el.value)}},{event:'change',attribute:'changeact'});installActions(root,{acc3RemoveItemRow:function(el,event){acc3RemoveItemRow(el)},acc3AddItemRow:function(el,event){acc3AddItemRow()},acc3SaveRegimen:function(el,event){acc3SaveRegimen()},acc3CancelEdit:function(el,event){acc3CancelEdit()},acc3EditRegimen:function(el,event){acc3EditRegimen(el.dataset.a1)},acc3PrintRegimen:function(el,event){acc3PrintRegimen(el.dataset.a1)},acc3ToggleRegimen:function(el,event){acc3ToggleRegimen(el.dataset.a1)},acc3RecordPlanReceipt:function(el,event){acc3RecordPlanReceipt(el.dataset.a1)},acc3CancelPlanUsage:function(el,event){acc3CancelPlanUsage(el.dataset.a1)},acc3SubmitPlanUsage:function(el,event){acc3SubmitPlanUsage(el.dataset.a1)},acc3SavePlanExpiry:function(el,event){acc3SavePlanExpiry(el.dataset.a1)},acc3ApprovePlanUsage:function(el,event){acc3ApprovePlanUsage(el.dataset.a1)},acc3RejectPlanUsage:function(el,event){acc3RejectPlanUsage(el.dataset.a1)}},{event:'click',attribute:'clickact'});}
function acc3DeptPlanView(deptId){acc3InstallActions(document.body);
    ...
    pendingReceiptHtml='<div class="card" style="border-left:4px solid var(--blue,#2196f3);margin-bottom:14px"><div class="ch"><span class="ct">🔔 Awaiting receipt / بانتظار استلام أدوية</span><div class="fhint">Pharmacy approved the following plan usages — confirm receipt after collecting the medicines.</div></div><div class="cb">'+
      pendingReceipts.map(function(u){
        ...
        return '<div style="border:1px solid var(--br);border-radius:6px;padding:10px;margin-bottom:8px">'+
          ...
          '<button class="btn bs" type="button" data-clickact="acc3RecordPlanReceipt" data-a1="'+esc(u.id)+'">✓ Confirm receipt / تأكيد الاستلام</button>'+
        '</div>';
      }).join('')+
    '</div></div>';
  }
  ...
  return waitHtml+'<div class="card" style="margin-top:16px"><div class="ch"><span class="ct">Plan usage review / مراجعة استهلاك الخطط</span><div class="fhint">Submitted by nursing staff — approve to deduct balances and send for receipt.</div></div><div class="cb">'+
    pending.map(function(u){
      ...
      return '<div style="border:1px solid var(--br);border-radius:6px;padding:10px;margin-bottom:10px">'+
        ...
        '<div style="margin:6px 0">'+medsHtml+'</div>'+
        '<div class="fl g6" data-acc3-pu-decision="'+esc(u.id)+'"><button class="btn bs bsm" type="button" data-clickact="acc3ApprovePlanUsage" data-a1="'+esc(u.id)+'">✓ Approve / اعتماد</button><button class="btn bd2c bsm" type="button" data-clickact="acc3RejectPlanUsage" data-a1="'+esc(u.id)+'">✕ Reject / رفض</button></div>'+
      '</div>';
    }).join('')+
  '</div></div>';
}
...
async function acc3ApprovePlanUsage(usageId){
  ...
  try{await save(PLAN_USAGE_KEY,all);toast('Plan usage approved ✓');window.renderMedicationAccountability();if(typeof window.ccUpdateBadges==='function')window.ccUpdateBadges()}catch(e){if(_grp){_grp.innerHTML='<button class="btn bs bsm" type="button" data-clickact="acc3ApprovePlanUsage" data-a1="'+esc(usageId)+'">✓ Approve / اعتماد</button><button class="btn bd2c bsm" type="button" data-clickact="acc3RejectPlanUsage" data-a1="'+esc(usageId)+'">✕ Reject / رفض</button>';}toast(String(e&&e.message||e).replace(/^FirebaseError:\s*/,''),'err')}
};
async function acc3RejectPlanUsage(usageId){
  if(!canManage())return toast('Not authorized.','err');
  var all=rows(PLAN_USAGE_KEY).map(function(u){return Object.assign({},u)});
  var rec=all.find(function(u){return String(u.id)===String(usageId)});
  if(!rec)return;
  ...
  }catch(e){if(_grp){_grp.innerHTML='<button class="btn bs bsm" type="button" data-clickact="acc3ApprovePlanUsage" data-a1="'+esc(usageId)+'">✓ Approve / اعتماد</button><button class="btn bd2c bsm" type="button" data-clickact="acc3RejectPlanUsage" data-a1="'+esc(usageId)+'">✕ Reject / رفض</button>';}toast(String(e&&e.message||e).replace(/^FirebaseError:\s*/,''),'err')}
};
async function acc3RecordPlanReceipt(usageId){
...
acc3InstallActions(document.body);
})();
...
/* One QR, for the receiving department only.

   There used to be two: the pharmacist scanned one and retyped their own name and
   employee number, then the nurse scanned the other. The pharmacy half was
   redundant — the pharmacist is the signed-in account that just pressed Create,
   so the system already knows who they are, and asking again added a step, a
   second code to hand out, and a way for a handover to stall with nobody having
   received anything. The pharmacist is now stamped from their account and only
   the nurse confirms. */
function showModal(data){
  closeModal();
  var departmentUrl=handoverUrl(data,'department',data.departmentToken);
  var pharmacist=(data.pharmacyConfirmation&&data.pharmacyConfirmation.name)||'';
  var modal=document.createElement('div');
  modal.id='r671-handover-modal';modal.className='modal-bg on';
  modal.innerHTML='<div class="modal" style="max-width:640px">'
    +'<div class="mh"><div><span class="mt">Handover QR for the department / رمز الاستلام للقسم</span>'
    +'<div class="fhint">'+esc(data.departmentName)+' · expires '+esc(new Date(data.expiresAt).toLocaleString())+'</div></div>'
    +'<button class="xbtn" type="button" data-r671-close>✕</button></div>'
    +'<div class="alert-banner-y">Delivered by '+esc(pharmacist||'the signed-in pharmacist')+', recorded from the account that created this handover. '
    +'The receiving nurse scans the code below and enters their name and employee number; the balance is replenished on that confirmation.'
    +'<br>سُجّل التسليم باسم '+esc(pharmacist||'الصيدلي الحالي')+' من حسابه مباشرة. يمسح ممرض القسم الرمز ويُدخل اسمه ورقمه الوظيفي، ويُعوَّض الرصيد عند تأكيده.</div>'
    +'<section>'+qrPanel(departmentUrl,'Department receipt QR')+'<textarea readonly>'+esc(departmentUrl)+'</textarea>'
    +'<a class="btn bg" href="'+esc(departmentUrl)+'" target="_blank" rel="noopener">Open department link / فتح رابط القسم</a></section>'
    +'<div class="fl g8" style="justify-content:flex-end;margin-top:14px"><button class="btn bp" type="button" data-r671-close>Done / تم</button></div>'
    +'</div>';
