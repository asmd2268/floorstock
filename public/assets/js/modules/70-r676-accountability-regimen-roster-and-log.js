...
function unitLabelSelect(cls,val){var opts=['tab','cap','mg','g','mcg','mL','vial','amp','IU','unit'];return '<select class="'+cls+'" style="width:65px" title="Unit / الوحدة">'+opts.map(function(o){return '<option value="'+o+'" '+(val===o?'selected':'')+'>'+o+'</option>'}).join('')+'</select>'}
function itemRows(existingItems){var items=existingItems&&existingItems.length?existingItems:[{medName:'',qty:'',unitLabel:'tab'}];var allNames=rows(CATALOG).filter(function(r){return r.active!==false}).map(function(r){return r.name});return '<div id="acc3-items-container" style="margin:8px 0">'+items.map(function(item){return '<div class="acc3-item-row fl g8 ic" style="margin-bottom:6px"><input class="acc3-item-name" list="acc3-med-suggestions" value="'+esc(item.medName||'')+'" placeholder="Medicine name / اسم الدواء" style="flex:1"><input class="acc3-item-qty" type="number" min="0.001" step="any" value="'+esc(item.qty||'')+'" placeholder="Qty" style="width:75px">'+unitLabelSelect('acc3-item-unitlabel',item.unitLabel||'tab')+'<button class="btn bd2c bxs" type="button" onclick="acc3RemoveItemRow(this)">✕</button></div>'}).join('')+'</div><button class="btn bg bsm" type="button" onclick="acc3AddItemRow()">+ Add medicine / إضافة دواء</button><datalist id="acc3-med-suggestions">'+allNames.map(function(n){return '<option value="'+esc(n)+'">'}).join('')+'</datalist>'}
