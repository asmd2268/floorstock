 ...
 * Original script position: 23
 * Original id: psychotropic-shared-list-v12
...
(function(){
  ...
  var _psySeedDone=false,_psySeedBusy=false;

  function psyNorm(v){return String(v==null?'':v).trim().toLowerCase();}
  function psySeedCodeUnique(field,value){
    value=psyNorm(value);if(!value)return false;
    return PSYCHOTROPIC_SHARED_SEED.filter(function(row){return psyNorm(row[field])===value}).length===1;
  }
  function psySame(a,b){
    if(psyNorm(a.name)&&psyNorm(a.name)===psyNorm(b.name))return true;
    if(psySeedCodeUnique('moh',b.moh)&&psyNorm(a.moh)===psyNorm(b.moh))return true;
    if(psySeedCodeUnique('nupco',b.nupco)&&psyNorm(a.nupco)===psyNorm(b.nupco))return true;
    return false;
  }
  async function ensurePsychotropicSharedList(){
    if(_psySeedDone||_psySeedBusy||typeof ctlCatalog!=='function'||typeof ctlSetCatalog!=='function')return false;
    _psySeedBusy=true;
    try{
      var cat=(ctlCatalog()||[]).slice(),changed=false;
      PSYCHOTROPIC_SHARED_SEED.forEach(function(src){
        var i=cat.findIndex(function(x){return psySame(x,src)});
        if(i<0){
          var baseId=(typeof ctlKey==='function'?ctlKey(src.moh,src.nupco,src.name):('psy_'+src.nupco+'_'+src.name.replace(/\W+/g,'_')))+'_psy';
          var uniqueId=baseId,slug=src.name.toLowerCase().replace(/[^a-z0-9]+/g,'_').replace(/^_|_$/g,'').slice(0,36),suffix=1;
          while(cat.some(function(x){return String(x.id)===String(uniqueId)})){uniqueId=baseId+'_'+(slug||'item')+'_'+suffix;suffix++}
          cat.push(Object.assign({},src,{id:uniqueId}));
          changed=true;
        }else{
          var old=cat[i],next=Object.assign({},old,{
            moh:src.moh||old.moh||'',
            nupco:String(src.nupco||old.nupco||''),
            name:src.name,
            classification:'psychotropic'
          });
          if(JSON.stringify(old)!==JSON.stringify(next)){cat[i]=next;changed=true;}
        }
      });
      if(changed)await ctlSetCatalog(cat);
      _psySeedDone=true;
      return changed;
    }catch(e){console.error('Psychotropic shared list seed failed',e);return false}
    finally{_psySeedBusy=false;}
  }

  // Shared catalogue permissions are defined once by ctlCanAddCatalog().
  window.ctlCanEditWarehouse=function(){
    return !!(window.CU&&(CU.master===true||CU.role==='warehouse'));
  };
