import { normalizeRole, resolvePermissionProfile, canWriteStateKey, canDeleteStateKey } from '../core/role-capabilities.js?v=367322a8f2';
(function(){
'use strict';
...
/* The CSP legacy-event bridge lived here: an allowlist of ~200 function names,
   a tiny expression grammar and a MutationObserver that stripped onclick /
   onchange / oninput / onkeydown off trusted controls and rebound them without
   eval. It existed so legacy markup could keep writing inline handlers under a
   CSP that blocks them.

   Every one of those handlers is now a delegated action (data-clickact and its
   siblings, core/delegated-actions.js), so the observer matched nothing and the
   allowlist named functions no caller looked up by name any more. CSP itself
   blocks an inline handler, which is what the stripping half was insuring
   against, so removing the bridge narrows the surface rather than widening it.

   tools/verify_action_handler_scopes.mjs keeps the replacement honest, and
   tests/ui-actions-regression.test.js fails if an inline handler comes back. */
