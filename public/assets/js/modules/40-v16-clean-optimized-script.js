import { consumeBatches, mergeBatches, reconcileBatchQuantities } from '../core/controlled-batch-consumption.js?v=80230e0fce';
...
function v16CleanInstallActions(root){if(!root)return; /* one listener per event: core/delegated-actions.js */installActions(root,{ctlImportMasterFile:function(el,event){ctlImportMasterFile(el.files[0])}},{event:'change',attribute:'changeact'});installActions(root,{ctlImportMasterText:function(el,event){ctlImportMasterText()},ctlEditWarehouseStock:function(el,event){ctlEditWarehouseStock(el.dataset.id)},ctlSendToPharmacy:function(el,event){ctlSendToPharmacy(el.dataset.id)},ctlEditPharmacyStock:function(el,event){ctlEditPharmacyStock(el.dataset.id)},ctlOpenDispense:function(el,event){ctlOpenDispense(el.dataset.id)},ctlEditCatalogMedicine:function(el,event){ctlEditCatalogMedicine(el.dataset.id)},ctlAddCatalogMedicine:function(el,event){ctlAddCatalogMedicine()}},{event:'click',attribute:'clickact'});}
function cleanImportCardC(){v16CleanInstallActions(document.body);if(!canCatalogC())return '';return '<details class="card ctl-clean-import"><summary>Import or paste shared catalogue / استيراد القائمة المشتركة</summary><div class="cb"><div class="g2"><div><label>Excel file (.xlsx, .xls, .csv)</label><input type="file" id="ctl-import-file" accept=".xlsx,.xls,.csv" data-changeact="ctlImportMasterFile"><div class="fhint">Imports medicine codes, balances and expiry dates.</div></div><div><label>Paste tab-separated text</label><textarea id="ctl-import-text" rows="4" placeholder="MOH Code    NUPCO Code    Medication ..."></textarea><button type="button" class="btn bp bsm" data-clickact="ctlImportMasterText">Import text</button></div></div></div></details>'}
function overviewActionsC(m){var a=[];if(canWarehouseC()){a.push('<button type="button" class="btn bg bxs" data-id="'+escC(m.id)+'" data-clickact="ctlEditWarehouseStock">Edit warehouse</button>');if(roleC()==='warehouse')a.push('<button type="button" class="btn bs bxs" data-id="'+escC(m.id)+'" data-clickact="ctlSendToPharmacy">Send to pharmacy</button>')}if(canPharmacyC())a.push('<button type="button" class="btn bp bxs" data-id="'+escC(m.id)+'" data-clickact="ctlEditPharmacyStock">Edit pharmacy</button>');if(canDispenseC())a.push('<button type="button" class="btn bs bxs" data-id="'+escC(m.id)+'" data-clickact="ctlOpenDispense">Dispense / صرف</button>');if(canCatalogC())a.push('<button type="button" class="btn bg bxs" data-id="'+escC(m.id)+'" data-clickact="ctlEditCatalogMedicine">Edit medicine</button>');return a.join(' ')||'<span class="chip">Read only</span>'}
...
function v16ToggleMultiApplicable(on){document.querySelectorAll('.v16-multi-dept:not(:disabled)').forEach(function(x){x.checked=!!on})};
...
async function v16ApplyMultiClean(){
...
function validateBatchesFinalF(actual,batches){return reconcileBatchQuantities(actual,batches)} /* One owner for the rule; the wording and the arithmetic live in core / القاعدة معرّفة مرة واحدة. */
