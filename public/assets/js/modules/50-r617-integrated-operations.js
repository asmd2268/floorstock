...
function acc2MySubmissionsTab(deptId){
  ...
  return filterBar+'<div class="card"><div class="ch"><span class="ct">📝 My Submissions / طلباتي</span><div class="fhint">Submissions disappear from this log after the receipt handover is completed. / يختفي الطلب بعد إتمام محضر الاستلام والتسليم.</div></div><div class="cb"><div class="tw"><table class="acc2-table" style="font-size:12px"><thead><tr><th>TYPE / النوع</th><th>MEDICINE(S) / الأدوية</th><th>DATE / التاريخ</th><th>PATIENT FILE / الملف</th><th>DOCTOR / الطبيب</th><th>STATUS / الحالة</th><th></th></tr></thead><tbody>'+
    all.map(function(u){
      ...
      else if(u.status==='rejected'){if(u._type==='plan'){cancelBtn='<div class="fl g4" style="flex-direction:column;gap:4px"><button class="btn bd2c bxs" type="button" onclick="acc3CancelPlanUsage(\''+esc(u.id)+'\')">🗑 Delete / حذف</button><button class="btn bs bxs" type="button" onclick="acc3CancelPlanUsage(\''+esc(u.id)+'\',\'plans\')">✏ Edit & Resubmit / تعديل وإعادة إرسال</button></div>'}else{cancelBtn='<div class="fl g4" style="flex-direction:column;gap:4px"><button class="btn bd2c bxs" type="button" onclick="acc2CancelUsage(\''+esc(u.id)+'\')">🗑 Delete / حذف</button><button class="btn bs bxs" type="button" onclick="acc2CancelUsage(\''+esc(u.id)+'\',\'custody\')">✏ Edit & Resubmit / تعديل وإعادة إرسال</button></div>'}}
