...
function acc2MySubmissionsTab(deptId){
  ...
  var all=custUsage.map(function(u){return {_type:'custody',id:u.id,status:u.status,meds:esc(u.medName||'—')+' × '+acc2Number(u.units),date:u.consumptionDate||'—',file:u.patientFile||'—',doctor:u.doctor||'—',note:u.pharmacyNote||u.rejectionReason||'',submittedAt:u.submittedAt||''}})
    .concat(planUsage.map(function(u){return {_type:'plan',id:u.id,status:u.status,meds:esc(u.planName||'—')+': '+(u.medicines||[]).map(function(m){return esc(m.medName)+' × '+m.actualQty}).join(', '),date:u.consumptionDate||'—',file:u.patientFile||'—',doctor:u.doctor||'—',note:u.pharmacyNote||u.rejectionReason||'',submittedAt:u.submittedAt||''}}))
  ...
  var medNames=[];all.forEach(function(u){var raw=custUsage.find(function(x){return String(x.id)===String(u.id)});var nm=raw?raw.medName:null;if(nm&&medNames.indexOf(nm)<0)medNames.push(nm);});medNames.sort();
  var subMed=ACC2_UI.subMed||'';
  if(subMed)all=all.filter(function(u){var raw=custUsage.find(function(x){return String(x.id)===String(u.id)});return raw&&raw.medName===subMed;});
  var medSelect=medNames.length>1?'<select onchange="acc2SubSetMed(this.value)" style="margin:0;padding:5px 8px;font-size:12px;border:1px solid var(--bd);border-radius:7px;background:var(--s2);color:var(--tx)"><option value="">All medicines / كل الأدوية</option>'+medNames.map(function(n){return '<option value="'+esc(n)+'"'+(n===subMed?' selected':'')+'>'+esc(n)+'</option>'}).join('')+'</select>':'';
...
window.acc2CreateReceipt=async function(){
  ...
  try{
    ...
    Object.keys(totals).forEach(function(assignmentId){var a=acc2Assignment(assignmentId,assignments);if(!a)throw new Error('Custody assignment was not found.');a.balance=Math.min(acc2Number(a.quota),acc2Number(a.balance)+totals[assignmentId]);a.updatedAt=now();a.updatedBy=actor().name});
