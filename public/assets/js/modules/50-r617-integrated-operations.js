...
function acc2RegimenForAssignment(deptId,assignmentId){return regimensForAssignment(acc2Regimens(),deptId,assignmentId)}
...
function acc2GraceMs(){var mins=30;try{var cfg=S.g('acc2_settings');if(cfg&&Number(cfg.approvalGraceMinutes)>0)mins=Number(cfg.approvalGraceMinutes);}catch(e){}return mins*60000;}
...
function acc2PendingUsageTable(deptId){var usage=acc2Usage().filter(function(u){return String(u.deptId)===String(deptId)&&u.status==='pending_pharmacy'});if(!usage.length)return '';return '<div class="card" style="border-left:4px solid var(--yellow,#f39c12);margin-bottom:14px"><div class="ch"><span class="ct">⏳ Pending pharmacy review / بانتظار مراجعة الصيدلية</span></div><div class="cb"><div class="tw"><table class="acc2-table" style="font-size:12px"><thead><tr><th>Medicine / الدواء</th><th>Units / الوحدات</th><th>Date / التاريخ</th><th>Patient file / الملف</th><th>Doctor / الطبيب</th><th>Reason / السبب</th><th></th></tr></thead><tbody>'+usage.map(function(u){return '<tr><td><b>'+esc(u.medName||'—')+'</b></td><td>'+esc(u.units)+'</td><td>'+acc2FmtDate(u.consumptionDate)+'</td><td>'+esc(u.patientFile||'—')+'</td><td>'+esc(u.doctor||'—')+'</td><td>'+esc(u.reasonLabel||'—')+(u.note?'<div class="fhint">'+esc(u.note)+'</div>':'')+'</td><td><button class="btn bd2c bxs" type="button" onclick="acc2CancelUsage(\''+esc(u.id)+'\')">✕ Cancel / إلغاء</button></td></tr>'}).join('')+'</tbody></table></div></div></div>'}
...
function piIntGetColors(){
  var saved={};try{saved=(window.S&&window.S.g&&window.S.g(PI_INT_COLORS_KEY))||{}}catch(e){}
  var result={};
