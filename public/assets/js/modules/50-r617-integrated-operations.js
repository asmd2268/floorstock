...
function canCrashAdmin(){return permission('crashCartAdmin',['pharmacy','inpatient_supervisor'])}
function canCrashOperate(){return permission('crashCartOperations',['pharmacy','inpatient_supervisor','pharmacy_staff'])}
...
function canAccManage(){return acc2EffectiveMaster()||['pharmacy','inpatient_supervisor','pharmacy_staff'].indexOf(role())>=0}
...
window.acc2SaveRegimenVersion=async function(){if(!canAccManage())return toast('Not authorized.','err');var list=acc2Regimens().map(function(x){return clone(x)}),edit=list.find(function(r){return String(r.id)===String(ACC2_UI.editRegimenId)})||null,deptId=edit?edit.deptId:((E('acc2-regimen-dept')||{}).value||''),name=edit?edit.name:String((E('acc2-regimen-name')||{}).value||'').trim(),label=String((E('acc2-regimen-version')||{}).value||'').trim(),items=[];document.querySelectorAll('[data-acc2-regimen-assignment]:checked').forEach(function(chk){var assignmentId=chk.getAttribute('data-acc2-regimen-assignment'),qty=acc2Number((document.querySelector('.acc2-regimen-qty[data-assignment="'+CSS.escape(assignmentId)+'"]')||{}).value);if(qty>0)items.push({assignmentId:assignmentId,medName:(acc2Assignment(assignmentId)||{}).medName||'',qty:qty})});if(!deptId||!name||!label||!items.length)return toast('Department, regimen name, version label and at least one medicine quantity are required.','err');var duplicate={};for(var i=0;i<items.length;i++){if(duplicate[items[i].assignmentId])return toast('A medicine can appear only once in a regimen version.','err');duplicate[items[i].assignmentId]=1}var version={id:uid('acc2v'),label:label,items:items,createdAt:now(),createdBy:actor().name};if(edit){edit.versions=Array.isArray(edit.versions)?edit.versions:[];edit.versions.push(version);edit.activeVersionId=version.id;edit.updatedAt=now();edit.updatedBy=actor().name}else{list.push({id:uid('acc2r'),name:name,deptId:deptId,versions:[version],activeVersionId:version.id,paused:false,active:true,createdAt:now(),createdBy:actor().name})}await S.s(ACC2_REGIMENS_KEY,list);ACC2_UI.editRegimenId='';toast('Regimen version saved and activated ✓','succ');renderMedicationAccountability()};
...
function acc2DepartmentReceipt(deptId){var rows=acc2Usage().filter(function(u){return String(u.deptId)===String(deptId)&&u.status==='approved_waiting_receipt'}),totals={};rows.forEach(function(u){if(!totals[u.assignmentId])totals[u.assignmentId]={medName:u.medName,count:0,units:0};totals[u.assignmentId].count++;totals[u.assignmentId].units+=acc2Number(u.units)});return '<div class="card"><div class="ch"><div><span class="ct">Nursing receipt and handover / استلام وتسليم التمريض</span><div class="fhint">Only pharmacy-approved requests are listed. Recording the nurse and receipt date replenishes the custody balance and locks all selected rows.</div></div></div><div class="cb">'+(rows.length?'<div class="acc2-receipt-summary">'+Object.keys(totals).map(function(k){var t=totals[k];return '<span><b>'+esc(t.medName)+'</b>: '+t.units+' units in '+t.count+' request(s)</span>'}).join('')+'</div><div class="fl g8" style="margin-bottom:10px"><button class="btn bg bsm" onclick="acc2ToggleAllReceipt(true)">Select all</button><button class="btn bg bsm" onclick="acc2ToggleAllReceipt(false)">Clear</button></div><div class="tw"><table class="acc2-table"><thead><tr><th></th><th>Date</th><th>Medicine</th><th>Units</th><th>Patient file</th><th>Approved by</th></tr></thead><tbody>'+rows.map(function(u){return '<tr><td><input class="acc2-receipt-check" type="checkbox" value="'+esc(u.id)+'"></td><td>'+esc(u.consumptionDate)+'</td><td>'+esc(u.medName)+'</td><td>'+acc2Number(u.units)+'</td><td>'+esc(u.patientFile)+'</td><td>'+esc(u.approvedBy||'—')+'</td></tr>'}).join('')+'</tbody></table></div><div class="acc2-form-grid" style="margin-top:12px"><div class="fg"><label>Nurse name / اسم الممرض *</label><input id="acc2-receipt-nurse"></div><div class="fg"><label>Receipt date / تاريخ الاستلام *</label><input id="acc2-receipt-date" type="date" max="'+acc2Today()+'" value="'+acc2Today()+'"></div></div><button class="btn bs" onclick="acc2CreateReceipt()">Confirm receipt and replenish balance / تأكيد الاستلام وتعويض الرصيد</button>':'<div class="acc2-empty">No pharmacy-approved requests are waiting for nursing receipt.</div>')+'</div></div>'}
...
window.controlledStoragePrint=async function(id,mode){
  ...
  try{
    ...
    var qr=
      ...
      '&qzone=4&ecc=H&data='+encodeURIComponent(publicUrl);
    ...
    var printHtml=
      ...
      '</div>'+
      '<div class="qrbox">'+
      '<img id="controlled-expiry-qr" src="'+qr+'" alt="Live expiry QR">'+
