...
window.acc2SaveAssignment=async function(){if(!canAccManage())return toast('Not authorized.','err');var id=(E('acc2-assignment-id')||{}).value||'',deptId=(E('acc2-assignment-dept')||{}).value||'',medName=String((E('acc2-assignment-med')||{}).value||'').trim(),quota=acc2Number((E('acc2-assignment-quota')||{}).value),reasons=acc2UniqueLines((E('acc2-assignment-reasons')||{}).value),active=!!(E('acc2-assignment-active')&&E('acc2-assignment-active').checked),expiryDate=String((E('acc2-assignment-expiry')||{}).value||'').trim(),itemDetails=acc2UniqueLines((E('acc2-assignment-items')||{}).value);if(!deptId||!medName||!(quota>0)||!reasons.length)return toast('Department, medicine, positive custody quantity and at least one reason are required.','err');if(!acc2DeptAllowed(deptId))return toast('Access to this department is restricted. / هذا القسم محظور لحسابك.','err');var list=acc2Assignments().map(function(x){return Object.assign({},x)}),duplicate=list.find(function(x){return String(x.deptId)===String(deptId)&&norm(x.medName)===norm(medName)&&String(x.id)!==String(id)});if(duplicate)return toast('This medicine already has a custody record for the selected department.','err');try{if(acc2UsesCF()){await acc2CallMutation({action:'saveAssignment',id:id,deptId:deptId,medName:medName,quota:quota,reasons:reasons,active:active,expiryDate:expiryDate,itemDetails:itemDetails})}else{var meta=acc2ActorMeta(),row=list.find(function(x){return String(x.id)===String(id)});if(row){var deficit=Math.max(0,acc2Number(row.quota)-acc2Number(row.balance));row.deptId=deptId;row.medName=medName;row.quota=quota;row.balance=Math.max(0,Math.min(quota,quota-deficit));row.reasons=reasons;row.active=active;row.expiryDate=expiryDate;row.itemDetails=itemDetails;row.updatedAt=meta.at;row.updatedBy=meta.name}else{list.push({id:uid('acc2a'),deptId:deptId,medName:medName,quota:quota,balance:quota,reasons:reasons,active:active,expiryDate:expiryDate,itemDetails:itemDetails,createdAt:meta.at,createdBy:meta.name,updatedAt:meta.at,updatedBy:meta.name})}await S.s(ACC2_ASSIGNMENTS_KEY,list)}ACC2_UI.editAssignmentId='';toast('Department custody saved ✓','succ');renderMedicationAccountability()}catch(error){toast(String(error&&error.message||error).replace(/^FirebaseError:\s*/,''),'err')}};
