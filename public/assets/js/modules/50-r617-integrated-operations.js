...
async function acc2ResetBalance(id){if(!acc2EffectiveMaster())return toast('Master only.','err');var list=acc2Assignments().map(function(x){return Object.assign({},x)}),row=acc2Assignment(id,list);if(!row)return;var ok=typeof uiConfirm==='function'?await uiConfirm('Reset balance of '+row.medName+' to quota ('+acc2Number(row.quota)+')?'):window.confirm('Reset balance of '+row.medName+' to quota ('+acc2Number(row.quota)+')?');if(!ok)return;try{if(acc2UsesCF()){await acc2CallMutation({action:'resetBalance',id:id})}else{row.balance=row.quota;row.updatedAt=now();row.updatedBy=actor().name;await S.s(ACC2_ASSIGNMENTS_KEY,list)}toast('Balance reset to '+acc2Number(row.quota)+' ✓','succ');renderMedicationAccountability()}catch(e){toast(String(e&&e.message||e).replace(/^FirebaseError:\s*/,''),'err')}};
