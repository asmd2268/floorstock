...
window.acc2CancelUsage=async function(id){if(!acc2DepartmentRole())return toast('Department access required.','err');var usageList=acc2Usage(),u=usageList.find(function(x){return String(x.id)===String(id)});if(!u||u.status!=='pending_pharmacy')return toast('This submission can no longer be cancelled. / لا يمكن إلغاء هذا الطلب بعد الآن.','err');if(String(u.deptId)!==String((window.CU&&CU.deptId)||''))return toast('Access denied.','err');var ok=typeof window.uiConfirm==='function'?await window.uiConfirm('Cancel this submission and restore '+acc2Number(u.units)+' unit(s) to balance? / إلغاء الطلب وإرجاع '+acc2Number(u.units)+' وحدة للرصيد؟'):window.confirm('Cancel this submission? / إلغاء هذا الطلب؟');if(!ok)return;var newUsage=usageList.filter(function(x){return String(x.id)!==String(id)});var list=acc2Assignments().map(function(x){return Object.assign({},x)}),a=acc2Assignment(u.assignmentId,list);if(a){a.balance=acc2Number(a.balance)+acc2Number(u.units);a.updatedAt=now();a.updatedBy=actor().name}try{await S.s(ACC2_ASSIGNMENTS_KEY,list);try{await S.s(ACC2_USAGE_KEY,newUsage)}catch(error){await S.s(ACC2_ASSIGNMENTS_KEY,acc2Assignments());throw error}toast('Submission cancelled; '+acc2Number(u.units)+' unit(s) restored ✓','succ');renderMedicationAccountability()}catch(error){toast(String(error&&error.message||error),'err')}};
function acc2MySubmissionsTab(deptId){
  ...
  return '<div class="card"><div class="ch"><span class="ct">📝 My Submissions / طلباتي</span><div class="fhint">Submissions disappear from this log after the receipt handover is completed. / يختفي الطلب بعد إتمام محضر الاستلام والتسليم.</div></div><div class="cb"><div class="tw"><table class="acc2-table" style="font-size:12px"><thead><tr><th>Type / النوع</th><th>Medicine(s) / الأدوية</th><th>Date / التاريخ</th><th>Patient file / الملف</th><th>Doctor / الطبيب</th><th>Status / الحالة</th><th></th></tr></thead><tbody>'+
    all.map(function(u){
      ...
      var noteHtml=u.note?'<div class="fhint" style="color:var(--red,#e74c3c)">'+esc(u.note)+'</div>':'';
