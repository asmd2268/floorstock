...
function acc2AdminStats(){var assignments=acc2VisibleAssignments(),usage=acc2VisibleUsage(),waiting=usage.filter(function(u){return u.status==='approved_waiting_receipt'});return acc2ExpiryAlertBanner(deptId)+'<div class="acc2-stats"><div class="sc"><div class="sl">Active custody lines / العهد</div><div class="sv">'+assignments.filter(function(a){return a.active!==false}).length+'</div></div><div class="sc"><div class="sl">Pending pharmacy review</div><div class="sv">'+usage.filter(function(u){return u.status==='pending_pharmacy'}).length+'</div></div><div class="sc"><div class="sl">Approved waiting receipt</div><div class="sv">'+waiting.length+'</div><div class="ss">'+acc2Sum(waiting)+' units</div></div><div class="sc"><div class="sl">Rejected</div><div class="sv">'+usage.filter(function(u){return u.status==='rejected'}).length+'</div></div></div>'}
