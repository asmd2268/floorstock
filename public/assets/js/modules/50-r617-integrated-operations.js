...
function acc2MySubmissionsTab(deptId){
  ...
  return filterBar+'<div class="card"><div class="ch"><span class="ct">📝 My Submissions / طلباتي</span><div class="fhint">Submissions disappear from this log after the receipt handover is completed. / يختفي الطلب بعد إتمام محضر الاستلام والتسليم.</div></div><div class="cb"><div class="tw"><table class="acc2-table" style="font-size:12px"><thead><tr><th>TYPE / النوع</th><th>MEDICINE(S) / الأدوية</th><th>DATE / التاريخ</th><th>PATIENT FILE / الملف</th><th>DOCTOR / الطبيب</th><th>STATUS / الحالة</th><th></th></tr></thead><tbody>'+
    all.map(function(u){
      ...
      else if(u.status==='rejected'){if(u._type==='plan'){cancelBtn='<div class="fl g4" style="flex-direction:column;gap:4px"><button class="btn bd2c bxs" type="button" onclick="acc3CancelPlanUsage(\''+esc(u.id)+'\')">🗑 حذف</button><button class="btn bs bxs" type="button" onclick="acc3CancelPlanUsage(\''+esc(u.id)+'\',\'plans\')">✏ تعديل وإعادة إرسال</button></div>'}else{cancelBtn='<div class="fl g4" style="flex-direction:column;gap:4px"><button class="btn bd2c bxs" type="button" onclick="acc2CancelUsage(\''+esc(u.id)+'\')">🗑 حذف</button><button class="btn bs bxs" type="button" onclick="acc2CancelUsage(\''+esc(u.id)+'\',\'custody\')">✏ تعديل وإعادة إرسال</button></div>'}}
...
window.acc2SubmitUsage=async function(assignmentId){if(!acc2DepartmentRole())return toast('Department access required.','err');var a=acc2Assignment(assignmentId,acc2Assignments());if(!a||a.active===false||String(a.deptId)!==String(CU.deptId))return toast('Custody is not available for this department.','err');var blocked=acc2SubmissionBlocked(a.deptId,a.id);if(blocked)return toast('Entry is paused while '+blocked.name+' is being replaced.','err');function read(kind){var el=document.querySelector('[data-acc2="'+kind+'"][data-assignment="'+CSS.escape(String(assignmentId))+'"]');return el?String(el.value||'').trim():''}var inputMode=a.inputMode||'units',unitSize=acc2Number(a.unitSize)||0,doseAmount=null,units=0;if(inputMode==='dose'&&unitSize>0){doseAmount=acc2Number(read('dose'));if(!(doseAmount>0))return toast('Dose amount is required. / الجرعة مطلوبة.','err');units=Math.ceil(doseAmount/unitSize)}else{units=acc2Number(read('units'));if(!(units>0))return toast('Units used is required. / عدد الوحدات مطلوب.','err')}var date=read('date'),file=read('file'),doctor=read('doctor'),reasonLabel=read('reason'),note=read('note');if(!date||!file||!doctor||!reasonLabel)return toast('Consumption date, patient file, doctor and reason are required.','err');if(date>acc2Today())return toast('Consumption date cannot be in the future.','err');var effBal=acc2EffectiveBalance(a);if(units>effBal)return toast('Used units exceed available balance. Available: '+effBal+' / الوحدات تتجاوز الرصيد المتاح.','err');var weightKg=acc2Number(read('weight'))||null,pregnancyStatus=read('pregnancy')||null,patientAge=acc2Number(read('age'))||null;if(a.requireWeight==='required'&&!weightKg)return toast('Patient weight is required for this medicine. / وزن المريض مطلوب.','err');if((a.minAge||a.maxAge)&&!patientAge)return toast('Patient age is required for this medicine due to age restriction. / عمر المريض مطلوب لهذا الدواء.','err');if(a.minAge&&patientAge&&patientAge<a.minAge)return toast('⚠ Patient age ('+patientAge+' yrs) is below minimum ('+a.minAge+' yrs) for this medicine. / عمر المريض أقل من الحد الأدنى المطلوب.','err');if(a.maxAge&&patientAge&&patientAge>a.maxAge)return toast('⚠ Patient age ('+patientAge+' yrs) exceeds maximum ('+a.maxAge+' yrs) for this medicine. / عمر المريض يتجاوز الحد الأقصى المسموح.','err');if(a.genderRestriction&&a.genderRestriction!=='any'){var gender=read('gender');if(!gender)return toast('Patient gender confirmation is required for this medicine. / تأكيد جنس المريض مطلوب.','err')}/* Submitted through the callable rather than a direct state write: it appends
  ...
  try{var submitted=await acc2CallMutation({action:'submitUsage',assignmentId:a.id,units:units,doseAmount:doseAmount,doseUnit:doseAmount?a.unitSizeLabel:null,weightKg:weightKg,pregnancyStatus:pregnancyStatus,patientAge:patientAge,consumptionDate:date,patientFile:file,doctor:doctor,reasonLabel:reasonLabel,note:note});if(submitted&&submitted.row)acc2ReflectUsageLocally(function(rows){return rows.concat([submitted.row])});toast('Submitted to pharmacy ✓ / تم الرفع للصيدلية ✓','succ');acc2RefreshCardAndStats(assignmentId)}catch(error){console.error(error);toast(String(error&&error.message||error).replace(/^FirebaseError:\s*/,''),'err')}};
