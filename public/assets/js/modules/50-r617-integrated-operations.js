...
/* The bulk-open arithmetic is in core/crash-cart-bulk-engine.js, where it is tested. */
function itemKey(it){return engineItemKey(it,norm)}
function findCartItem(cart,key){return engineFindCartItem(cart,key,norm)}
function uniqueCrashItems(){return engineUniqueCrashItems(cartList(),norm)} // offered once per medicine
function sealIsValid(seal){return typeof window.crashCartSealMeetsMinLength!=='function'||window.crashCartSealMeetsMinLength(seal)}
function buildCrashBulkResult(carts,reports,plan){return engineBuildCrashBulkResult(carts,reports,plan,{now:now,actor:actor,newId:uid,normalize:norm,sealIsValid:sealIsValid})}
var removeFromItem=engineRemoveFromItem;window.R17Core=Object.assign(window.R17Core||{},{buildCrashBulkResult:buildCrashBulkResult,removeFromItem:removeFromItem});

function r17InstallActions(root){if(!root)return; /* one listener per event: core/delegated-actions.js */
  installActions(root,{removeClosest:function(el){var row=el.closest(el.dataset.a1);if(row)row.remove()},removeReplacement:function(el){var row=el.closest('.r17-replacement');if(row)row.remove();r17CrashRenderMatrix()},acc2SaveGrace:function(){acc2SaveGrace((E('acc2-grace-input')||{}).value)},ccTabCarts:function(el,event){ccTabCarts()},ccOpsTab:function(el,event){ccOpsTab(el.dataset.a1)},r17CrashSaveDetails:function(el,event){r17CrashSaveDetails(el.dataset.a1)},r18OpenCrashCorrection:function(el,event){r18OpenCrashCorrection(el.dataset.a1)},r18CrashCorrectionAddBatch:function(el,event){r18CrashCorrectionAddBatch(el)},acc2SetAdminTab:function(el,event){acc2SetAdminTab(el.dataset.a1)},acc2SetDeptTab:function(el,event){acc2SetDeptTab(el.dataset.a1)},acc2SaveAssignment:function(el,event){acc2SaveAssignment()},acc2CancelAssignmentEdit:function(el,event){acc2CancelAssignmentEdit()},acc2EditAssignment:function(el,event){acc2EditAssignment(el.dataset.a1)},acc2ToggleAssignment:function(el,event){acc2ToggleAssignment(el.dataset.a1)},acc2DeleteAssignment:function(el,event){acc2DeleteAssignment(el.dataset.a1)},acc2ResetBalance:function(el,event){acc2ResetBalance(el.dataset.a1)},acc2Decision:function(el,event){acc2Decision(el.dataset.a1,el.dataset.a2)},acc2UndoApproval:function(el,event){acc2UndoApproval(el.dataset.a1)},acc2SaveRegimenVersion:function(el,event){acc2SaveRegimenVersion()},acc2CancelRegimenEdit:function(el,event){acc2CancelRegimenEdit()},acc2ToggleRegimenPause:function(el,event){acc2ToggleRegimenPause(el.dataset.a1)},acc2StartRegimenVersion:function(el,event){acc2StartRegimenVersion(el.dataset.a1)},acc2DeleteRegimen:function(el,event){acc2DeleteRegimen(el.dataset.a1)},acc2RemoveExpiryRow:function(el,event){acc2RemoveExpiryRow(el)},acc3CancelPlanUsage:function(el,event){acc3CancelPlanUsage(el.dataset.a1,el.dataset.a2)},acc2CancelUsage:function(el,event){acc2CancelUsage(el.dataset.a1,el.dataset.a2)},acc2SubmitUsage:function(el,event){acc2SubmitUsage(el.dataset.a1)},acc2ShowExpirySection:function(el,event){acc2ShowExpirySection(el.dataset.a1)},acc2AddExpiryRow:function(el,event){acc2AddExpiryRow(el.dataset.a1)},acc2SaveExpiryBatches:function(el,event){acc2SaveExpiryBatches(el.dataset.a1)},acc2SetMedTab:function(el,event){acc2SetMedTab(el.dataset.a1)},controlledStorageEditUnit:function(el,event){controlledStorageEditUnit(el.dataset.a1)},controlledStoragePrint:function(el,event){controlledStoragePrint(el.dataset.a1,el.dataset.a2)},controlledStorageDeleteUnit:function(el,event){controlledStorageDeleteUnit(el.dataset.a1)},acc2SaveClassColors:function(el,event){acc2SaveClassColors()},acc2ResetClassColors:function(el,event){acc2ResetClassColors()},acc2SaveIntColors:function(el,event){acc2SaveIntColors()},acc2ResetIntColors:function(el,event){acc2ResetIntColors()}},{event:'click',attribute:'clickact'});
  installActions(root,{r17CrashRenderMatrix:function(el,event){r17CrashRenderMatrix()},acc2RefreshMedicationList:function(el,event){acc2RefreshMedicationList()},acc2SetCustFilter:function(el,event){acc2SetCustFilter(el.dataset.a1,el.value)},acc2SetFilter:function(el,event){acc2SetFilter(el.dataset.a1,el.value)},acc2SetReceiptDept:function(el,event){acc2SetReceiptDept(el.value)},acc2RegimenDeptChanged:function(el,event){acc2RegimenDeptChanged(el.value)},acc2ActivateRegimen:function(el,event){acc2ActivateRegimen(el.dataset.a1,el.value)},acc2SubSetMed:function(el,event){acc2SubSetMed(el.value)},controlledStorageAssignByName:function(el,event){controlledStorageAssignByName(el)}},{event:'change',attribute:'changeact'});
  installActions(root,{    filterMedicine:function(el){ACC2_UI.filters.medicine=el.value;clearTimeout(window.acc2FilterTimer);window.acc2FilterTimer=setTimeout(r17AccRenderVerify,140)},r17CrashRenderMatrix:function(el,event){r17CrashRenderMatrix()},acc2UpdateDoseCalc:function(el,event){acc2UpdateDoseCalc(el.dataset.a1,el.dataset.a2)},acc2MedSearch:function(el,event){acc2MedSearch(el.value)},controlledStorageCountsChanged:function(el,event){controlledStorageCountsChanged()},controlledStorageValidateSearch:function(el,event){controlledStorageValidateSearch(el)}},{event:'input',attribute:'inputact'});
}

window.renderCrashOperations=function(){
  r17InstallActions(document.body);
...
function injectCcOpsTabBar(){var pg=E('pg-crash-ops');if(!pg||pg.querySelector('.cc-tab-bar'))return;var bar=document.createElement('div');bar.className='cc-tab-bar';bar.style.cssText='display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap';bar.innerHTML='<button class="btn bg bsm" data-clickact="ccTabCarts">🚑 Crash Carts</button><button class="btn bp bsm" disabled><b>⚙ Operations</b></button>';pg.insertBefore(bar,pg.firstChild)}
function ccOpsTab(tab){window.__ccOpsTab=tab;renderCrashOperations()};
function renderCrashOperationsCore(){
  ...
  subBar.innerHTML=subTabs.map(function(t){var on=t[0]===tab;return '<button class="btn '+(on?'bp':'bg')+' bsm" '+(on?'disabled':'')+' data-clickact="ccOpsTab" data-a1="'+t[0]+'">'+(on?'<b>'+t[1]+'</b>':t[1])+'</button>'}).join('');
  ...
  if(tab==='details'){
    ...
    list.innerHTML=cartList().filter(function(cart){return !externalScope||String(cart.deptId)===externalScope}).map(function(cart){
      return '<div class="r17-admin-row" data-cart="'+esc(cart.id)+'">'+
        ...
        '<div class="fl g8" style="align-items:flex-end"><button class="btn bs bsm" data-clickact="r17CrashSaveDetails" data-a1="'+esc(cart.id)+'">Save details</button>'+
        (allowCorrection?'<button class="btn bg bsm r18-master-correction-btn" data-clickact="r18OpenCrashCorrection" data-a1="'+esc(cart.id)+'">Master correction</button>':'')+
        '</div></div>';
...
function r18CrashBatchRow(batch){
  ...
  return '<div class="r18-cc-batch" data-batch-id="'+esc(batch.batchId||batch.id||uid('ccb'))+'">'+
    ...
    '<button class="btn bd2c bsm" type="button" data-clickact="removeClosest" data-a1=".r18-cc-batch">Remove date</button>'+
  '</div>';
}
function r18CrashItemRow(item){
  ...
  return '<div class="r18-cc-item" data-item-id="'+esc(item.id||uid('cci'))+'" data-med-id="'+esc(item.medId||'')+'">'+
    '<div class="r18-cc-item-head">'+
      ...
      '<button class="btn bd2c bsm" type="button" data-clickact="removeClosest" data-a1=".r18-cc-item">Delete medicine</button>'+
    '</div>'+
    '<div class="r18-cc-batches">'+batches+'</div>'+
    '<button class="btn bg bsm" type="button" data-clickact="r18CrashCorrectionAddBatch">+ Expiry / Batch</button>'+
  '</div>';
}
function r18OpenCrashCorrection(cartId){
...
function r18CrashCorrectionAddBatch(button){
...
var ACC2_UI={adminTab:'custody',deptTab:'custody',filters:{dept:'',status:'',medicine:''},receiptDept:'',regimenDept:'',editAssignmentId:'',editRegimenId:'',subDays:30};
function acc2SubSetMed(v){ACC2_UI.subMed=v;renderMedicationAccountability()};
function acc2MedSearch(val){ACC2_UI.medSearch=val;clearTimeout(window._acc2ST);window._acc2ST=setTimeout(function(){var pos=val.length;renderMedicationAccountability();var inp=document.getElementById('acc2-med-search');if(inp){inp.focus();try{inp.setSelectionRange(pos,pos)}catch(e){/* tearing down something already gone is not a failure */}}},120)};
...
/* The callable commits the write server-side, so unlike the old S.s() path it
   ...
   showed the new row at once, and the accountability draft guard cleared its
   "unsaved form" flag by diffing the cached rows before and after a save - with
   the cache unchanged the flag stayed set and refreshCurrentPage() kept
   returning false, freezing the page behind the "background refresh is paused"
   banner. Mirroring the committed row locally restores both. If the mirrored
   value ever disagrees with the server, the listener snapshot overwrites it. */
...
/* The assignments twin of acc2ReflectUsageLocally, and the reason Activate /
   Deactivate looked dead: the callable commits server-side and deliberately
   leaves the cache alone, so the re-render redrew the row from the value it
   already had — the write happened, nothing on screen said so. Usage was
   mirrored for this exact reason; assignments were not. The listener still
   wins: its snapshot overwrites anything mirrored here.
   نعكس صف العهدة محلياً بعد تثبيت الخادم، والمستمع يبقى المرجع. */
function acc2ReflectAssignmentsLocally(mutate){
  try{if(!window.S||typeof S.g!=='function'||!S.cache)return;S.cache[ACC2_ASSIGNMENTS_KEY]=mutate((S.g(ACC2_ASSIGNMENTS_KEY)||[]).slice())}
  catch(e){console.warn('Local assignment mirror failed; the listener will correct it.',e)}
}
...
function acc2DeptAssignments(deptId,includeInactive){var nd=norm(deptId);return acc2Assignments().filter(function(a){return norm(a.deptId)===nd&&(includeInactive||a.active!==false)})}
function acc2SubmissionBlocked(deptId,assignmentId){return submissionBlockedBy(acc2Regimens(),deptId,assignmentId)}
function acc2MedicationNames(deptId){var map={};try{(getMeds(deptId)||[]).forEach(function(m){var name=String(m.name||'').trim();if(name)map[name]=1})}catch(e){console.warn('getMeds was skipped after an error; the rest of this screen still renders.', e);}acc2DeptAssignments(deptId,true).forEach(function(a){if(a.medName)map[a.medName]=1});acc2Array('accountability_regimen_catalog_v1').filter(function(row){return row&&row.active!==false&&String(row.deptId)===String(deptId)}).forEach(function(row){if(row.name)map[String(row.name).trim()]=1});return Object.keys(map).sort(function(a,b){return a.localeCompare(b)})}
...
function acc2UniqueLines(value){return uniqueLines(value,norm)}
function acc2FilterRows(rows){return filterUsageRows(rows,ACC2_UI.filters||{},function(id){return acc2Assignment(id)},norm)}
...
function acc2SetReceiptDept(value){ACC2_UI.receiptDept=value;renderMedicationAccountability()};
...
function acc2AdminTabs(){var pending=acc2VisibleUsage().filter(function(u){return u.status==='pending_pharmacy'}).length;var tabs=[['custody','Custody / العهدة'],['review','Review / المراجعة'+(pending?'<span class="acc2-tab-badge">'+pending+'</span>':'')],['handover','Receipt / الاستلام'],['regimens','Plans / الخطط']];return '<div class="acc2-tabs">'+tabs.map(function(t){return '<button class="acc2-tab '+(ACC2_UI.adminTab===t[0]?'on':'')+'" type="button" data-clickact="acc2SetAdminTab" data-a1="'+t[0]+'">'+t[1]+'</button>'}).join('')+'</div>'}
function acc2DeptTabs(){var tabs=[['custody','🏥 Custody / العهدة'],['plans','📋 Plans / الخطط العلاجية'],['submissions','📝 My Submissions / طلباتي']];return '<div class="acc2-tabs" data-acc2-dept-tabs="1">'+tabs.map(function(t){return '<button class="acc2-tab '+(ACC2_UI.deptTab===t[0]?'on':'')+'" type="button" data-clickact="acc2SetDeptTab" data-a1="'+t[0]+'">'+t[1]+'</button>'}).join('')+'</div>'}
function acc2SetDeptTab(tab){ACC2_UI.deptTab=tab;renderMedicationAccountability()};

function acc2CustodyTab(){
  var assignments=acc2VisibleAssignments(),edit=acc2Assignment(ACC2_UI.editAssignmentId,assignments),deptId=edit?edit.deptId:'',names=acc2MedicationNames(deptId),cf=ACC2_UI.custFilter||{};
  var tableRows=assignments.filter(function(a){return(!cf.dept||String(a.deptId)===String(cf.dept))&&(cf.status==='active'?a.active!==false:cf.status==='inactive'?a.active===false:true)});
  var hasPatientRestrictions=edit&&((edit.requireWeight&&edit.requireWeight!=="no")||(edit.genderRestriction&&edit.genderRestriction!=="any")||(edit.pregnancyAllowed&&edit.pregnancyAllowed!=="any")||(edit.trimesterRestriction&&edit.trimesterRestriction!=="any")||edit.minAge||edit.maxAge);
  var unitLabels=[['mg','mg'],['g','g'],['mcg','mcg'],['mL','mL'],['IU','IU'],['unit','unit']];
  var inputModeOptions='<option value="units" '+(!edit||edit.inputMode!=="dose"?"selected":"")+'>Units / وحدات — nurse records vials or tablets directly</option><option value="dose" '+(edit&&edit.inputMode==="dose"?"selected":"")+'>Dose → auto-calc / جرعة — nurse enters mg/mL, system calculates vials</option>';
  return (
    '<div class="card" style="border-top:3px solid var(--ac,#2e7dff)">'+
      '<div class="ch" style="flex-wrap:wrap;gap:8px">'+
        '<div><span class="ct">'+(edit?'Edit custody / تعديل العهدة':'New custody / عهدة جديدة')+'</span>'+
          '<div class="fhint">Independent from department inventory — medicine names here are not added to stock.<br>مستقلة عن مخزون القسم — لا تُضاف الأسماء هنا للمخزون.</div>'+
        '</div>'+
        (edit?'<div class="fl g6"><button class="btn bp bsm" type="button" data-clickact="acc2SaveAssignment">Save / حفظ</button><button class="btn bg bsm" type="button" data-clickact="acc2CancelAssignmentEdit">Cancel / إلغاء</button></div>':'')+
      '</div>'+
      '<div class="cb" style="display:flex;flex-direction:column;gap:0">'+
        '<input id="acc2-assignment-id" type="hidden" value="'+esc(edit&&edit.id||'')+'">'+

        '<div style="background:var(--bg2);border-radius:8px;padding:14px 16px;margin-bottom:14px">'+
          '<div style="font-size:11px;font-weight:700;letter-spacing:.05em;opacity:.5;text-transform:uppercase;margin-bottom:10px">Required / الحقول الإلزامية</div>'+
          '<div class="grid g3" style="gap:12px">'+
            '<div class="fg"><label>Department / القسم <span style="color:#e74c3c">*</span></label>'+
              '<select id="acc2-assignment-dept" data-changeact="acc2RefreshMedicationList"><option value="">اختر القسم...</option>'+
              depts().map(function(d){return '<option value="'+esc(d.id)+'" '+(String(deptId)===String(d.id)?'selected':'')+'>'+esc(d.name)+'</option>'}).join('')+
            '</select></div>'+
            '<div class="fg"><label>Medicine / العلاج <span style="color:#e74c3c">*</span></label>'+
              '<input id="acc2-assignment-med" list="acc2-medication-list" value="'+esc(edit&&edit.medName||'')+'" placeholder="Generic name / الاسم العلمي">'+
              '<datalist id="acc2-medication-list">'+names.map(function(x){return '<option value="'+esc(x)+'">'}).join('')+'</datalist>'+
            '</div>'+
            '<div class="fg"><label>Concentration / التركيز <span style="color:#e74c3c">*</span> <small style="font-weight:400;opacity:.6">(per vial/tab)</small></label>'+
              '<div class="fl g6 ic">'+
                '<input id="acc2-assignment-unit-size" type="number" min="0.001" step="any" placeholder="e.g. 500" style="flex:1;min-width:0" value="'+esc(edit&&edit.unitSize!=null?edit.unitSize:'')+'">'+
                '<select id="acc2-assignment-unit-label" style="width:70px">'+
                  unitLabels.map(function(u){return '<option value="'+u[0]+'" '+((!edit||!edit.unitSizeLabel?u[0]==='mg':edit.unitSizeLabel===u[0])?"selected":"")+'>'+u[1]+'</option>'}).join('')+
                '</select>'+
              '</div>'+
            '</div>'+
          '</div>'+
          '<div class="fl g8 ic" style="margin-top:12px;flex-wrap:wrap">'+
            '<div class="fg" style="flex:1;min-width:180px"><label>Custody quantity / كمية العهدة <span style="color:#e74c3c">*</span> <small style="font-weight:400;opacity:.6">(vials / tablets)</small></label>'+
              '<input id="acc2-assignment-quota" type="number" min="0.001" step="any" value="'+esc(edit&&edit.quota!=null?edit.quota:'')+'">'+
            '</div>'+
            '<div style="display:flex;align-items:flex-end;padding-bottom:6px;margin-left:8px">'+
              '<label style="display:flex;align-items:center;gap:8px;margin:0;cursor:pointer">'+
                '<input id="acc2-assignment-active" type="checkbox" '+(!edit||edit.active!==false?'checked':'')+' style="width:16px;height:16px">'+
                '<span style="font-weight:500">Active / مفعّلة</span>'+
              '</label>'+
            '</div>'+
          '</div>'+
        '</div>'+
        ...
        '<div style="margin-bottom:14px"><div class="fg">'+
          '<label>Allowed usage reasons / أسباب الاستهلاك <span style="color:#e74c3c">*</span> <small style="font-weight:400;opacity:.6">(one per line / سبب لكل سطر)</small></label>'+
          '<textarea id="acc2-assignment-reasons" rows="3" placeholder="Emergency dose&#10;Protocol dose&#10;Hemodialysis" style="resize:vertical">'+esc(edit&&(edit.reasons||[]).join('\n')||'')+'</textarea>'+
        '</div></div>'+

        '<details style="border:1px solid var(--br);border-radius:8px;margin-bottom:10px" '+(edit&&edit.inputMode==="dose"?"open":"")+'>'+
          '<summary style="padding:10px 14px;cursor:pointer;font-weight:600;list-style:none;display:flex;align-items:center;gap:6px">'+
            '<span>⚙️</span> Nursing input mode / طريقة إدخال التمريض <span class="badge bbl" style="font-weight:400;font-size:11px;margin-left:4px">optional</span>'+
          '</summary>'+
          '<div style="padding:12px 14px;border-top:1px solid var(--br)">'+
            '<div class="fg"><label>Input mode</label><select id="acc2-assignment-input-mode">'+inputModeOptions+'</select></div>'+
            '<div class="fhint" style="margin-top:6px">Dose mode: vial = 500 mg → nurse enters 560 mg → system deducts 2 vials. Requires concentration above.</div>'+
          '</div>'+
        '</details>'+

        '<details style="border:1px solid var(--br);border-radius:8px;margin-bottom:10px" '+(edit&&(edit.itemDetails||[]).length?"open":"")+'>'+
          '<summary style="padding:10px 14px;cursor:pointer;font-weight:600;list-style:none;display:flex;align-items:center;gap:6px">'+
            '<span>📦</span> Item quantities / كميات البنود <span class="badge bbl" style="font-weight:400;font-size:11px;margin-left:4px">optional</span>'+
          '</summary>'+
          '<div style="padding:12px 14px;border-top:1px solid var(--br)">'+
            '<div class="fhint" style="margin-bottom:8px">Track extra items (syringes, sets…) — one per line: Name: Qty</div>'+
            '<textarea id="acc2-assignment-items" rows="3" placeholder="Syringe 10ml: 5&#10;Infusion set: 2" style="resize:vertical">'+esc(edit&&(edit.itemDetails||[]).join('\n')||'')+'</textarea>'+
          '</div>'+
        '</details>'+

        '<details style="border:1px solid var(--br);border-radius:8px;margin-bottom:14px" '+(hasPatientRestrictions?"open":"")+'>'+
          '<summary style="padding:10px 14px;cursor:pointer;font-weight:600;list-style:none;display:flex;align-items:center;gap:6px">'+
            '<span>🔒</span> Patient restrictions / تقييدات المريض <span class="badge bbl" style="font-weight:400;font-size:11px;margin-left:4px">optional</span>'+
          '</summary>'+
          '<div class="acc2-form-grid" style="padding:12px 14px;border-top:1px solid var(--br)">'+
            '<div class="fg"><label>Weight / الوزن</label><select id="acc2-assignment-require-weight"><option value="no" '+((!edit||!edit.requireWeight||edit.requireWeight==="no")?"selected":"")+'>Not required</option><option value="optional" '+(edit&&edit.requireWeight==="optional"?"selected":"")+'>Optional</option><option value="required" '+(edit&&edit.requireWeight==="required"?"selected":"")+'>Required</option></select></div>'+
            '<div class="fg"><label>Gender / الجنس</label><select id="acc2-assignment-gender-restriction"><option value="any" '+((!edit||!edit.genderRestriction||edit.genderRestriction==="any")?"selected":"")+'>Any</option><option value="male" '+(edit&&edit.genderRestriction==="male"?"selected":"")+'>Male only</option><option value="female" '+(edit&&edit.genderRestriction==="female"?"selected":"")+'>Female only</option></select></div>'+
            '<div class="fg"><label>Pregnancy / الحمل</label><select id="acc2-assignment-pregnancy-allowed"><option value="any" '+((!edit||!edit.pregnancyAllowed||edit.pregnancyAllowed==="any")?"selected":"")+'>No restriction</option><option value="caution" '+(edit&&edit.pregnancyAllowed==="caution"?"selected":"")+'>Caution</option><option value="avoid" '+(edit&&edit.pregnancyAllowed==="avoid"?"selected":"")+'>Avoid</option><option value="contraindicated" '+(edit&&edit.pregnancyAllowed==="contraindicated"?"selected":"")+'>Contraindicated</option></select></div>'+
            '<div class="fg"><label>Trimester / الثلاثي</label><select id="acc2-assignment-trimester"><option value="any" '+((!edit||!edit.trimesterRestriction||edit.trimesterRestriction==="any")?"selected":"")+'>Any</option><option value="avoid_1" '+(edit&&edit.trimesterRestriction==="avoid_1"?"selected":"")+'>Avoid 1st</option><option value="avoid_2" '+(edit&&edit.trimesterRestriction==="avoid_2"?"selected":"")+'>Avoid 2nd</option><option value="avoid_3" '+(edit&&edit.trimesterRestriction==="avoid_3"?"selected":"")+'>Avoid 3rd</option><option value="only_1" '+(edit&&edit.trimesterRestriction==="only_1"?"selected":"")+'>1st only</option><option value="only_2" '+(edit&&edit.trimesterRestriction==="only_2"?"selected":"")+'>2nd only</option><option value="only_3" '+(edit&&edit.trimesterRestriction==="only_3"?"selected":"")+'>3rd only</option></select></div>'+
            '<div class="fg"><label>Age / العمر (yrs)</label><div class="fl g8"><div style="flex:1"><small style="opacity:.65">Min</small><input id="acc2-assignment-min-age" type="number" min="0" max="120" step="1" placeholder="—" value="'+esc(edit&&edit.minAge!=null?edit.minAge:'')+'" style="width:100%;margin-top:2px"></div><div style="flex:1"><small style="opacity:.65">Max</small><input id="acc2-assignment-max-age" type="number" min="0" max="120" step="1" placeholder="—" value="'+esc(edit&&edit.maxAge!=null?edit.maxAge:'')+'" style="width:100%;margin-top:2px"></div></div></div>'+
          '</div>'+
        '</details>'+

        '<div class="fl g8">'+
          '<button class="btn bp" type="button" data-clickact="acc2SaveAssignment">'+(edit?'Save changes / حفظ التعديلات':'Create custody / إنشاء العهدة')+'</button>'+
          (edit?'<button class="btn bg" type="button" data-clickact="acc2CancelAssignmentEdit">Cancel / إلغاء</button>':'')+
        '</div>'+
      '</div>'+
    '</div>'+
    ...
    '<div class="card">'+
      '<div class="ch" style="flex-wrap:wrap;gap:8px">'+
        '<span class="ct">Custody register / سجل العهد</span>'+
        '<div class="fl g6 ic">'+
          '<select style="min-width:160px" data-changeact="acc2SetCustFilter" data-a1="dept"><option value="">كل الأقسام</option>'+depts().map(function(d){return '<option value="'+esc(d.id)+'" '+(cf.dept===d.id?'selected':'')+'>'+esc(d.name)+'</option>'}).join('')+'</select>'+
          '<select style="min-width:110px" data-changeact="acc2SetCustFilter" data-a1="status"><option value="" '+(cf.status===''||!cf.status?'selected':'')+'>All / الكل</option><option value="active" '+(cf.status==='active'?'selected':'')+'>Active</option><option value="inactive" '+(cf.status==='inactive'?'selected':'')+'>Inactive</option></select>'+
          '<small style="opacity:.5">'+tableRows.length+(tableRows.length<assignments.length?' / '+assignments.length:'')+' records</small>'+
        '</div>'+
      '</div>'+
      '<div class="tw">'+
        '<table class="acc2-table">'+
          '<thead><tr><th>Department / القسم</th><th>Medicine / العلاج</th><th>Conc.</th><th>Quota</th><th>Balance</th><th>Reasons</th><th>Status</th><th>Actions</th></tr></thead>'+
          '<tbody>'+tableRows.map(function(a){
            var concCell=a.unitSize?'<b style="font-size:12px">'+acc2Number(a.unitSize)+'</b><br><small style="opacity:.6">'+esc(a.unitSizeLabel||'mg')+'</small>':'<span style="opacity:.3">—</span>';
            return '<tr>'+
              '<td>'+esc(deptName(a.deptId))+'</td>'+
              '<td><b>'+esc(a.medName)+'</b></td>'+
              '<td style="text-align:center;white-space:nowrap">'+concCell+'</td>'+
              '<td style="text-align:center">'+acc2Number(a.quota)+'</td>'+
              '<td style="text-align:center"><b>'+acc2Number(a.balance)+'</b></td>'+
              '<td style="font-size:12px">'+esc((a.reasons||[]).join('، ')||'—')+'</td>'+
              '<td><span class="badge '+(a.active===false?'bgr':'bgn')+'">'+(a.active===false?'Inactive':'Active')+'</span></td>'+
              '<td style="white-space:nowrap">'+
                '<button class="btn bg bxs" data-clickact="acc2EditAssignment" data-a1="'+esc(a.id)+'">Edit</button> '+
                '<button class="btn '+(a.active===false?'bs':'bd2c')+' bxs" data-clickact="acc2ToggleAssignment" data-a1="'+esc(a.id)+'">'+(a.active===false?'Activate':'Deactivate')+'</button> '+
                '<button class="btn bd2c bxs" data-clickact="acc2DeleteAssignment" data-a1="'+esc(a.id)+'">Delete</button>'+
                (acc2EffectiveMaster()?'<button class="btn bs bxs" title="Reset balance to quota" data-clickact="acc2ResetBalance" data-a1="'+esc(a.id)+'">&#8635;</button>':'')+
              '</td>'+
            '</tr>';
          }).join('')+(tableRows.length===0?'<tr><td colspan="8" style="text-align:center;opacity:.4;padding:20px">No records / لا توجد نتائج</td></tr>':'')+
          '</tbody>'+
        '</table>'+
      '</div>'+
    '</div>'
...
function acc2RefreshMedicationList(){var dept=(E('acc2-assignment-dept')||{}).value||'',list=E('acc2-medication-list');if(list)list.innerHTML=acc2MedicationNames(dept).map(function(x){return '<option value="'+esc(x)+'">'}).join('')};
function acc2EditAssignment(id){ACC2_UI.editAssignmentId=id;ACC2_UI.adminTab='custody';renderMedicationAccountability()};
function acc2CancelAssignmentEdit(){ACC2_UI.editAssignmentId='';renderMedicationAccountability()};
window.acc2SaveAssignment=async function(){if(!canAccManage())return toast('Not authorized.','err');var id=(E('acc2-assignment-id')||{}).value||'',deptId=(E('acc2-assignment-dept')||{}).value||'',medName=String((E('acc2-assignment-med')||{}).value||'').trim(),quota=acc2Number((E('acc2-assignment-quota')||{}).value),reasons=acc2UniqueLines((E('acc2-assignment-reasons')||{}).value),active=!!(E('acc2-assignment-active')&&E('acc2-assignment-active').checked),itemDetails=acc2UniqueLines((E('acc2-assignment-items')||{}).value),unitSize=acc2Number((E('acc2-assignment-unit-size')||{}).value)||0,unitSizeLabel=String((E('acc2-assignment-unit-label')||{}).value||'mg'),inputMode=String((E('acc2-assignment-input-mode')||{}).value||'units'),requireWeight=String((E('acc2-assignment-require-weight')||{}).value||'no'),genderRestriction=String((E('acc2-assignment-gender-restriction')||{}).value||'any'),pregnancyAllowed=String((E('acc2-assignment-pregnancy-allowed')||{}).value||'any'),trimesterRestriction=String((E('acc2-assignment-trimester')||{}).value||'any'),minAge=acc2Number((E('acc2-assignment-min-age')||{}).value)||null,maxAge=acc2Number((E('acc2-assignment-max-age')||{}).value)||null;if(!deptId||!medName||!(quota>0)||!reasons.length)return toast('Department, medicine, positive custody quantity and at least one reason are required.','err');if(!acc2DeptAllowed(deptId))return toast('Access to this department is restricted. / هذا القسم محظور لحسابك.','err');var list=acc2Assignments().map(function(x){return Object.assign({},x)}),duplicate=list.find(function(x){return String(x.deptId)===String(deptId)&&norm(x.medName)===norm(medName)&&acc2Number(x.unitSize||0)===unitSize&&String(x.unitSizeLabel||'mg')===unitSizeLabel&&String(x.id)!==String(id)});if(duplicate)return toast('A custody for '+medName+(unitSize?' ('+acc2Number(unitSize)+' '+unitSizeLabel+')':'')+' already exists in this department. / يوجد عهدة لهذا العلاج بنفس التركيز في هذا القسم.','err');try{if(acc2UsesCF()){await acc2CallMutation({action:'saveAssignment',id:id,deptId:deptId,medName:medName,quota:quota,reasons:reasons,active:active,itemDetails:itemDetails,unitSize:unitSize,unitSizeLabel:unitSizeLabel,inputMode:inputMode,requireWeight:requireWeight,genderRestriction:genderRestriction,pregnancyAllowed:pregnancyAllowed,trimesterRestriction:trimesterRestriction,minAge:minAge,maxAge:maxAge})}else{var meta=acc2ActorMeta(),row=list.find(function(x){return String(x.id)===String(id)});if(row){var deficit=Math.max(0,acc2Number(row.quota)-acc2Number(row.balance));row.deptId=deptId;row.medName=medName;row.quota=quota;row.balance=Math.max(0,Math.min(quota,quota-deficit));row.reasons=reasons;row.active=active;row.itemDetails=itemDetails;row.unitSize=unitSize;row.unitSizeLabel=unitSizeLabel;row.inputMode=inputMode;row.requireWeight=requireWeight;row.genderRestriction=genderRestriction;row.pregnancyAllowed=pregnancyAllowed;row.trimesterRestriction=trimesterRestriction;row.minAge=minAge;row.maxAge=maxAge;row.updatedAt=meta.at;row.updatedBy=meta.name}else{list.push({id:uid('acc2a'),deptId:deptId,medName:medName,quota:quota,balance:quota,reasons:reasons,active:active,itemDetails:itemDetails,unitSize:unitSize,unitSizeLabel:unitSizeLabel,inputMode:inputMode,requireWeight:requireWeight,genderRestriction:genderRestriction,pregnancyAllowed:pregnancyAllowed,trimesterRestriction:trimesterRestriction,minAge:minAge,maxAge:maxAge,createdAt:meta.at,createdBy:meta.name,updatedAt:meta.at,updatedBy:meta.name})}await S.s(ACC2_ASSIGNMENTS_KEY,list)}ACC2_UI.editAssignmentId='';toast('Department custody saved ✓','succ');renderMedicationAccountability()}catch(error){toast(String(error&&error.message||error).replace(/^FirebaseError:\s*/,''),'err')}};
async function acc2ToggleAssignment(id){if(!canAccManage())return toast('Not authorized.','err');var list=acc2Assignments().map(function(x){return Object.assign({},x)}),row=acc2Assignment(id,list);if(!row)return;if(!acc2DeptAllowed(row.deptId))return toast('Access to this department is restricted.','err');try{if(acc2UsesCF()){await acc2CallMutation({action:'toggleAssignment',id:id});acc2ReflectAssignmentsLocally(function(rows){return rows.map(function(x){return String(x.id)===String(id)?Object.assign({},x,{active:x.active===false}):x})})}else{row.active=row.active===false;row.updatedAt=now();row.updatedBy=actor().name;await S.s(ACC2_ASSIGNMENTS_KEY,list)}renderMedicationAccountability()}catch(error){toast(String(error&&error.message||error).replace(/^FirebaseError:\s*/,''),'err')}};
async function acc2DeleteAssignment(id){if(!canAccManage())return toast('Not authorized.','err');var masterOverride=acc2EffectiveMaster()&&!acc2UsesCF();var _chkRow=acc2Assignment(id);if(_chkRow&&!acc2DeptAllowed(_chkRow.deptId))return toast('Access to this department is restricted.','err');var pending=acc2Usage().some(function(u){return String(u.assignmentId)===String(id)&&u.status==='approved_waiting_receipt'});if(pending&&!masterOverride)return toast('This custody has a request awaiting receipt or an active handover and cannot be deleted until it is received. Deactivate it instead. / لهذه العهدة طلب بانتظار الاستلام أو تسليم نشط، لا يمكن حذفها قبل إتمامه. عطّلها بدلاً من ذلك.','err');var history=acc2Usage().some(function(u){return String(u.assignmentId)===String(id)});if(history&&!masterOverride)return toast('This custody has transaction history and cannot be deleted. Deactivate it instead.','err');var prompt=pending?'Master deletion: a request is still awaiting receipt / an active handover exists for this custody. If you delete it now, the receipt can still be completed later — the system will automatically recreate a minimal custody record (capped to exactly the received quantity) once the pending receipt is confirmed. Continue?\n\nحذف للماستر: يوجد طلب بانتظار الاستلام أو تسليم نشط لهذه العهدة. لو حذفتها الآن، يمكن إتمام الاستلام لاحقاً — سيُعاد إنشاء سجل عهدة بالحد الأدنى (بمقدار الكمية المستلمة فقط) تلقائياً عند تأكيد الاستلام المعلق. متابعة؟':history?'Master deletion: remove this custody record? Historical use and handover records will be retained. / حذف العهدة للماستر؟ سيبقى سجل الاستهلاك والاستلام والتسليم محفوظًا.':'Delete this custody permanently? / حذف هذه العهدة نهائيًا؟';var ok=typeof uiConfirm==='function'?await uiConfirm(prompt):window.confirm(prompt);if(!ok)return;try{if(acc2UsesCF()){await acc2CallMutation({action:'deleteAssignment',id:id});acc2ReflectAssignmentsLocally(function(rows){return rows.filter(function(x){return String(x.id)!==String(id)})})}else{await S.s(ACC2_ASSIGNMENTS_KEY,acc2Assignments().filter(function(a){return String(a.id)!==String(id)}))}toast(history?'Custody removed; historical records retained ✓':'Custody deleted ✓','succ');renderMedicationAccountability()}catch(error){toast(String(error&&error.message||error).replace(/^FirebaseError:\s*/,''),'err')}};

function acc2SetCustFilter(key,val){ACC2_UI.custFilter=ACC2_UI.custFilter||{};ACC2_UI.custFilter[key]=val;renderMedicationAccountability()};
async function acc2ResetBalance(id){if(!acc2EffectiveMaster())return toast('Master only.','err');var list=acc2Assignments().map(function(x){return Object.assign({},x)}),row=acc2Assignment(id,list);if(!row)return;var ok=typeof uiConfirm==='function'?await uiConfirm('Reset balance of '+row.medName+' to quota ('+acc2Number(row.quota)+')?'):window.confirm('Reset balance of '+row.medName+' to quota ('+acc2Number(row.quota)+')?');if(!ok)return;try{if(acc2UsesCF()){await acc2CallMutation({action:'resetBalance',id:id});acc2ReflectAssignmentsLocally(function(rows){return rows.map(function(x){return String(x.id)===String(id)?Object.assign({},x,{balance:x.quota}):x})})}else{row.balance=row.quota;row.updatedAt=now();row.updatedBy=actor().name;await S.s(ACC2_ASSIGNMENTS_KEY,list)}toast('Balance reset to '+acc2Number(row.quota)+' ✓','succ');renderMedicationAccountability()}catch(e){toast(String(e&&e.message||e).replace(/^FirebaseError:\s*/,''),'err')}};
function acc2GraceMs(){var mins=30;try{var cfg=S.g('acc2_settings');if(cfg&&Number(cfg.approvalGraceMinutes)>0)mins=Number(cfg.approvalGraceMinutes);}catch(e){console.warn('acc2_settings was skipped after an error; the rest of this screen still renders.', e);}return mins*60000;}
async function acc2SaveGrace(mins){var n=parseInt(mins,10);if(isNaN(n)||n<1||n>720)return toast('Enter a value between 1 and 720 minutes.','err');try{var cfg=Object.assign({},S.g('acc2_settings')||{});cfg.approvalGraceMinutes=n;await S.s('acc2_settings',cfg);toast('Grace period updated to '+n+' min ✓','succ');r17AccRenderVerify();}catch(e){toast(String(e&&e.message||e),'err');}}
async function acc2UndoApproval(id){if(!canAccManage())return toast('Not authorized.','err');var rows=acc2Usage().map(function(x){return Object.assign({},x)}),u=rows.find(function(x){return String(x.id)===String(id)});if(!u||u.status!=='approved_waiting_receipt')return toast('This request is no longer in approved state.','err');if(u.approvedAt&&(Date.now()-new Date(u.approvedAt).getTime())>=acc2GraceMs())return toast('Undo window has expired.','err');try{if(acc2UsesCF()){await acc2CallMutation({action:'undo_approve',id:id})}else{var meta=acc2ActorMeta();var origAList=acc2Assignments(),aList=origAList.map(function(x){return Object.assign({},x)}),aRow=aList.find(function(x){return String(x.id)===String(u.assignmentId)});if(aRow){aRow.balance=Math.min(acc2Number(aRow.quota),acc2Number(aRow.balance)+acc2Number(u.units));aRow.updatedAt=meta.at;await S.s(ACC2_ASSIGNMENTS_KEY,aList)}u.status='pending_pharmacy';delete u.approvedAt;delete u.approvedBy;delete u.approvedByUser;u.pharmacyNote='';try{await S.s(ACC2_USAGE_KEY,rows)}catch(e2){if(aRow)try{await S.s(ACC2_ASSIGNMENTS_KEY,origAList)}catch(e3){console.error('Rollback failed',e3)}throw e2}}toast('Approval undone; request returned to pending ✓','succ');renderMedicationAccountability();}catch(e){toast(String(e&&e.message||e).replace(/^FirebaseError:\s*/,''),'err');}};
function acc2ReviewTab(){var f=ACC2_UI.filters||{},rows=acc2FilterRows(acc2VisibleUsage().slice().sort(function(a,b){return String(b.submittedAt||'').localeCompare(String(a.submittedAt||''))}));var graceMin=Math.round(acc2GraceMs()/60000);var graceCfg=(typeof isMaster==='function'&&isMaster())?'<div class="fg" style="max-width:200px"><label>Undo window (min)</label><div class="fl g6 ic"><input id="acc2-grace-input" type="number" min="1" max="720" value="'+graceMin+'" style="width:70px"><button class="btn bs bxs" data-clickact="acc2SaveGrace">Save</button></div></div>':'';return '<div class="card"><div class="ch"><div><span class="ct">Pharmacy review / مراجعة الصيدلية</span><div class="fhint">Approving does not replenish the department balance. Replenishment occurs only after nursing receipt is recorded.</div></div></div><div class="cb"><div class="acc2-filter-grid"><div class="fg"><label>Department</label><select data-changeact="acc2SetFilter" data-a1="dept"><option value="">All departments</option>'+depts().map(function(d){return '<option value="'+esc(d.id)+'" '+(String(f.dept)===String(d.id)?'selected':'')+'>'+esc(d.name)+'</option>'}).join('')+'</select></div><div class="fg"><label>Status</label><select data-changeact="acc2SetFilter" data-a1="status"><option value="">All statuses</option>'+['pending_pharmacy','approved_waiting_receipt','received_locked','rejected'].map(function(s){return '<option value="'+s+'" '+(f.status===s?'selected':'')+'>'+acc2StatusLabel(s)+'</option>'}).join('')+'</select></div><div class="fg"><label>Medicine</label><input value="'+esc(f.medicine||'')+'" data-inputact="filterMedicine" placeholder="Search medicine"></div>'+graceCfg+'</div><div id="r17-acc-verify"></div></div></div>'}
function r17AccRenderVerify(){var host=E('r17-acc-verify');if(!host){renderMedicationAccountability();return}var rows=acc2FilterRows(acc2VisibleUsage().slice().sort(function(a,b){return String(b.submittedAt||'').localeCompare(String(a.submittedAt||''))}));if(!rows.length){host.innerHTML='<div class="acc2-empty">No records match the selected filters.</div>';return}host.innerHTML='<div class="tw"><table class="acc2-table"><thead><tr><th>Date</th><th>Department</th><th>Medicine</th><th>Used</th><th>Patient file</th><th>Doctor</th><th>Reason</th><th>Status</th><th>Decision</th></tr></thead><tbody>'+rows.map(function(u){var isPending=u.status==='pending_pharmacy';var inGrace=u.status==='approved_waiting_receipt'&&u.approvedAt&&(Date.now()-new Date(u.approvedAt).getTime()<acc2GraceMs());var _asgn=acc2Assignment(u.assignmentId)||{},_conc=(_asgn.unitSize&&_asgn.unitSizeLabel)?acc2Number(_asgn.unitSize)+' '+esc(_asgn.unitSizeLabel):'';return '<tr><td>'+acc2FmtDate(u.consumptionDate)+'</td><td>'+esc(deptName(u.deptId))+'</td><td><b>'+esc(_asgn.medName||u.medName||'—')+'</b>'+(_conc?'<span class="fhint" style="display:inline;margin-right:4px"> '+_conc+'</span>':'')+'</td><td>'+acc2Number(u.units)+'</td><td>'+esc(u.patientFile||'—')+'</td><td>'+esc(u.doctor||'—')+'</td><td>'+esc(u.reasonLabel||u.reason||'—')+(u.note?'<div class="fhint">'+esc(u.note)+'</div>':'')+'</td><td><span class="badge '+acc2StatusClass(u.status)+'">'+acc2StatusLabel(u.status)+'</span><div class="fhint">'+esc(u.pharmacyNote||u.rejectionReason||'')+'</div></td><td>'+(isPending?'<div class="fl g6 ic"><input id="acc2-decision-note-'+esc(u.id)+'" placeholder="Optional note" style="flex:1;min-width:60px"><button class="btn bs bxs" data-clickact="acc2Decision" data-a1="'+esc(u.id)+'" data-a2="approve">Approve</button><button class="btn bd2c bxs" data-clickact="acc2Decision" data-a1="'+esc(u.id)+'" data-a2="reject">Reject</button></div>':inGrace?'<div class="fl g6 ic"><span class="fhint">Approved ✓</span><button class="btn bd2c bxs" data-clickact="acc2UndoApproval" data-a1="'+esc(u.id)+'">Undo / تراجع</button></div>':'<span class="fhint">Decision completed</span>')+'</td></tr>'}).join('')+'</tbody></table></div>'};
window.acc2Decision=async function(id,decision){if(!canAccManage())return toast('Not authorized.','err');var rows=acc2Usage().map(function(x){return Object.assign({},x)}),u=rows.find(function(x){return String(x.id)===String(id)});if(!u||u.status!=='pending_pharmacy')return toast('This request is no longer pending.','err');if(!acc2DeptAllowed(u.deptId))return toast('Access to this department is restricted.','err');var note=String((E('acc2-decision-note-'+id)||{}).value||'').trim();var _noteEl=E('acc2-decision-note-'+id),_decisionCell=_noteEl&&_noteEl.closest('td');if(_decisionCell){_decisionCell.innerHTML='<span style="opacity:.6;font-size:12px">⏳ '+(decision==='approve'?'Approving… / جاري الاعتماد':'Rejecting… / جاري الرفض')+'</span>'}try{if(acc2UsesCF()){await acc2CallMutation({action:'decision',id:id,decision:decision,note:note})}else{var meta=acc2ActorMeta();if(decision==='approve'){u.status='approved_waiting_receipt';u.approvedAt=meta.at;u.approvedBy=meta.name;u.approvedByUser=meta.user;u.pharmacyNote=note;var origAList=acc2Assignments(),aList=origAList.map(function(x){return Object.assign({},x)}),aRow=aList.find(function(x){return String(x.id)===String(u.assignmentId)});if(aRow){aRow.balance=Math.max(0,acc2Number(aRow.balance)-acc2Number(u.units));aRow.updatedAt=meta.at;await S.s(ACC2_ASSIGNMENTS_KEY,aList)}try{await S.s(ACC2_USAGE_KEY,rows)}catch(e2){if(aRow)try{await S.s(ACC2_ASSIGNMENTS_KEY,origAList)}catch(e3){console.error('Rollback failed',e3)}throw e2}}else{u.status='rejected';u.rejectedAt=meta.at;u.rejectedBy=meta.name;u.rejectedByUser=meta.user;u.rejectionReason=note;u.pharmacyNote=note;await S.s(ACC2_USAGE_KEY,rows)}}toast(decision==='approve'?'Request approved; balance deducted. Replenished after nursing receipt ✓':'Request rejected; balance unchanged / مرفوض، الرصيد بدون تغيير ✓','succ');r17AccRenderVerify();if(typeof window.ccUpdateBadges==='function')window.ccUpdateBadges()}catch(error){r17AccRenderVerify();if(typeof window.ccUpdateBadges==='function')window.ccUpdateBadges();toast(String(error&&error.message||error).replace(/^FirebaseError:\s*/,''),'err')}};

function acc2HandoverTab(){var deptFilter=ACC2_UI.receiptDept||'',usage=acc2VisibleUsage(),waiting=usage.filter(function(u){return u.status==='approved_waiting_receipt'&&(!deptFilter||String(u.deptId)===String(deptFilter))}),receipts=acc2Receipts().filter(function(r){return !deptFilter||String(r.deptId)===String(deptFilter)}).sort(function(a,b){return String(b.receivedAt||b.createdAt||'').localeCompare(String(a.receivedAt||a.createdAt||''))}),byDept={};waiting.forEach(function(u){var key=String(u.deptId||'');if(!byDept[key])byDept[key]=[];byDept[key].push(u)});var cards=Object.keys(byDept).map(function(deptId){var rows=byDept[deptId],total=acc2Sum(rows);return '<div class="card acc2-qr-dept-card" data-acc2-qr-dept="'+esc(deptId)+'"><div class="ch"><div><span class="ct">'+esc(deptName(deptId))+'</span><div class="fhint">Select approved records, then create two temporary QR codes: pharmacy delivery and department receipt. The balance is replenished only after both confirmations.<br>حدد السجلات المعتمدة ثم أنشئ رمزين مؤقتين: تسليم الصيدلية واستلام القسم. لا يُعوض الرصيد إلا بعد تأكيد الطرفين.</div></div><span class="badge bbl">'+rows.length+' request(s) · '+total+' units</span></div><div class="tw"><table class="acc2-table"><thead><tr><th></th><th>Medicine / العلاج</th><th>Units / الوحدات</th><th>Consumption date</th><th>Patient file</th><th>QR status</th></tr></thead><tbody>'+rows.map(function(u){var activeQr=!!(u.handoverSessionId&&u.handoverExpiresAt&&new Date(u.handoverExpiresAt).getTime()>Date.now());return '<tr><td><input class="acc2-qr-usage" data-dept="'+esc(deptId)+'" type="checkbox" value="'+esc(u.id)+'" '+(activeQr?'disabled':'')+'></td><td><b>'+esc(u.medName||'—')+'</b></td><td>'+acc2Number(u.units)+'</td><td>'+acc2FmtDate(u.consumptionDate)+'</td><td>'+esc(u.patientFile||'—')+'</td><td>'+(activeQr?'<span class="badge byl">Active QR session</span> <button class="btn bg bxs" type="button" style="font-size:11px;padding:2px 8px" data-acc2-qr-action="reissue-one" data-usage-id="'+esc(u.id)+'" data-dept="'+esc(deptId)+'">Reissue / إعادة إصدار</button>':u.handoverSessionId?'<span class="badge brd">Expired — may reissue</span>':'<span class="badge bgr">Not created</span>')+'</td></tr>'}).join('')+'</tbody></table></div><div class="fl g8" style="margin-top:12px"><button class="btn bg bsm" type="button" data-acc2-qr-action="select" data-dept="'+esc(deptId)+'">Select available / تحديد المتاح</button><button class="btn bs" type="button" data-acc2-qr-action="create" data-dept="'+esc(deptId)+'">Create temporary dual QR / إنشاء رمزي QR مؤقتين</button><button class="btn bg bsm" type="button" data-acc2-qr-action="reissue-selected" data-dept="'+esc(deptId)+'" title="One QR for every selected record of this department / رمز واحد لكل السجلات المحددة لهذا القسم">Reissue selected / إعادة إصدار المحدد</button></div></div>'}).join('');return '<div class="card"><div class="ch"><div><span class="ct">Temporary dual-QR handover / الاستلام والتسليم برمزي QR مؤقتين</span><div class="fhint">Each QR expires after 30 minutes and can confirm only its assigned party. No login or OTP is required; employee name and number are recorded.<br>تنتهي صلاحية كل رمز بعد 30 دقيقة ويؤكد الطرف المخصص له فقط. لا يلزم تسجيل دخول أو رمز OTP، ويُسجل اسم الموظف ورقمه.</div></div></div><div class="cb"><div class="fg" style="max-width:320px"><label>Department filter</label><select data-changeact="acc2SetReceiptDept"><option value="">All departments</option>'+depts().map(function(d){return '<option value="'+esc(d.id)+'" '+(String(deptFilter)===String(d.id)?'selected':'')+'>'+esc(d.name)+'</option>'}).join('')+'</select></div></div></div>'+(cards||'<div class="card"><div class="cb"><div class="acc2-empty">No approved requests are waiting for QR handover.</div></div></div>')+'<div class="card"><div class="ch"><span class="ct">Receipt and handover history / سجل الاستلام والتسليم</span></div><div class="tw"><table class="acc2-table"><thead><tr><th>Receipt date</th><th>Department</th><th>Pharmacy delivered by</th><th>Department received by</th><th>Requests</th><th>Units</th><th>Medicines</th><th>Method</th></tr></thead><tbody>'+receipts.map(function(r){return '<tr><td>'+acc2FmtDate(r.receivedDate||acc2Date(r.receivedAt))+'</td><td>'+esc(deptName(r.deptId))+'</td><td><b>'+esc(r.pharmacyName||'—')+'</b>'+(r.pharmacyEmployeeId?'<div class="fhint">'+esc(r.pharmacyEmployeeId)+'</div>':'')+'</td><td><b>'+esc(r.nurseName||'—')+'</b>'+(r.departmentEmployeeId?'<div class="fhint">'+esc(r.departmentEmployeeId)+'</div>':'')+'</td><td>'+((r.usageIds||[]).length)+'</td><td>'+acc2Number(r.totalUnits)+'</td><td>'+esc((r.medicineTotals||[]).map(function(x){return x.medName+' × '+x.units}).join('، '))+'</td><td>'+esc(r.confirmationMethod==='temporary_dual_qr'?'Dual QR / رمزان QR':'Legacy / سابق')+'</td></tr>'}).join('')+'</tbody></table></div></div>'}

function acc2RegimenBuilder(){var edit=acc2Regimens().find(function(r){return String(r.id)===String(ACC2_UI.editRegimenId)})||null,scopeDept=acc2RegimenScopeDept(),deptId=edit?edit.deptId:(scopeDept||ACC2_UI.regimenDept||''),assignments=acc2DeptAssignments(deptId),activeVersion=edit&&regimenVersion(edit),options=scopeDept?depts().filter(function(d){return String(d.id)===String(scopeDept)}):depts();return '<div class="card"><div class="ch"><div><span class="ct">'+(edit?'Add a new regimen version / إضافة نسخة جديدة':'Create regimen / إنشاء خطة علاجية')+'</span><div class="fhint">Each regimen belongs to one department and can contain several medicines with approved quantities.'+(scopeDept?' This account is restricted to '+esc(deptName(scopeDept))+'.':'')+'</div></div></div><div class="cb"><div class="acc2-form-grid"><div class="fg"><label>Department *</label><select id="acc2-regimen-dept" '+(edit||scopeDept?'disabled':'')+' data-changeact="acc2RegimenDeptChanged"><option value="">Select department...</option>'+options.map(function(d){return '<option value="'+esc(d.id)+'" '+(String(deptId)===String(d.id)?'selected':'')+'>'+esc(d.name)+'</option>'}).join('')+'</select></div><div class="fg"><label>Regimen name *</label><input id="acc2-regimen-name" value="'+esc(edit&&edit.name||'')+'" '+(edit?'readonly':'')+' placeholder="Sepsis"></div><div class="fg"><label>Infection source / indication *</label><input id="acc2-regimen-source" value="'+esc(edit&&edit.infectionSource||'')+'" placeholder="Pneumonia, UTI, abdominal..."></div><div class="fg"><label>Treatment line *</label><select id="acc2-regimen-line"><option value="first_line">First-line</option><option value="second_line">Second-line</option><option value="third_line">Third-line</option></select></div><div class="fg"><label>Version label *</label><input id="acc2-regimen-version" placeholder="Regimen A"></div></div><div class="acc2-regimen-items">'+(assignments.length?assignments.map(function(a){var defaultQty=(activeVersion&&((activeVersion.items||[]).find(function(i){return String(i.assignmentId)===String(a.id)})||{}).qty)||'';return '<label class="acc2-regimen-item"><input type="checkbox" data-acc2-regimen-assignment="'+esc(a.id)+'" '+(defaultQty?'checked':'')+'><span><b>'+esc(a.medName)+'</b><small>Custody '+acc2Number(a.quota)+' · balance '+acc2Number(a.balance)+'</small></span><input class="acc2-regimen-qty" data-assignment="'+esc(a.id)+'" type="number" min="0.001" step="any" value="'+esc(defaultQty)+'" placeholder="Qty"></label>'}).join(''):'<div class="acc2-empty">Select a department with active custody medicines first.</div>')+'</div><div class="fl g8"><button class="btn bp" data-clickact="acc2SaveRegimenVersion">'+(edit?'Add version':'Create regimen')+'</button>'+(edit?'<button class="btn bg" data-clickact="acc2CancelRegimenEdit">Cancel</button>':'')+'</div></div></div>'}
...
function acc2RegimensTab(){var regimens=acc2Regimens();return acc2RegimenBuilder()+'<div class="card"><div class="ch"><span class="ct">Configured regimens / الخطط المعرّفة</span></div><div class="cb">'+(regimens.length?regimens.map(function(r){var version=regimenVersion(r),items=(version&&version.items)||[];return '<div class="acc2-regimen-admin '+(r.paused?'paused':'')+'"><div class="acc2-regimen-head"><div><b>'+esc(r.name)+'</b><div class="fhint">'+esc(r.infectionSource||'Indication not set')+' · '+esc(deptName(r.deptId))+'</div></div><div class="fl g8"><select data-changeact="acc2ActivateRegimen" data-a1="'+esc(r.id)+'">'+(r.versions||[]).map(function(v){return '<option value="'+esc(v.id)+'" '+(String(v.id)===String(r.activeVersionId)?'selected':'')+'>'+esc(v.label)+'</option>'}).join('')+'</select><button class="btn '+(r.paused?'bs':'bd2c')+' bsm" data-clickact="acc2ToggleRegimenPause" data-a1="'+esc(r.id)+'">'+(r.paused?'Resume entry / استئناف':'Pause for replacement / إيقاف للاستبدال')+'</button><button class="btn bg bsm" data-clickact="acc2StartRegimenVersion" data-a1="'+esc(r.id)+'">+ Version</button><button class="btn bd2c bsm" data-clickact="acc2DeleteRegimen" data-a1="'+esc(r.id)+'">Delete</button></div></div><div class="acc2-regimen-lines">'+items.map(function(i){var a=acc2Assignment(i.assignmentId)||{};return '<span>'+esc(a.medName||i.medName||'Medicine')+' × '+acc2Number(i.qty)+'</span>'}).join('')+'</div><div class="fhint">'+(r.paused?'Department entry is currently blocked for medicines in this regimen.':'Department sees this as the currently approved regimen.')+'</div></div>'}).join(''):'<div class="acc2-empty">No regimens configured.</div>')+'</div></div>'}
function acc2StartRegimenVersion(id){ACC2_UI.editRegimenId=id;ACC2_UI.adminTab='regimens';renderMedicationAccountability()};
function acc2CancelRegimenEdit(){ACC2_UI.editRegimenId='';renderMedicationAccountability()};
...
function acc2ShowExpirySection(assignmentId){var sec=document.getElementById('acc2-expiry-section-'+assignmentId);var btn=document.getElementById('acc2-expiry-reveal-'+assignmentId);if(sec)sec.hidden=false;if(btn)btn.hidden=true;if(sec){var d=sec.querySelector('.acc2-expiry-date');if(d)d.focus()}};
function acc2AddExpiryRow(assignmentId){var container=document.getElementById('acc2-expiry-rows-'+CSS.escape(String(assignmentId)));if(!container)return;var row=document.createElement('div');row.className='acc2-expiry-row';row.innerHTML='<div class="acc2-expiry-row-inner"><span class="acc2-expiry-lbl">Expiry / تاريخ الانتهاء</span><input class="acc2-expiry-date" type="date" lang="en" dir="ltr" style="flex:1"><span class="acc2-expiry-lbl">Qty / الكمية</span><input class="acc2-expiry-qty" type="number" min="1" step="1" placeholder="—" style="width:80px"><button class="btn bd2c bxs" type="button" data-clickact="acc2RemoveExpiryRow">✕</button></div>';container.appendChild(row)};
function acc2RemoveExpiryRow(btn){var row=btn.closest('.acc2-expiry-row');if(row)row.remove()};
async function acc2SaveExpiryBatches(assignmentId){if(!acc2DepartmentRole())return toast('Department access required.','err');var a=acc2Assignments().find(function(x){return String(x.id)===String(assignmentId)});if(!a||a.active===false||String(a.deptId)!==String((window.CU&&CU.deptId)||''))return toast('Custody not available for this department.','err');var batches=Array.from(document.querySelectorAll('#acc2-expiry-rows-'+CSS.escape(String(assignmentId))+' .acc2-expiry-row')).map(function(row){var d=String((row.querySelector('.acc2-expiry-date')||{}).value||'').trim();var q=acc2Number((row.querySelector('.acc2-expiry-qty')||{}).value)||null;return (d||q)?{date:d||null,qty:q}:null}).filter(Boolean);var all=acc2Array(ACC2_EXPIRY_KEY).filter(function(x){return String(x.assignmentId)!==String(assignmentId)});all.push({assignmentId:assignmentId,deptId:a.deptId,batches:batches,updatedAt:now(),updatedBy:actor().name});try{await S.s(ACC2_EXPIRY_KEY,all);toast('Expiry batches saved ✓','succ');var _eb=E('acc2-expiry-banner');if(_eb)_eb.innerHTML=acc2ExpiryAlertBanner(a.deptId);else renderMedicationAccountability()}catch(error){toast(String(error&&error.message||error),'err')}};
async function acc2CancelUsage(id,redirectTab){if(!acc2DepartmentRole())return toast('Department access required.','err');var usageList=acc2Usage(),u=usageList.find(function(x){return String(x.id)===String(id)});if(!u||!(u.status==='pending_pharmacy'||u.status==='rejected'))return toast('This submission can no longer be modified. / لا يمكن تعديل هذا الطلب.','err');if(String(u.deptId)!==String((window.CU&&CU.deptId)||''))return toast('Access denied.','err');var ok=typeof window.uiConfirm==='function'?await window.uiConfirm('Delete this submission? / حذف هذا الطلب؟'):window.confirm('Delete this submission? / حذف هذا الطلب؟');if(!ok)return;try{await acc2CallMutation({action:'cancelUsage',id:id});acc2ReflectUsageLocally(function(rows){return rows.filter(function(x){return String(x.id)!==String(id)})});toast('Deleted ✓ / تم الحذف ✓','succ');if(redirectTab)ACC2_UI.deptTab=redirectTab;renderMedicationAccountability()}catch(error){toast(String(error&&error.message||error).replace(/^FirebaseError:\s*/,''),'err')}};
function acc2MySubmissionsTab(deptId){
  var subDays=ACC2_UI.subDays!=null?ACC2_UI.subDays:30;
  var cutoff=subDays>0?Date.now()-subDays*86400000:0;
  var custUsage=(acc2Usage()||[]).filter(function(u){return u&&String(u.deptId)===String(deptId)&&u.status!=='received_locked'});
  var planUsage=((window.S&&S.g('accountability_plan_usage_v1'))||[]).filter(function(u){return u&&String(u.deptId)===String(deptId)&&u.status!=='received_locked'});
  // _medNames carries the raw (unescaped) medicine names for filtering; meds is the display string.
  var all=custUsage.map(function(u){return {_type:'custody',id:u.id,status:u.status,_medNames:[u.medName].filter(Boolean),meds:esc(u.medName||'—')+' × '+acc2Number(u.units),date:u.consumptionDate||'—',file:u.patientFile||'—',doctor:u.doctor||'—',note:u.pharmacyNote||u.rejectionReason||'',submittedAt:u.submittedAt||''}})
    .concat(planUsage.map(function(u){return {_type:'plan',id:u.id,status:u.status,_medNames:(u.medicines||[]).map(function(m){return m.medName}).filter(Boolean),meds:esc(u.planName||'—')+': '+(u.medicines||[]).map(function(m){return esc(m.medName)+' × '+m.actualQty}).join(', '),date:u.consumptionDate||'—',file:u.patientFile||'—',doctor:u.doctor||'—',note:u.pharmacyNote||u.rejectionReason||'',submittedAt:u.submittedAt||''}}))
    .sort(function(a,b){return String(b.submittedAt).localeCompare(String(a.submittedAt))});
  if(cutoff>0)all=all.filter(function(u){var t=new Date(u.submittedAt||u.date||0).getTime();return t>=cutoff;});
  // Options come from the date-filtered set before the medicine filter, so the
  // dropdown keeps every choice available while one is selected.
  var medNames=[];all.forEach(function(u){u._medNames.forEach(function(nm){if(medNames.indexOf(nm)<0)medNames.push(nm)})});medNames.sort();
  var subMed=ACC2_UI.subMed||'';
  if(subMed)all=all.filter(function(u){return u._medNames.indexOf(subMed)>=0});
  var medSelect=medNames.length?'<select data-acc2-view-control="1" data-changeact="acc2SubSetMed" style="margin:0;padding:5px 8px;font-size:12px;border:1px solid var(--bd);border-radius:7px;background:var(--s2);color:var(--tx)"><option value="">All medicines / كل الأدوية</option>'+medNames.map(function(n){return '<option value="'+esc(n)+'"'+(n===subMed?' selected':'')+'>'+esc(n)+'</option>'}).join('')+'</select>':'';
  // Only the medicine filter is offered here; the period buttons were removed on
  // request. subDays stays at its default so the list is still bounded.
  var filterBar=medSelect?'<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:10px">'+medSelect+'</div>':'';
  if(!all.length)return filterBar+'<div class="acc2-empty" style="padding:32px 0;text-align:center">No submissions in this period. / لا توجد طلبات في هذه الفترة.<div class="fhint" style="margin-top:6px">Entries disappear once a receipt handover is recorded. / يختفي الطلب عند تأكيد استلام الأدوية.</div></div>';
  var statusCls={pending_pharmacy:'byw',approved_waiting_receipt:'bbl',rejected:'brd'};
  var statusLbl={pending_pharmacy:'⏳ PENDING / معلق',approved_waiting_receipt:'✓ Approved / معتمد',rejected:'✕ Rejected / مرفوض'};
  return filterBar+'<div class="card"><div class="ch"><span class="ct">📝 My Submissions / طلباتي</span><div class="fhint">Submissions disappear from this log after the receipt handover is completed. / يختفي الطلب بعد إتمام محضر الاستلام والتسليم.</div></div><div class="cb"><div class="tw"><table class="acc2-table" style="font-size:12px"><thead><tr><th>TYPE / النوع</th><th>MEDICINE(S) / الأدوية</th><th>DATE / التاريخ</th><th>PATIENT FILE / الملف</th><th>DOCTOR / الطبيب</th><th>STATUS / الحالة</th><th></th></tr></thead><tbody>'+
    all.map(function(u){
      var typeBadge=u._type==='plan'?'<span class="badge bbl" style="font-size:10px">خطة / Plan</span>':'<span class="badge bgn" style="font-size:10px">عهدة / Custody</span>';
      var cancelBtn='';
      if(u.status==='pending_pharmacy'){cancelBtn=u._type==='plan'?'<button class="btn bd2c bxs" type="button" data-clickact="acc3CancelPlanUsage" data-a1="'+esc(u.id)+'">✕ إلغاء</button>':'<button class="btn bd2c bxs" type="button" data-clickact="acc2CancelUsage" data-a1="'+esc(u.id)+'">✕ إلغاء</button>'}
      else if(u.status==='rejected'){if(u._type==='plan'){cancelBtn='<div class="fl g4" style="flex-wrap:wrap;gap:4px"><button class="btn bd2c bxs" type="button" data-clickact="acc3CancelPlanUsage" data-a1="'+esc(u.id)+'">🗑 Delete / حذف</button><button class="btn bs bxs" type="button" data-clickact="acc3CancelPlanUsage" data-a1="'+esc(u.id)+'" data-a2="plans">✏ Edit & Resubmit / تعديل وإعادة إرسال</button></div>'}else{cancelBtn='<div class="fl g4" style="flex-wrap:wrap;gap:4px"><button class="btn bd2c bxs" type="button" data-clickact="acc2CancelUsage" data-a1="'+esc(u.id)+'">🗑 Delete / حذف</button><button class="btn bs bxs" type="button" data-clickact="acc2CancelUsage" data-a1="'+esc(u.id)+'" data-a2="custody">✏ Edit & Resubmit / تعديل وإعادة إرسال</button></div>'}}
      var noteHtml=u.note?'<div class="fhint" style="color:var(--red,#e74c3c);margin-top:4px">📋 '+esc(u.note)+'</div>':'';
      return '<tr><td>'+typeBadge+'</td><td><b>'+u.meds+'</b></td><td>'+acc2FmtDate(u.date)+'</td><td>'+esc(u.file)+'</td><td>'+esc(u.doctor)+'</td><td><span class="badge '+(statusCls[u.status]||'bgr')+'">'+(statusLbl[u.status]||esc(u.status))+'</span>'+noteHtml+'</td><td>'+cancelBtn+'</td></tr>';
...
function acc2PendingUnits(assignmentId){return pendingUnits(acc2Usage(),assignmentId)}
function acc2EffectiveBalance(a){return effectiveBalance(a,acc2Usage())}
function acc2BuildDeptStatsHTML(deptId,assignments,usage){var waiting=usage.filter(function(u){return u.status==='approved_waiting_receipt'});var totalEffective=assignments.reduce(function(s,a){return s+acc2EffectiveBalance(a)},0);return '<div id="acc2-dept-stats" class="acc2-stats"><div class="sc"><div class="sl">Assigned medicines</div><div class="sv">'+assignments.length+'</div></div><div class="sc"><div class="sl">Approved custody</div><div class="sv">'+acc2Sum(assignments,'quota')+'</div></div><div class="sc"><div class="sl">الرصيد المتاح</div><div class="sv">'+totalEffective+'</div></div><div class="sc"><div class="sl">Waiting receipt</div><div class="sv">'+acc2Sum(waiting)+'</div><div class="ss">'+waiting.length+' requests</div></div></div>';}
function acc2BuildCardHTML(a,usage,deptId){var blocked=acc2SubmissionBlocked(deptId,a.id),remaining=acc2EffectiveBalance(a);/* Expiry batches are optional and most custody records never use them, so the
   panel stays collapsed behind a single button and only opens on its own when
   a batch has already been saved. */var hasExpiry=acc2GetExpiryBatches(a.id).length>0;return '<div id="acc2-card-'+esc(a.id)+'" class="card acc2-med-card '+(blocked?'blocked':'')+'"><div class="ch"><div><span class="ct" dir="ltr">'+esc(a.medName)+'</span><div class="fhint"><span dir="rtl">الرصيد المتاح: <b>'+remaining+'</b> من '+acc2Number(a.quota)+'</span><span style="margin:0 6px;opacity:.4">·</span><span dir="ltr">Available: <b>'+remaining+'</b> of '+acc2Number(a.quota)+'</span></div></div><span class="badge '+(remaining>0?'bgn':'brd')+'">'+remaining+' / '+acc2Number(a.quota)+'</span></div><div class="cb">'+(blocked?'<div class="alert-banner">Entry is paused because '+esc(blocked.name)+' is being replaced by pharmacy. / الإدخال متوقف مؤقتًا لاستبدال الخطة.</div>':'')+'<div style="font-weight:600;padding:4px 0;margin-bottom:6px">Submit usage / إدخال استهلاك</div><div class="acc2-form-grid">'+(a.inputMode==='dose'&&acc2Number(a.unitSize)>0?'<div class="fg"><label>Dose given / الجرعة المُعطاة * ('+esc(a.unitSizeLabel||'mg')+')</label><input data-acc2="dose" data-assignment="'+esc(a.id)+'" type="number" min="0.001" step="any" data-inputact="acc2UpdateDoseCalc" data-a1="'+esc(a.id)+'" data-a2="'+acc2Number(a.unitSize)+'"><div class="fhint acc2-dose-calc-'+esc(a.id)+'">Enter dose → units auto-calculated (1 unit = '+acc2Number(a.unitSize)+esc(' '+(a.unitSizeLabel||'mg'))+')</div></div>':'<div class="fg"><label>Units used / الوحدات المستعملة *</label><input data-acc2="units" data-assignment="'+esc(a.id)+'" type="number" min="1" max="'+remaining+'" step="1" inputmode="numeric"></div>')+(a.requireWeight&&a.requireWeight!=='no'?'<div class="fg"><label>Patient weight / وزن المريض (kg)'+(a.requireWeight==='required'?' *':' (optional)')+'</label><input data-acc2="weight" data-assignment="'+esc(a.id)+'" type="number" min="1" max="500" step="0.1" placeholder="kg"></div>':'')+(a.genderRestriction&&a.genderRestriction!=='any'?'<div class="fg"><div class="alert-banner-y">⚠ '+esc(a.genderRestriction==='male'?'Male patients only / ذكور فقط':'Female patients only / إناث فقط')+'</div><label>Patient gender / جنس المريض *</label><select data-acc2="gender" data-assignment="'+esc(a.id)+'"><option value="">Select...</option><option value="male">Male / ذكر</option><option value="female">Female / أنثى</option></select></div>':'')+(a.pregnancyAllowed&&a.pregnancyAllowed!=='any'?'<div class="fg"><div class="alert-banner-y">⚠ '+esc({caution:'Use with caution / بحذر',avoid:'Avoid in pregnancy / يُتجنب',contraindicated:'Contraindicated / موانع'}[a.pregnancyAllowed]||a.pregnancyAllowed)+'</div><label>Pregnancy status / حالة الحمل</label><select data-acc2="pregnancy" data-assignment="'+esc(a.id)+'"><option value="not_pregnant">Not pregnant / غير حامل</option><option value="trimester_1">1st trimester / الثلث الأول</option><option value="trimester_2">2nd trimester / الثلث الثاني</option><option value="trimester_3">3rd trimester / الثلث الثالث</option></select></div>':'')+(a.minAge||a.maxAge?'<div class="fg"><div class="alert-banner-y" style="margin-bottom:6px">⚠ Age restriction / تقييد العمر: '+(a.minAge&&a.maxAge?a.minAge+' – '+a.maxAge+' years / سنة':a.minAge?'≥ '+a.minAge+' years / سنة':'≤ '+a.maxAge+' years / سنة')+'</div><label>Patient age / عمر المريض (years) '+(a.minAge||a.maxAge?'*':'')+'</label><input data-acc2="age" data-assignment="'+esc(a.id)+'" type="number" min="0" max="120" step="1" placeholder="years / سنة"></div>':'')+'<div class="fg"><label>Consumption date / تاريخ الاستهلاك *</label><input data-acc2="date" data-assignment="'+esc(a.id)+'" type="date" dir="ltr" lang="en" max="'+acc2Today()+'"></div><div class="fg"><label>Patient file number / رقم الملف *</label><input data-acc2="file" data-assignment="'+esc(a.id)+'"></div><div class="fg"><label>Doctor recorded in system / الطبيب المسجل *</label><input data-acc2="doctor" data-assignment="'+esc(a.id)+'"></div><div class="fg"><label>Usage reason / سبب الاستهلاك *</label><select data-acc2="reason" data-assignment="'+esc(a.id)+'"><option value="">Select reason...</option>'+(a.reasons||[]).map(function(r){return '<option value="'+esc(r)+'">'+esc(r)+'</option>'}).join('')+'</select></div><div class="fg"><label>Notes / ملاحظات</label><input data-acc2="note" data-assignment="'+esc(a.id)+'"></div></div><button class="btn bp" style="margin-top:10px" '+(blocked||remaining<=0?'disabled':'')+' data-clickact="acc2SubmitUsage" data-a1="'+esc(a.id)+'">Submit to pharmacy / رفع للصيدلية</button></div>'+(hasExpiry?'':'<button class="btn bg bxs" type="button" id="acc2-expiry-reveal-'+esc(a.id)+'" data-clickact="acc2ShowExpirySection" data-a1="'+esc(a.id)+'" style="margin-top:12px">📅 + دفعة انتهاء صلاحية / Add expiry batch</button>')+'<div class="acc2-expiry-section" id="acc2-expiry-section-'+esc(a.id)+'"'+(hasExpiry?'':' hidden')+'><div class="acc2-expiry-sec-hdr"><span>📅 دفعات انتهاء الصلاحية / Expiry Batches <span style="font-weight:400;opacity:.6;font-size:11px">(اختياري / Optional)</span></span><button class="btn bg bxs" type="button" data-clickact="acc2AddExpiryRow" data-a1="'+esc(a.id)+'">+ دفعة جديدة / New +</button></div><div id="acc2-expiry-rows-'+esc(a.id)+'" class="acc2-expiry-rows">'+acc2GetExpiryBatches(a.id).map(function(b,i){return '<div class="acc2-expiry-row"><div class="acc2-expiry-row-inner"><span class="acc2-expiry-lbl">Expiry / تاريخ الانتهاء</span><input class="acc2-expiry-date" type="date" lang="en" dir="ltr" value="'+esc(b.date||'')+'"><span class="acc2-expiry-lbl">Qty / الكمية</span><input class="acc2-expiry-qty" type="number" min="1" step="1" value="'+esc(b.qty||'')+'" placeholder="—" style="width:80px"><button class="btn bd2c bxs" type="button" data-clickact="acc2RemoveExpiryRow">✕</button></div></div>';}).join('')+(acc2GetExpiryBatches(a.id).length===0?'<div class="acc2-expiry-row"><div class="acc2-expiry-row-inner"><span class="acc2-expiry-lbl">Expiry / تاريخ الانتهاء</span><input class="acc2-expiry-date" type="date" lang="en" dir="ltr"><span class="acc2-expiry-lbl">Qty / الكمية</span><input class="acc2-expiry-qty" type="number" min="1" step="1" placeholder="—" style="width:80px"><button class="btn bd2c bxs" type="button" data-clickact="acc2RemoveExpiryRow">✕</button></div></div>':'')+'</div><div class="acc2-expiry-save-row"><button class="btn bp bxs" type="button" data-clickact="acc2SaveExpiryBatches" data-a1="'+esc(a.id)+'">💾 حفظ الدفعات / Save</button></div></div></div></div>'}
function acc2DepartmentCards(deptId){var assignments=acc2DeptAssignments(deptId),usage=acc2Usage().filter(function(u){return String(u.deptId)===String(deptId)});var banner='<div id="acc2-expiry-banner">'+acc2ExpiryAlertBanner(deptId)+'</div>';var stats=acc2BuildDeptStatsHTML(deptId,assignments,usage);var notice=acc2RegimenNotice(deptId);if(!assignments.length){return banner+stats+notice+'<div class="acc2-empty">No active custody medicines assigned.</div>';}var activeId=ACC2_UI.deptMedTab;if(!activeId||!assignments.find(function(a){return String(a.id)===String(activeId)})){activeId=assignments[0].id;}ACC2_UI.deptMedTab=activeId;var search=(ACC2_UI.medSearch||'').toLowerCase().trim();var filtered=search?assignments.filter(function(a){return(a.medName||'').toLowerCase().indexOf(search)>=0}):assignments;var sidebar='<div class="acc2-med-sidebar"><div style="padding:10px 12px 8px"><input id="acc2-med-search" data-acc2-view-control="1" type="text" placeholder="🔍 ابحث / Search..." value="'+esc(ACC2_UI.medSearch||'')+'" data-inputact="acc2MedSearch" style="width:100%;box-sizing:border-box;padding:8px 10px;border:1px solid var(--br);border-radius:6px;background:var(--bg2,var(--bg));color:var(--tx);font-size:13px"></div>'+(!filtered.length?'<div style="padding:16px 14px;opacity:.6;font-size:13px">لا توجد نتائج</div>':filtered.map(function(a){var eff=acc2EffectiveBalance(a);var pending=acc2PendingUnits(a.id);var label=a.medName+(a.unitSize?' '+acc2Number(a.unitSize)+' '+(a.unitSizeLabel||'mg').toUpperCase():'');var isActive=String(a.id)===String(activeId);return '<button class="acc2-med-list-item'+(isActive?' active':'')+'" type="button" data-clickact="acc2SetMedTab" data-a1="'+esc(a.id)+'"><span class="acc2-med-list-name" dir="ltr">'+esc(label)+'</span><div class="acc2-med-list-badges"><span class="badge '+(eff>0?'bgn':'brd')+'" style="font-size:11px">'+eff+' / '+acc2Number(a.quota)+'</span>'+(pending>0?'<span class="badge byl" style="font-size:11px">'+pending+' ⏳</span>':'')+'</div></button>';}).join(''))+'</div>';var active=assignments.find(function(a){return String(a.id)===String(activeId)});var panel='<div class="acc2-med-panel">'+(active?acc2BuildCardHTML(active,usage,deptId):'<div class="acc2-empty" style="margin-top:40px">اختر علاجاً من القائمة</div>')+'</div>';return banner+stats+notice+'<div class="acc2-dept-layout">'+sidebar+panel+'</div>';}
function acc2SetMedTab(id){ACC2_UI.deptMedTab=id;renderMedicationAccountability()};
window.acc2SubmitUsage=async function(assignmentId){if(!acc2DepartmentRole())return toast('Department access required.','err');var a=acc2Assignment(assignmentId,acc2Assignments());if(!a||a.active===false||String(a.deptId)!==String(CU.deptId))return toast('Custody is not available for this department.','err');var blocked=acc2SubmissionBlocked(a.deptId,a.id);if(blocked)return toast('Entry is paused while '+blocked.name+' is being replaced.','err');function read(kind){var el=document.querySelector('[data-acc2="'+kind+'"][data-assignment="'+CSS.escape(String(assignmentId))+'"]');return el?String(el.value||'').trim():''}var inputMode=a.inputMode||'units',unitSize=acc2Number(a.unitSize)||0,doseAmount=null,units=0;if(inputMode==='dose'&&unitSize>0){doseAmount=acc2Number(read('dose'));if(!(doseAmount>0))return toast('Dose amount is required. / الجرعة مطلوبة.','err');units=Math.ceil(doseAmount/unitSize)}else{units=wholeUnits(read('units'));if(units===null)return toast('Units used must be a whole number. / عدد الوحدات يجب أن يكون رقماً صحيحاً.','err')}var date=read('date'),file=read('file'),doctor=read('doctor'),reasonLabel=read('reason'),note=read('note');if(!date||!file||!doctor||!reasonLabel)return toast('Consumption date, patient file, doctor and reason are required.','err');if(date>acc2Today())return toast('Consumption date cannot be in the future.','err');var effBal=acc2EffectiveBalance(a);if(units>effBal)return toast('Used units exceed available balance. Available: '+effBal+' / الوحدات تتجاوز الرصيد المتاح.','err');var weightKg=acc2Number(read('weight'))||null,pregnancyStatus=read('pregnancy')||null,patientAge=acc2Number(read('age'))||null;if(a.requireWeight==='required'&&!weightKg)return toast('Patient weight is required for this medicine. / وزن المريض مطلوب.','err');if((a.minAge||a.maxAge)&&!patientAge)return toast('Patient age is required for this medicine due to age restriction. / عمر المريض مطلوب لهذا الدواء.','err');if(a.minAge&&patientAge&&patientAge<a.minAge)return toast('⚠ Patient age ('+patientAge+' yrs) is below minimum ('+a.minAge+' yrs) for this medicine. / عمر المريض أقل من الحد الأدنى المطلوب.','err');if(a.maxAge&&patientAge&&patientAge>a.maxAge)return toast('⚠ Patient age ('+patientAge+' yrs) exceeds maximum ('+a.maxAge+' yrs) for this medicine. / عمر المريض يتجاوز الحد الأقصى المسموح.','err');if(a.genderRestriction&&a.genderRestriction!=='any'){var gender=read('gender');if(!gender)return toast('Patient gender confirmation is required for this medicine. / تأكيد جنس المريض مطلوب.','err')}/* Submitted through the callable rather than a direct state write: it appends
   inside a transaction (concurrent submissions from other departments can no
   longer overwrite each other) and re-checks the balance server-side, so the
   limit holds even against a hand-crafted request. */
  var _tmpId='__opt_'+Date.now();
  var _optimistic={id:_tmpId,assignmentId:a.id,deptId:String(CU.deptId||''),medName:a.medName,units:units,consumptionDate:date,patientFile:file,doctor:doctor,reasonLabel:reasonLabel,note:note,status:'pending_pharmacy',submittedAt:new Date().toISOString(),submittedBy:String((CU.name||CU.email||''))};
  acc2ReflectUsageLocally(function(rows){return rows.concat([_optimistic])});
  ACC2_UI.deptTab='submissions';
  renderMedicationAccountability();
  try{var submitted=await acc2CallMutation({action:'submitUsage',assignmentId:a.id,units:units,doseAmount:doseAmount,doseUnit:doseAmount?a.unitSizeLabel:null,weightKg:weightKg,pregnancyStatus:pregnancyStatus,patientAge:patientAge,consumptionDate:date,patientFile:file,doctor:doctor,reasonLabel:reasonLabel,note:note});acc2ReflectUsageLocally(function(rows){var without=rows.filter(function(x){return x.id!==_tmpId});return submitted&&submitted.row?without.concat([submitted.row]):without});toast('Submitted to pharmacy ✓ / تم الرفع للصيدلية ✓','succ');renderMedicationAccountability()}catch(error){acc2ReflectUsageLocally(function(rows){return rows.filter(function(x){return x.id!==_tmpId})});renderMedicationAccountability();console.error(error);toast(String(error&&error.message||error).replace(/^FirebaseError:\s*/,'')+'  — Go back to Custody to retry. / ارجع لتبويب العهدة وأعد المحاولة.','err')}};
function acc2UpdateDoseCalc(assignmentId,unitSize){var input=document.querySelector('[data-acc2="dose"][data-assignment="'+CSS.escape(String(assignmentId))+'"]'),display=document.querySelector('.acc2-dose-calc-'+CSS.escape(String(assignmentId)));if(!display)return;var dose=Number((input&&input.value)||0);display.textContent=dose>0&&unitSize>0?'→ '+Math.ceil(dose/unitSize)+' unit(s) will be deducted / سيُخصم '+Math.ceil(dose/unitSize)+' وحدة':'Enter dose → units auto-calculated'};
/* acc2ToggleAllReceipt / acc2CreateReceipt removed: both operated on
   .acc2-receipt-check inputs that no renderer produces, so neither was
   reachable. acc2CreateReceipt also wrote accountability_assignments_v2
   from a department session, which firestore.rules denies. The live
...
function acc2DeptNotifications(deptId){
  ...
  var html='';
  if(pending.length)html+='<div class="alert-banner-y" style="margin-bottom:8px">⏳ '+pending.length+' submission(s) pending pharmacy review / بانتظار مراجعة الصيدلية: '+pending.map(function(u){var lbl=u.medName?esc(u.medName)+'&nbsp;('+acc2FmtDate(u.consumptionDate)+')':esc(u.planName||'Plan')+'&nbsp;— File: '+esc(u.patientFile||'—');return '<span class="chip" style="background:rgba(255,193,7,.2)">'+lbl+'</span>';}).join(' ')+'</div>';
  if(rejected.length)html+='<div class="alert-banner" style="margin-bottom:8px">❌ '+rejected.length+' submission(s) rejected by pharmacy / مرفوض من الصيدلية: '+rejected.map(function(u){var lbl=u.medName?esc(u.medName)+'&nbsp;('+acc2FmtDate(u.consumptionDate)+')':esc(u.planName||'Plan')+'&nbsp;— File: '+esc(u.patientFile||'—');return '<span class="chip" style="background:rgba(220,53,69,.2)">'+lbl+'</span>';}).join(' ')+'</div>';
  return html;
}
window.renderMedicationAccountability=function(){r17InstallActions(document.body);var root=E('r17-accountability-root');if(!root)return;if(acc2DepartmentRole()&&!acc2EffectiveMaster()){var deptId=String((window.CU&&CU.deptId)||'');if(!deptId){root.innerHTML='<div class="alert-banner">Department account is not linked to a department.</div>';return}var deptContent;if(ACC2_UI.deptTab==='plans'){if(typeof window.acc2IndependentRegimensTab==='function'){try{deptContent=window.acc2IndependentRegimensTab()}catch(e){console.error('Plans tab error:',e);deptContent='<div class="alert-banner">⚠ Plans error: '+esc(String(e&&e.message||e))+'</div>'}}else{deptContent='<div class="acc2-empty">Plans not loaded yet.</div>'}}else if(ACC2_UI.deptTab==='submissions'){deptContent=acc2MySubmissionsTab(deptId)}else{deptContent=acc2DepartmentCards(deptId)}root.innerHTML=acc2DeptNotifications(deptId)+acc2DeptTabs()+deptContent;acc2LockOutpatientSelectors();return}if(!canAccManage()){root.innerHTML='<div class="alert-banner">Medication Accountability is available only to the Master, Pharmacy Director, Inpatient Pharmacy Supervisor, Pharmacy Staff, and department employees. / الصفحة غير متاحة لهذا الدور.</div>';return}var content=ACC2_UI.adminTab==='review'?acc2ReviewTab():ACC2_UI.adminTab==='handover'?acc2HandoverTab():ACC2_UI.adminTab==='regimens'?acc2IndependentRegimensContent():acc2CustodyTab();root.innerHTML=acc2AdminStats()+acc2AdminTabs()+content;if(ACC2_UI.adminTab==='review')r17AccRenderVerify();acc2LockOutpatientSelectors()};
...
function renderStorageBuilder(counts){
  ...
  if(rows){rows.innerHTML=counts.map(function(count,index){
    var label=rowLetter(index);
    return '<div class="r21-storage-shelf-config"><div class="r21-storage-shelf-label">Shelf '+esc(label)+'<br><small>الرف '+(index+1)+'</small></div><div class="fg"><label>Cells on this shelf / عدد الخانات</label><input type="number" min="1" max="50" value="'+count+'" data-storage-cell-count="'+index+'" data-inputact="controlledStorageCountsChanged"></div><div class="r21-storage-shelf-codes">'+Array.from({length:count},function(_,cellIndex){return '<span class="r21-storage-code">'+esc(label+(cellIndex+1))+'</span>';}).join('')+'</div></div>';
  }).join('');}
  var preview=E('controlled-storage-code-preview');if(preview)preview.innerHTML=storagePreviewHtml(counts);
}
function controlledStorageCountsChanged(){
...
function controlledStorageEditUnit(unitId){
...
window.renderControlledStorage=async function(){
  ...
  if(unitFilter==='near_expiry_30'){
    ...
    E('controlled-storage-root').innerHTML='<div class="card"><div class="ch"><span class="ct">Near-expiry medicines (≤30 days) / الأدوية القريبة من الانتهاء (خلال 30 يومًا)</span></div>'+
      '<div class="cb">'+
      (nearExpMeds.length?
        '<div class="tw"><table><thead><tr><th>Medicine</th><th>Cell / الخانة</th></tr></thead><tbody>'+
  ...
  E('controlled-storage-root').innerHTML=units.map(function(unit){
    var actions='<div class="r21-storage-unit-actions"><button class="btn bg bxs" type="button" data-clickact="controlledStorageEditUnit" data-a1="'+unit.id+'">Edit layout</button><button class="btn bg bxs" type="button" data-clickact="controlledStoragePrint" data-a1="'+unit.id+'" data-a2="list">Print list</button><button class="btn bg bxs" type="button" data-clickact="controlledStoragePrint" data-a1="'+unit.id+'" data-a2="map">Print map</button><button class="btn bd2c bxs" type="button" data-clickact="controlledStorageDeleteUnit" data-a1="'+unit.id+'">Delete</button></div>';
...
function controlledStorageValidateSearch(input){
...
async function controlledStorageAssignByName(input){
...
async function controlledStorageAssign(unitId,rowIndex,cellIndex,medId){
...
async function controlledStorageDeleteUnit(unitId){
...
async function controlledStoragePrint(id,mode){
  var data=storageState();
...
/* The manual-receipt fallback was removed: it hunted for .acc2-receipt-check
   elements that no renderer has produced since the temporary dual-QR handover
   became the only receipt path, so it polled the DOM every second for the life
   of the session and never attached anything. Its target, acc2CreateReceipt,
   also wrote accountability_assignments_v2 under a department session, which
   firestore.rules denies — it would have failed had it ever been reachable.
   Receipts are completed through the dual-QR flow (admin SDK, transactional). */
/* The selector lock ran on a 700ms poll for the life of the session. It only
   matters right after the accountability page rebuilds its markup, which is
   where it is now called from — both the department and the admin branch of
   renderMedicationAccountability. A scoped supervisor therefore has the
   department pinned from the first paint rather than up to 700ms later. */
...
/* Internal classification colors — status, urgency, dosage form */
var PI_INT_COLORS_KEY='pharm_inv_int_colors_v1';
var PI_INT_CLS={
  status:{
    label:'Availability Status / حالة التوفر',
    options:[
      {key:'available',label:'AVAILABLE',def:'#7aaa5e'},
      {key:'inpatient',label:'INPATIENT only',def:'#e8c84a'},
      {key:'unavailable',label:'UNAVAILABLE',def:'#8b3a2a'},
      {key:'wasfaty',label:'AVAILABLE Via Wasfaty',def:'#a87fb8'},
      {key:'central',label:'Central medication (report needed)',def:'#00bcd4'},
      {key:'narcotic',label:'NARCOTIC PRESCRIPTION',def:'#7a6a1e'},
      {key:'not_wasfaty',label:'Not AVAILABLE Via Wasfaty',def:'#e8845a'},
      {key:'low_stock',label:'Low Stock',def:'#e87a30'}
    ]
  },
  urgency:{
    label:'Urgency of Medication / أولوية الطلب',
    options:[
      {key:'urg_low',label:'Low priority',def:'#d4e8c4'},
      {key:'urg_medium',label:'Medium priority',def:'#f5e098'},
      {key:'urg_high',label:'High Priority',def:'#8b3a2a'}
    ]
  },
  dosageForm:{
    label:'Dosage Form / الشكل الصيدلاني',
    options:[
      {key:'df_drops',label:'Drops',def:'#c8e6c9'},
      {key:'df_tablets',label:'Tablets',def:'#fff9c4'},
      {key:'df_syrups',label:'Syrups',def:'#ffe0b2'},
      {key:'df_applicable',label:'Applicable form',def:'#e8f5e9'},
      {key:'df_psychiatric',label:'Psychiatric',def:'#e8eaf6'},
      {key:'df_nebulizer',label:'Nebulizer',def:'#e0f7fa'},
      {key:'df_misc',label:'Miscellaneous',def:'#f5f5f5'}
    ]
  }
};
window.PI_INT_CLS=PI_INT_CLS;
function piIntGetColors(){
  var saved={};try{saved=(window.S&&window.S.g&&window.S.g(PI_INT_COLORS_KEY))||{}}catch(e){console.warn('g was skipped after an error; the rest of this screen still renders.', e);}
  var result={};
  Object.keys(PI_INT_CLS).forEach(function(grp){
    PI_INT_CLS[grp].options.forEach(function(o){result[o.key]=saved[o.key]||o.def});
  });
  return result;
}
window.piIntGetColors=piIntGetColors;
...
function acc2InjectClassColors(){
  var saved={};try{saved=(window.S&&window.S.g&&window.S.g(CC_COLORS_KEY))||{}}catch(e){console.warn('g was skipped after an error; the rest of this screen still renders.', e);}
...
function acc2IsPharmDir(){
  var role=window.fsEffectiveRole?window.fsEffectiveRole():String((window.CU&&window.CU.role)||'');
  return role==='pharmacy'||acc2IsMaster();
}
window.renderClassColorSettings=function(){
  var pg=document.getElementById('pg-class-colors');if(!pg)return;
  var canEdit=acc2IsPharmDir();
  if(!canEdit){pg.innerHTML='<div class="alert-banner">Pharmacy Director or Master access required / مطلوب صلاحية مدير الصيدلية أو الماستر</div>';if(typeof window.injectInvTabBar==='function')window.injectInvTabBar('pg-class-colors');return}
  var saved={};try{saved=(window.S&&window.S.g&&window.S.g(CC_COLORS_KEY))||{}}catch(e){console.warn('g was skipped after an error; the rest of this screen still renders.', e);}
  var c={ha:saved.ha||CC_DEFAULTS.ha,haz:saved.haz||CC_DEFAULTS.haz,lasa:saved.lasa||CC_DEFAULTS.lasa,ref:saved.ref||CC_DEFAULTS.ref};
  ...
  // Internal classification color pickers
  var intColors=piIntGetColors();
  var intSections=Object.keys(PI_INT_CLS).map(function(grp){
    var g=PI_INT_CLS[grp];
    var pickers=g.options.map(function(o){return picker('pi-'+o.key,o.label,intColors[o.key]||o.def)}).join('');
    return '<div style="margin-bottom:14px"><div style="font-weight:600;font-size:13px;margin-bottom:8px;color:var(--tx2)">'+g.label+'</div><div class="acc2-form-grid">'+pickers+'</div></div>';
  }).join('');

  pg.innerHTML=
    '<div class="card" style="margin-bottom:12px"><div class="ch"><span class="ct">Classification Badge Colors / ألوان شارات التصنيف الأساسية</span><span style="font-size:11px;opacity:.6">Pharmacy Director + Master</span></div><div class="cb">'+
    '<div class="fhint" style="margin-bottom:12px">HA / HAZ / LASA / Refrigerated — used across all inventory views.</div>'+
    '<div class="acc2-form-grid">'+picker('ha','🔴 High Alert',c.ha)+picker('haz','⚠ Hazard',c.haz)+picker('lasa','🔵 LASA',c.lasa)+picker('ref','❄ Refrigerator',c.ref)+'</div>'+
    '<div style="margin-top:14px"><button class="btn bp" data-clickact="acc2SaveClassColors">Save / حفظ</button> <button class="btn bg bsm" data-clickact="acc2ResetClassColors">Reset / إعادة الافتراضي</button></div></div></div>'+
    '<div class="card"><div class="ch"><span class="ct">🏥 Pharmacy Inventory — Internal Classification Colors / ألوان التصنيفات الداخلية</span><span style="font-size:11px;opacity:.6">Pharmacy Director + Master</span></div><div class="cb">'+
    '<div class="fhint" style="margin-bottom:14px">Colors for Status, Urgency, and Dosage Form badges in Pharmacy Inventory. Each medicine selects at most one from each group.</div>'+
    intSections+
    '<div style="margin-top:14px"><button class="btn bp" data-clickact="acc2SaveIntColors">Save Internal Colors / حفظ</button> <button class="btn bg bsm" data-clickact="acc2ResetIntColors">Reset / إعادة الافتراضي</button></div>'+
    '</div></div>';
  if(typeof window.injectInvTabBar==='function')window.injectInvTabBar('pg-class-colors');
  // Bind hex preview for all pickers
  ...
  Object.keys(PI_INT_CLS).forEach(function(grp){PI_INT_CLS[grp].options.forEach(function(o){var el=document.getElementById('cc-pi-'+o.key);if(el)el.addEventListener('input',function(){var h=document.getElementById('cc-pi-'+o.key+'-hex');if(h)h.textContent=el.value})})});
};
async function acc2SaveIntColors(){
  if(!acc2IsPharmDir())return toast('No permission','err');
  var result={};var valid=true;
  Object.keys(PI_INT_CLS).forEach(function(grp){PI_INT_CLS[grp].options.forEach(function(o){var el=document.getElementById('cc-pi-'+o.key);if(el&&/^#[0-9a-fA-F]{6}$/.test(el.value)){result[o.key]=el.value}else{valid=false}})});
  if(!valid)return toast('Invalid color — use the color picker','err');
  try{await window.S.s(PI_INT_COLORS_KEY,result);toast('Internal colors saved ✓','succ')}catch(e){toast(String(e&&e.message||e),'err')}
};
async function acc2ResetIntColors(){
  if(!acc2IsPharmDir())return;
  var defs={};Object.keys(PI_INT_CLS).forEach(function(grp){PI_INT_CLS[grp].options.forEach(function(o){defs[o.key]=o.def})});
  try{await window.S.s(PI_INT_COLORS_KEY,defs);toast('Reset ✓','succ');window.renderClassColorSettings()}catch(e){toast(String(e&&e.message||e),'err')}
};
async function acc2SaveClassColors(){
  if(!acc2IsPharmDir())return toast('Pharmacy Director or Master required.','err');
  var result={};var valid=true;
...
async function acc2ResetClassColors(){
  if(!acc2IsPharmDir())return;
