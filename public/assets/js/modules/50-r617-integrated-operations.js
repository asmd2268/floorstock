...
function acc2MySubmissionsTab(deptId){
  ...
  return filterBar+'<div class="card"><div class="ch"><span class="ct">📝 My Submissions / طلباتي</span><div class="fhint">Submissions disappear from this log after the receipt handover is completed. / يختفي الطلب بعد إتمام محضر الاستلام والتسليم.</div></div><div class="cb"><div class="tw"><table class="acc2-table" style="font-size:12px"><thead><tr><th>TYPE / النوع</th><th>MEDICINE(S) / الأدوية</th><th>DATE / التاريخ</th><th>PATIENT FILE / الملف</th><th>DOCTOR / الطبيب</th><th>STATUS / الحالة</th><th></th></tr></thead><tbody>'+
    all.map(function(u){
      ...
      return '<tr><td>'+typeBadge+'</td><td><b>'+u.meds+'</b></td><td>'+esc(u.date)+'</td><td>'+esc(u.file)+'</td><td>'+esc(u.doctor)+'</td><td><span class="badge '+(statusCls[u.status]||'bgr')+'">'+(statusLbl[u.status]||esc(u.status))+'</span>'+noteHtml+'</td><td>'+cancelBtn+'</td></tr>';
...
function acc2PendingUsageTable(deptId){var usage=acc2Usage().filter(function(u){return String(u.deptId)===String(deptId)&&u.status==='pending_pharmacy'});if(!usage.length)return '';return '<div class="card" style="border-left:4px solid var(--yellow,#f39c12);margin-bottom:14px"><div class="ch"><span class="ct">⏳ Pending pharmacy review / بانتظار مراجعة الصيدلية</span></div><div class="cb"><div class="tw"><table class="acc2-table" style="font-size:12px"><thead><tr><th>Medicine / الدواء</th><th>Units / الوحدات</th><th>Date / التاريخ</th><th>Patient file / الملف</th><th>Doctor / الطبيب</th><th>Reason / السبب</th><th></th></tr></thead><tbody>'+usage.map(function(u){return '<tr><td><b>'+esc(u.medName||'—')+'</b></td><td>'+esc(u.units)+'</td><td>'+esc(u.consumptionDate||'—')+'</td><td>'+esc(u.patientFile||'—')+'</td><td>'+esc(u.doctor||'—')+'</td><td>'+esc(u.reasonLabel||'—')+(u.note?'<div class="fhint">'+esc(u.note)+'</div>':'')+'</td><td><button class="btn bd2c bxs" type="button" onclick="acc2CancelUsage(\''+esc(u.id)+'\')">✕ Cancel / إلغاء</button></td></tr>'}).join('')+'</tbody></table></div></div></div>'}
