...
window.acc2CancelUsage=async function(id,redirectTab){if(!acc2DepartmentRole())return toast('Department access required.','err');var usageList=acc2Usage(),u=usageList.find(function(x){return String(x.id)===String(id)});if(!u||!(u.status==='pending_pharmacy'||u.status==='rejected'))return toast('This submission can no longer be modified. / لا يمكن تعديل هذا الطلب.','err');if(String(u.deptId)!==String((window.CU&&CU.deptId)||''))return toast('Access denied.','err');var restores=u.status==='rejected'||u.status==='pending_pharmacy';var msg=restores?'Delete this submission and restore '+acc2Number(u.units)+' unit(s) to balance? / حذف الطلب وإرجاع '+acc2Number(u.units)+' وحدة للرصيد؟':'Delete this submission? / حذف هذا الطلب؟';var ok=typeof window.uiConfirm==='function'?await window.uiConfirm(msg):window.confirm(msg);if(!ok)return;var newUsage=usageList.filter(function(x){return String(x.id)!==String(id)});var originalList=acc2Assignments(),list=originalList.map(function(x){return Object.assign({},x)}),a=acc2Assignment(u.assignmentId,list);if(a&&restores){a.balance=acc2Number(a.balance)+acc2Number(u.units);a.updatedAt=now();a.updatedBy=actor().name}try{await S.s(ACC2_ASSIGNMENTS_KEY,list);try{await S.s(ACC2_USAGE_KEY,newUsage)}catch(error){await S.s(ACC2_ASSIGNMENTS_KEY,originalList);throw error}toast('Deleted; '+acc2Number(u.units)+' unit(s) restored ✓','succ');if(redirectTab)ACC2_UI.deptTab=redirectTab;renderMedicationAccountability()}catch(error){toast(String(error&&error.message||error),'err')}};
