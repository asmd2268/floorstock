...
window.acc2AddExpiryRow=function(assignmentId){var container=document.getElementById('acc2-expiry-rows-'+CSS.escape(String(assignmentId)));if(!container)return;var row=document.createElement('div');row.className='acc2-expiry-row fl g6 ic';row.style.marginBottom='4px';row.innerHTML='<input class="acc2-expiry-date" type="date" style="flex:1"><input class="acc2-expiry-qty" type="number" min="1" step="1" placeholder="Qty / الكمية (اختياري)" style="width:110px"><button class="btn bd2c bxs" type="button" onclick="acc2RemoveExpiryRow(this)">✕</button>';container.appendChild(row)};
window.acc2RemoveExpiryRow=function(btn){var row=btn.closest('.acc2-expiry-row');if(row)row.remove()};
...
function acc2MySubmissionsTab(deptId){
  ...
  if(!all.length)return '<div class="acc2-empty" style="padding:32px 0;text-align:center">No active submissions.<br>لا توجد طلبات معلقة أو معتمدة.<div class="fhint" style="margin-top:6px">Entries disappear once a receipt handover is recorded. / يختفي الطلب عند تأكيد استلام الأدوية.</div></div>';
  ...
  return '<div class="card"><div class="ch"><span class="ct">📝 My Submissions / طلباتي</span><div class="fhint">Submissions disappear from this log after the receipt handover is completed. / يختفي الطلب بعد إتمام محضر الاستلام والتسليم.</div></div><div class="cb"><div class="tw"><table class="acc2-table" style="font-size:12px"><thead><tr><th>Type / النوع</th><th>Medicine(s) / الأدوية</th><th>Date / التاريخ</th><th>Patient file / الملف</th><th>Doctor / الطبيب</th><th>Status / الحالة</th><th></th></tr></thead><tbody>'+
    all.map(function(u){
      var typeBadge=u._type==='plan'?'<span class="badge bbl" style="font-size:10px">خطة</span>':'<span class="badge bgn" style="font-size:10px">عهدة</span>';
      var cancelBtn='';
...
function acc2BuildDeptStatsHTML(deptId,assignments,usage){var waiting=usage.filter(function(u){return u.status==='approved_waiting_receipt'});var totalEffective=assignments.reduce(function(s,a){var p=usage.filter(function(u){return String(u.assignmentId)===String(a.id)&&(u.status==='pending_pharmacy'||u.status==='approved_waiting_receipt')}).reduce(function(t,u){return t+acc2Number(u.units)},0);return s+Math.max(0,acc2Number(a.balance)-p)},0);return '<div id="acc2-dept-stats" class="acc2-stats"><div class="sc"><div class="sl">Assigned medicines</div><div class="sv">'+assignments.length+'</div></div><div class="sc"><div class="sl">Approved custody</div><div class="sv">'+acc2Sum(assignments,'quota')+'</div></div><div class="sc"><div class="sl">Available balance</div><div class="sv">'+totalEffective+'</div></div><div class="sc"><div class="sl">Waiting receipt</div><div class="sv">'+acc2Sum(waiting)+'</div><div class="ss">'+waiting.length+' requests</div></div></div>';}
function acc2BuildCardHTML(a,usage,deptId){var blocked=acc2SubmissionBlocked(deptId,a.id),pendingForA=usage.filter(function(u){return String(u.assignmentId)===String(a.id)&&(u.status==='pending_pharmacy'||u.status==='approved_waiting_receipt')}).reduce(function(s,u){return s+acc2Number(u.units)},0),remaining=Math.max(0,acc2Number(a.balance)-pendingForA);return '<div id="acc2-card-'+esc(a.id)+'" class="card acc2-med-card '+(blocked?'blocked':'')+'"><div class="ch"><div><span class="ct">'+esc(a.medName)+' — عهدتهم '+acc2Number(a.quota)+'</span><div class="fhint">Available balance / الرصيد المتاح: <b>'+remaining+'</b>'+(pendingForA?' <span style="color:var(--yellow,#f39c12)">('+pendingForA+' in use / محجوز)</span>':'')+' of '+acc2Number(a.quota)+'</div></div><span class="badge '+(remaining>0?'bgn':'brd')+'">'+remaining+' / '+acc2Number(a.quota)+'</span></div><div class="cb">'+(blocked?'<div class="alert-banner">Entry is paused because '+esc(blocked.name)+' is being replaced by pharmacy. / الإدخال متوقف مؤقتًا لاستبدال الخطة.</div>':'')+'<div style="font-weight:600;padding:4px 0;margin-bottom:6px">Submit usage / إدخال استهلاك</div><div class="acc2-form-grid">'+(a.inputMode==='dose'&&acc2Number(a.unitSize)>0?'<div class="fg"><label>Dose given / الجرعة المُعطاة * ('+esc(a.unitSizeLabel||'mg')+')</label><input data-acc2="dose" data-assignment="'+esc(a.id)+'" type="number" min="0.001" step="any" oninput="acc2UpdateDoseCalc(\''+esc(a.id)+'\','+acc2Number(a.unitSize)+')"><div class="fhint acc2-dose-calc-'+esc(a.id)+'">Enter dose → units auto-calculated (1 unit = '+acc2Number(a.unitSize)+esc(' '+(a.unitSizeLabel||'mg'))+')</div></div>':'<div class="fg"><label>Units used / الوحدات المستعملة *</label><input data-acc2="units" data-assignment="'+esc(a.id)+'" type="number" min="0.001" max="'+remaining+'" step="any"></div>')+(a.requireWeight&&a.requireWeight!=='no'?'<div class="fg"><label>Patient weight / وزن المريض (kg)'+(a.requireWeight==='required'?' *':' (optional)')+'</label><input data-acc2="weight" data-assignment="'+esc(a.id)+'" type="number" min="1" max="500" step="0.1" placeholder="kg"></div>':'')+(a.genderRestriction&&a.genderRestriction!=='any'?'<div class="fg"><div class="alert-banner-y">⚠ '+esc(a.genderRestriction==='male'?'Male patients only / ذكور فقط':'Female patients only / إناث فقط')+'</div><label>Patient gender / جنس المريض *</label><select data-acc2="gender" data-assignment="'+esc(a.id)+'"><option value="">Select...</option><option value="male">Male / ذكر</option><option value="female">Female / أنثى</option></select></div>':'')+(a.pregnancyAllowed&&a.pregnancyAllowed!=='any'?'<div class="fg"><div class="alert-banner-y">⚠ '+esc({caution:'Use with caution / بحذر',avoid:'Avoid in pregnancy / يُتجنب',contraindicated:'Contraindicated / موانع'}[a.pregnancyAllowed]||a.pregnancyAllowed)+'</div><label>Pregnancy status / حالة الحمل</label><select data-acc2="pregnancy" data-assignment="'+esc(a.id)+'"><option value="not_pregnant">Not pregnant / غير حامل</option><option value="trimester_1">1st trimester / الثلث الأول</option><option value="trimester_2">2nd trimester / الثلث الثاني</option><option value="trimester_3">3rd trimester / الثلث الثالث</option></select></div>':'')+(a.minAge||a.maxAge?'<div class="fg"><div class="alert-banner-y" style="margin-bottom:6px">⚠ Age restriction / تقييد العمر: '+(a.minAge&&a.maxAge?a.minAge+' – '+a.maxAge+' years / سنة':a.minAge?'≥ '+a.minAge+' years / سنة':'≤ '+a.maxAge+' years / سنة')+'</div><label>Patient age / عمر المريض (years) '+(a.minAge||a.maxAge?'*':'')+'</label><input data-acc2="age" data-assignment="'+esc(a.id)+'" type="number" min="0" max="120" step="1" placeholder="years / سنة"></div>':'')+'<div class="fg"><label>Consumption date / تاريخ الاستهلاك *</label><input data-acc2="date" data-assignment="'+esc(a.id)+'" type="date" max="'+acc2Today()+'"></div><div class="fg"><label>Patient file number / رقم الملف *</label><input data-acc2="file" data-assignment="'+esc(a.id)+'"></div><div class="fg"><label>Doctor recorded in system / الطبيب المسجل *</label><input data-acc2="doctor" data-assignment="'+esc(a.id)+'"></div><div class="fg"><label>Usage reason / سبب الاستهلاك *</label><select data-acc2="reason" data-assignment="'+esc(a.id)+'"><option value="">Select reason...</option>'+(a.reasons||[]).map(function(r){return '<option value="'+esc(r)+'">'+esc(r)+'</option>'}).join('')+'</select></div><div class="fg"><label>Notes / ملاحظات</label><input data-acc2="note" data-assignment="'+esc(a.id)+'"></div></div><button class="btn bp" style="margin-top:10px" '+(blocked||remaining<=0?'disabled':'')+' onclick="acc2SubmitUsage(\''+esc(a.id)+'\')">Submit to pharmacy / رفع للصيدلية</button></div><details style="margin-top:10px;border-top:1px solid var(--br);padding-top:10px"><summary style="cursor:pointer;font-size:12px;opacity:.7;list-style:none;user-select:none">📅 Medicine expiry batches / دفعات انتهاء الصلاحية</summary><div style="margin-top:8px"><div id="acc2-expiry-rows-'+esc(a.id)+'" style="margin-bottom:6px">'+acc2GetExpiryBatches(a.id).map(function(b,i){return '<div class="acc2-expiry-row fl g6 ic" style="margin-bottom:4px"><input class="acc2-expiry-date" type="date" value="'+esc(b.date||'')+'" style="flex:1"><input class="acc2-expiry-qty" type="number" min="1" step="1" placeholder="Qty / الكمية (اختياري)" value="'+esc(b.qty||'')+'" style="width:110px"><button class="btn bd2c bxs" type="button" onclick="acc2RemoveExpiryRow(this)">✕</button></div>'}).join('')+(acc2GetExpiryBatches(a.id).length===0?'<div class="acc2-expiry-row fl g6 ic" style="margin-bottom:4px"><input class="acc2-expiry-date" type="date" style="flex:1"><input class="acc2-expiry-qty" type="number" min="1" step="1" placeholder="Qty / الكمية (اختياري)" style="width:110px"><button class="btn bd2c bxs" type="button" onclick="acc2RemoveExpiryRow(this)">✕</button></div>':'')+'</div><div class="fl g6"><button class="btn bg bxs" type="button" onclick="acc2AddExpiryRow(\''+esc(a.id)+'\')">+ Add batch / إضافة دفعة</button><button class="btn bp bxs" type="button" onclick="acc2SaveExpiryBatches(\''+esc(a.id)+'\')">Save / حفظ</button></div></div></details></div></div></div>'}
function acc2DepartmentCards(deptId){var assignments=acc2DeptAssignments(deptId),usage=acc2Usage().filter(function(u){return String(u.deptId)===String(deptId)});return '<div id="acc2-expiry-banner">'+acc2ExpiryAlertBanner(deptId)+'</div>'+acc2BuildDeptStatsHTML(deptId,assignments,usage)+acc2RegimenNotice(deptId)+'<div class="acc2-custody-list">'+assignments.map(function(a){return acc2BuildCardHTML(a,usage,deptId)}).join('')+'</div>';}
...
window.renderClassColorSettings=function(){
  ...
  if(!acc2IsMaster()){pg.innerHTML='<div class="alert-banner">Master access required / مطلوب صلاحية الماستر</div>';if(typeof window.injectInvTabBar==='function')window.injectInvTabBar('pg-class-colors');return}
  var saved={};try{saved=(window.S&&window.S.g&&window.S.g(CC_COLORS_KEY))||{}}catch(e){}
  var c={ha:saved.ha||CC_DEFAULTS.ha,haz:saved.haz||CC_DEFAULTS.haz,lasa:saved.lasa||CC_DEFAULTS.lasa,ref:saved.ref||CC_DEFAULTS.ref};
