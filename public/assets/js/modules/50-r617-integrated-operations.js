...
function acc2AdminTabs(){var pending=acc2VisibleUsage().filter(function(u){return u.status==='pending_pharmacy'}).length;var tabs=[['custody','Custody / العهدة'],['review','Review / المراجعة'+(pending?'<span class="acc2-tab-badge">'+pending+'</span>':'')],['handover','Receipt / الاستلام'],['regimens','Plans / الخطط']];return '<div class="acc2-tabs">'+tabs.map(function(t){return '<button class="acc2-tab '+(ACC2_UI.adminTab===t[0]?'on':'')+'" type="button" onclick="acc2SetAdminTab(\''+t[0]+'\')">'+t[1]+'</button>'}).join('')+'</div>'}
function acc2DeptTabs(){var tabs=[['custody','🏥 Custody / العهدة'],['plans','📋 Plans / الخطط العلاجية'],['submissions','📝 My Submissions / طلباتي']];return '<div class="acc2-tabs" data-acc2-dept-tabs="1">'+tabs.map(function(t){return '<button class="acc2-tab '+(ACC2_UI.deptTab===t[0]?'on':'')+'" type="button" onclick="acc2SetDeptTab(\''+t[0]+'\')">'+t[1]+'</button>'}).join('')+'</div>'}
  ...
  return (
    '<div class="card" style="border-top:3px solid var(--ac,#2e7dff)">'+
      '<div class="ch" style="flex-wrap:wrap;gap:8px">'+
        ...
        '</div>'+
        (edit?'<div class="fl g6"><button class="btn bp bsm" type="button" onclick="acc2SaveAssignment()">Save / حفظ</button><button class="btn bg bsm" type="button" onclick="acc2CancelAssignmentEdit()">Cancel / إلغاء</button></div>':'')+
      '</div>'+
      '<div class="cb" style="display:flex;flex-direction:column;gap:0">'+
        ...
        '<div style="background:var(--bg2);border-radius:8px;padding:14px 16px;margin-bottom:14px">'+
          ...
          '<div class="grid g3" style="gap:12px">'+
            '<div class="fg"><label>Department / القسم <span style="color:#e74c3c">*</span></label>'+
              '<select id="acc2-assignment-dept" onchange="acc2RefreshMedicationList()"><option value="">اختر القسم...</option>'+
        ...
        '<div class="fl g8">'+
          '<button class="btn bp" type="button" onclick="acc2SaveAssignment()">'+(edit?'Save changes / حفظ التعديلات':'Create custody / إنشاء العهدة')+'</button>'+
          (edit?'<button class="btn bg" type="button" onclick="acc2CancelAssignmentEdit()">Cancel / إلغاء</button>':'')+
        '</div>'+
      '</div>'+
    '</div>'+
    ...
    '<div class="card">'+
      '<div class="ch" style="flex-wrap:wrap;gap:8px">'+
        ...
        '<div class="fl g6 ic">'+
          '<select style="min-width:160px" onchange="acc2SetCustFilter(\'dept\',this.value)"><option value="">كل الأقسام</option>'+depts().map(function(d){return '<option value="'+esc(d.id)+'" '+(cf.dept===d.id?'selected':'')+'>'+esc(d.name)+'</option>'}).join('')+'</select>'+
          '<select style="min-width:110px" onchange="acc2SetCustFilter(\'status\',this.value)"><option value="" '+(cf.status===''||!cf.status?'selected':'')+'>All / الكل</option><option value="active" '+(cf.status==='active'?'selected':'')+'>Active</option><option value="inactive" '+(cf.status==='inactive'?'selected':'')+'>Inactive</option></select>'+
          ...
          '<tbody>'+tableRows.map(function(a){
            ...
            return '<tr>'+
              ...
              '<td style="white-space:nowrap">'+
                '<button class="btn bg bxs" onclick="acc2EditAssignment(\''+esc(a.id)+'\')">Edit</button> '+
                '<button class="btn '+(a.active===false?'bs':'bd2c')+' bxs" onclick="acc2ToggleAssignment(\''+esc(a.id)+'\')">'+(a.active===false?'Activate':'Deactivate')+'</button> '+
                '<button class="btn bd2c bxs" onclick="acc2DeleteAssignment(\''+esc(a.id)+'\')">Delete</button>'+
                (acc2EffectiveMaster()?'<button class="btn bs bxs" title="Reset balance to quota" onclick="acc2ResetBalance(\''+esc(a.id)+'\')">&#8635;</button>':'')+
              '</td>'+
            '</tr>';
...
window.acc2ShowExpirySection=function(assignmentId){var sec=document.getElementById('acc2-expiry-section-'+assignmentId);var btn=document.getElementById('acc2-expiry-reveal-'+assignmentId);if(sec)sec.hidden=false;if(btn)btn.hidden=true;if(sec){var d=sec.querySelector('.acc2-expiry-date');if(d)d.focus()}};
window.acc2AddExpiryRow=function(assignmentId){var container=document.getElementById('acc2-expiry-rows-'+CSS.escape(String(assignmentId)));if(!container)return;var row=document.createElement('div');row.className='acc2-expiry-row';row.innerHTML='<div class="acc2-expiry-row-inner"><span class="acc2-expiry-lbl">Expiry / تاريخ الانتهاء</span><input class="acc2-expiry-date" type="date" lang="en" dir="ltr" style="flex:1"><span class="acc2-expiry-lbl">Qty / الكمية</span><input class="acc2-expiry-qty" type="number" min="1" step="1" placeholder="—" style="width:80px"><button class="btn bd2c bxs" type="button" onclick="acc2RemoveExpiryRow(this)">✕</button></div>';container.appendChild(row)};
...
function acc2MySubmissionsTab(deptId){
  ...
  return filterBar+'<div class="card"><div class="ch"><span class="ct">📝 My Submissions / طلباتي</span><div class="fhint">Submissions disappear from this log after the receipt handover is completed. / يختفي الطلب بعد إتمام محضر الاستلام والتسليم.</div></div><div class="cb"><div class="tw"><table class="acc2-table" style="font-size:12px"><thead><tr><th>TYPE / النوع</th><th>MEDICINE(S) / الأدوية</th><th>DATE / التاريخ</th><th>PATIENT FILE / الملف</th><th>DOCTOR / الطبيب</th><th>STATUS / الحالة</th><th></th></tr></thead><tbody>'+
    all.map(function(u){
      ...
      var cancelBtn='';
      if(u.status==='pending_pharmacy'){cancelBtn=u._type==='plan'?'<button class="btn bd2c bxs" type="button" onclick="acc3CancelPlanUsage(\''+esc(u.id)+'\')">✕ إلغاء</button>':'<button class="btn bd2c bxs" type="button" onclick="acc2CancelUsage(\''+esc(u.id)+'\')">✕ إلغاء</button>'}
      else if(u.status==='rejected'){if(u._type==='plan'){cancelBtn='<div class="fl g4" style="flex-wrap:wrap;gap:4px"><button class="btn bd2c bxs" type="button" onclick="acc3CancelPlanUsage(\''+esc(u.id)+'\')">🗑 Delete / حذف</button><button class="btn bs bxs" type="button" onclick="acc3CancelPlanUsage(\''+esc(u.id)+'\',\'plans\')">✏ Edit & Resubmit / تعديل وإعادة إرسال</button></div>'}else{cancelBtn='<div class="fl g4" style="flex-wrap:wrap;gap:4px"><button class="btn bd2c bxs" type="button" onclick="acc2CancelUsage(\''+esc(u.id)+'\')">🗑 Delete / حذف</button><button class="btn bs bxs" type="button" onclick="acc2CancelUsage(\''+esc(u.id)+'\',\'custody\')">✏ Edit & Resubmit / تعديل وإعادة إرسال</button></div>'}}
...
window.PI_INT_COLORS_KEY=PI_INT_COLORS_KEY;
...
window.renderClassColorSettings=function(){
  ...
  pg.innerHTML=
    ...
    '<div style="margin-top:14px"><button class="btn bp" onclick="acc2SaveClassColors()">Save / حفظ</button> <button class="btn bg bsm" onclick="acc2ResetClassColors()">Reset / إعادة الافتراضي</button></div></div></div>'+
    ...
    intSections+
    '<div style="margin-top:14px"><button class="btn bp" onclick="acc2SaveIntColors()">Save Internal Colors / حفظ</button> <button class="btn bg bsm" onclick="acc2ResetIntColors()">Reset / إعادة الافتراضي</button></div>'+
    '</div></div>';
...
window.acc2SaveIntColors=async function(){
...
window.acc2ResetIntColors=async function(){
