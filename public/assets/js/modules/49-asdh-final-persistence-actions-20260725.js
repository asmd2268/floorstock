 ...
 * Original script position: 52
 * Original id: asdh-final-persistence-actions-20260725
...
/* Schedule and limits. */
window.saveReqWindow=async function(){var label=el('rwin-label').value.trim(),from=el('rwin-from').value,to=el('rwin-to').value;if(!label)return toast('Enter a label','err');if(!from||!to)return toast('Set start and end time','err');var days=Array.from(el('rwin-days').querySelectorAll('input:checked')).map(function(c){return +c.value});if(!days.length)return toast('Select at least one day','err');var wins=(getReqWindows()||[]).slice(),idx=el('rwin-edit-id').value,w={label:label,dept:el('rwin-dept').value,from:from,to:to,days:days,active:el('rwin-active').checked};if(idx!=='')wins[+idx]=w;else wins.push(w);try{await setReqWindows(wins);CM('mreq-window');renderSchedule();toast('Request window saved permanently ✓','succ')}catch(e){toast('Request window was not saved.','err')}};
window.toggleWindow=async function(i){var wins=(getReqWindows()||[]).map(function(x){return Object.assign({},x)});if(!wins[i])return;wins[i].active=!wins[i].active;try{await setReqWindows(wins);renderSchedule()}catch(e){toast('Window status was not saved.','err')}};
window.delWindow=async function(i){if(!await uiConfirm('Delete this window?'))return;var wins=(getReqWindows()||[]).slice();wins.splice(i,1);try{await setReqWindows(wins);renderSchedule();toast('Window deleted and saved ✓','info')}catch(e){toast('Window deletion was not saved.','err')}};
window.saveDispSlot=async function(){var label=el('dslot-label').value.trim(),time=el('dslot-time').value;if(!label)return toast('Enter a label','err');if(!time)return toast('Set dispense time','err');var days=Array.from(el('dslot-days').querySelectorAll('input:checked')).map(function(c){return +c.value});if(!days.length)return toast('Select at least one day','err');var slots=(getDispSlots()||[]).slice(),idx=el('dslot-edit-id').value,v={label:label,dept:el('dslot-dept').value,time:time,days:days,notes:el('dslot-notes').value.trim()};if(idx!=='')slots[+idx]=v;else slots.push(v);try{await setDispSlots(slots);CM('mdisp-slot');renderSchedule();toast('Dispense slot saved permanently ✓','succ')}catch(e){toast('Dispense slot was not saved.','err')}};
window.delSlot=async function(i){if(!await uiConfirm('Delete?'))return;var slots=(getDispSlots()||[]).slice();slots.splice(i,1);try{await setDispSlots(slots);renderSchedule();toast('Dispense slot deleted and saved ✓','info')}catch(e){toast('Dispense slot deletion was not saved.','err')}};
