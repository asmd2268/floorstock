...
function enhanceBuilder(){
  ...
  var hint=line.closest('.card').querySelector('.fhint');if(hint)hint.textContent='Each regimen must contain at least one medicine. Set the planned units for every medicine and the severity of this version.';
}
...
function printRegimen(id){
  ...
  var version=activeVersion(regimen),items=(version.items||[]),body=items.map(function(item,index){return '<tr><td>'+String(index+1)+'</td><td>'+esc(item.medName||'Medicine')+'</td><td>'+esc(item.qty)+' units / وحدة</td></tr>'}).join(''),qrRuntime=window.ASD_QR&&window.ASD_QR.printRuntimeScript?window.ASD_QR.printRuntimeScript():'',header=typeof window.officialPrintHeaderHTML==='function'?window.officialPrintHeaderHTML():'';
  if(!header)return window.toast&&window.toast('Save the official header and logo before printing the regimen.','err');
  var html='<!doctype html><html><head><meta charset="utf-8"><title>Approved regimen</title><style>@page{size:A4;margin:13mm 11mm 17mm}body{font-family:Arial,sans-serif;color:#111;margin:0}.head{display:flex;justify-content:space-between;gap:18px;align-items:center;border-bottom:2px solid #14532d;padding-bottom:10px}.head h1{margin:0;font-size:20px}.meta{margin:12px 0;background:#f2f8f3;padding:10px;border-radius:6px}.sev{font-weight:bold;color:#9f1239}table{width:100%;border-collapse:collapse;margin-top:12px}th,td{border:1px solid #555;padding:8px;text-align:left}th{background:#e7f3ea}.qr{text-align:center;margin-top:16px}.qr img{display:block;margin:0 auto}.foot{margin-top:14px;font-size:10px;color:#555}.official-print-footer{position:fixed;bottom:4mm;left:0;right:0;text-align:center;font:9px Arial;color:#555}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body>'+header+'<div class="head"><div><h1>'+esc(regimen.name||'Regimen')+'</h1><div>Approved treatment plan / الخطة العلاجية المعتمدة حاليًا</div></div><div class="qr">'+qr(payload(regimen),false)+'</div></div><div class="meta"><b>Department:</b> '+esc(deptName(regimen.deptId))+' &nbsp; | &nbsp; <b>Indication:</b> '+esc(regimen.infectionSource||'—')+'<br><b>Line:</b> '+esc(version.lineType||'—')+' &nbsp; | &nbsp; <span class="sev">Severity: '+esc(severityLabel(version.severity))+'</span></div><table><thead><tr><th>#</th><th>Medicine / العلاج</th><th>Planned quantity / الكمية المحددة</th></tr></thead><tbody>'+body+'</tbody></table><div class="foot">QR contains the active approved regimen and its planned quantities. Printed '+esc(new Date().toLocaleString())+'.</div><footer class="official-print-footer">By Ali AbuDahash</footer></body>'+qrRuntime+'</html>';
...
function wrapSave(){
  ...
  var wrapped=async function(){
    var checked=document.querySelectorAll('[data-acc2-regimen-assignment]:checked');if(checked.length<1){return window.toast&&window.toast('Select at least one medicine. / اختر علاجًا واحدًا على الأقل.','err')}
