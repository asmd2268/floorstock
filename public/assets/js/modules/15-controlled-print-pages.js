...
(function(){
  ...
  /* One printer for the whole project: core/print-window.js opens a blob: URL.
     This used to open a blank window and document.write() into it, which Safari
     does not reliably fire `load` on — only an unconditional timeout was saving
     the print at all. The printed document itself is unchanged: same stylesheet,
     same official header, same certification line. A caller that pre-opened a
     window inside the user's click still gets that window used. */
  var CTL_PRINT_CSS =
  ...
  window.ctlPrintHTML=function(title,body,preparedWindow){
    if(preparedWindow&&!preparedWindow.closed)window.__preOpenedPW=preparedWindow;
    var page='<div class="print-page">'+officialPrintHeaderHTML()+body+
      '<div class="electronic-cert">هذه القائمة معتمدة ومصدقة إلكترونيًا ولا تحتاج إلى ختم<br>'+
      'This list is electronically approved and certified and does not require a stamp.</div>'+
      '<div class="by">By Ali Abudahash</div></div>';
    return printDocument({ title: title, html: page, css: CTL_PRINT_CSS, brand: '' });
  };
})();
