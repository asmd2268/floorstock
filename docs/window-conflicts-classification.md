...
| الاسم | القرار | السبب |
|---|---|---|
| `fsR17MigrateMedicationIdentity` | **حُذف المنطق الحقيقي من وحدة 03** ✅ | تعليق صريح فوق التعطيل بوحدة 40 (`/* No login-time inventory mutation is permitted. */`) يعطّلها هي و`window.seed` و`repairImportedDepartmentAliases` معًا — سياسة أمان متعمّدة موثّقة، مو خطأ. الدالة الحقيقية بقيت "كود ميت موسوم" فقط، فحُذفت. |
| `ctlSetView` | **لا تغيير — سلوك وحدة 80 صحيح** ✅ | إضافة `normalizeViewC(v)` كانت ستكسر تبويب "Analytics" الجديد: قائمة `normalizeViewC` البيضاء (`overview/storage/departments/pharmacy/warehouse/catalog`) لا تعرف `'analytics'` (أُضيفت لاحقًا في وحدة 80)، فكانت سترفضه وترجع `'overview'` بصمت. نسخة 40 هي القديمة الناقصة، مو 80. |

كل الحالات الـ12 (بعد استبعاد `MASTER_EFFECTIVE`) محسومة الآن. `npm run verify` و`test:ui` (82/82) ناجحين بعد كل تعديل، وتحقّق حي عبر المتصفح لكل حالة.
